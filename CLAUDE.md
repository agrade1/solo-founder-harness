...
> `docs/WORKLOG.md`(4147줄)·`docs/DECISIONS.md`(1739줄)는 **존재한다** — 초기 커밋 `d2f220e`(2026-07-06)부터
> 있었고 **2026-08-13(M8)에서 멈췄다**. 둘 다 `init`이 생성하는 6문서 집합에 속하고(`src/commands/init.ts`),
> 이 레포의 것은 손으로 쓴 dogfooding 사본이다.
>
> **이 세션(V3 오케스트레이션 작업)의 갱신 대상은 아니다** — 실제 기록은 위 네 곳이 담고 있고 중복 문서를
> 만들지 않는다. 되살리려면 그 자체를 하나의 slice로 승인받아야 한다.
> **단, v1 문서 자동화 층에서는 `WORKLOG.md`가 살아 있는 산출 대상이다**(`src/core/handoff.ts`의
> 경로 계약이 worker에게 `<contextRoot>/docs/WORKLOG.md`에 쓰라고 지시한다). 위 문장은 **이 세션의
> 문서 의무**에 대한 것이지 그 계약을 뒤집지 않는다.
>
> (2026-08-19판은 "이 레포에 **존재하지 않는다**(M5~M9 내내 만들어지지 않았다)"라고 적었는데 **사실이
> 아니었다** — 작성 시점에도 두 파일은 있었다. 2026-08-23 M11에서 정정했다.
> **실상을 확인하지 않고 계약을 적은 사례로 남긴다.**)
