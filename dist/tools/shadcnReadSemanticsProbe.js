...
const EXPECTED_NS_TOOLS = deepFreeze(["get_add_command_for_items", "get_audit_checklist", "get_item_examples_from_registries", "get_project_registries", "list_items_in_registries", "search_items_in_registries", "view_items_in_registries"].map((n) => `mcp__${SHADCN_SERVER}__${n}`).sort());
// 독립 리터럴 contract(시작 시 CALL_PLAN 자기검증용 — 소스 변조 방지 defense-in-depth).
const CONTRACT_NAMES = ["get_project_registries", "list_items_in_registries", "search_items_in_registries", "view_items_in_registries", "get_item_examples_from_registries"];
const CONTRACT_ARGS = [
    {},
...
function assertCallPlanContract() {
    if (CALL_PLAN.length !== 5 || CONTRACT_NAMES.length !== 5)
        throw new ShadcnReadSemanticsError("bad_calls", "고정 호출은 정확히 5개여야 함");
    const seen = new Set();
    for (let i = 0; i < 5; i++) {
        const c = CALL_PLAN[i];
        if (c.name !== CONTRACT_NAMES[i])
            throw new ShadcnReadSemanticsError("contract_mismatch", "호출 이름/순서가 contract와 불일치");
        if (seen.has(c.name))
            throw new ShadcnReadSemanticsError("duplicate_call", "중복 호출");
        seen.add(c.name);
        if (FORBIDDEN_TOOLS.includes(c.name))
            throw new ShadcnReadSemanticsError("forbidden_call", "금지 도구 호출 시도");
        if (!EXPECTED_NS_TOOLS.includes(`mcp__${SHADCN_SERVER}__${c.name}`))
            throw new ShadcnReadSemanticsError("unknown_call", "미확정 도구 호출 시도");
        if (sha256(canonicalJson(c.arguments)) !== sha256(canonicalJson(CONTRACT_ARGS[i])))
            throw new ShadcnReadSemanticsError("contract_mismatch", "arguments canonical hash가 contract와 불일치");
    }
}
/**
    ...
    const walk = (dir) => {
        ...
        for (const name of names) {
            if (entries.length >= MAX_FS_ENTRIES)
                throw new ShadcnReadSemanticsError("fs_too_many", `serviceCwd 항목 ${MAX_FS_ENTRIES} 초과`);
            ...
            if (lst.isFile()) {
                if (lst.size > MAX_FILE_BYTES)
                    throw new ShadcnReadSemanticsError("fs_file_too_large", `파일이 ${MAX_FILE_BYTES} byte 초과`);
                let fd;
                try {
                ...
                try {
                    ...
                    if (fst.size > MAX_FILE_BYTES)
                        throw new ShadcnReadSemanticsError("fs_file_too_large", `파일이 ${MAX_FILE_BYTES} byte 초과`);
                    totalRead += fst.size;
                    if (totalRead > MAX_TOTAL_READ_BYTES)
                        throw new ShadcnReadSemanticsError("fs_read_too_large", `전체 read가 ${MAX_TOTAL_READ_BYTES} byte 초과`);
...
function analyzeResult(result) {
    if (!isPlainObject(result))
        throw new ShadcnReadSemanticsError("bad_result", "CallToolResult가 객체 아님");
    const content = result.content;
    if (!Array.isArray(content))
        throw new ShadcnReadSemanticsError("bad_result", "content가 배열 아님");
    if (content.length === 0)
        throw new ShadcnReadSemanticsError("empty_result", "content가 비어 있음");
    if (result.isError === true)
        throw new ShadcnReadSemanticsError("tool_is_error", "CallToolResult.isError=true");
    if (result.isError !== undefined && typeof result.isError !== "boolean")
        throw new ShadcnReadSemanticsError("bad_result", "isError가 boolean 아님");
    let textChars = 0;
    const types = new Set();
    for (const block of content) {
        if (!isPlainObject(block) || typeof block.type !== "string")
            throw new ShadcnReadSemanticsError("bad_result", "content block 계약 위반");
        types.add(block.type);
        if (block.type === "text") {
            if (typeof block.text !== "string")
                throw new ShadcnReadSemanticsError("bad_result", "text block에 text 문자열 없음");
            textChars += block.text.length;
        }
    }
    const structuredContentPresent = result.structuredContent !== undefined;
    if (structuredContentPresent && !isPlainObject(result.structuredContent))
        throw new ShadcnReadSemanticsError("bad_result", "structuredContent가 객체 아님");
...
export async function runShadcnReadSemanticsProbe(opts) {
    ...
    const norm = (code, message) => new ShadcnReadSemanticsError(code, scrub(message));
    // 시작 전: 고정 호출 계획 self-check(변조/소스 divergence 감지).
    try {
        assertCallPlanContract();
    }
    catch (e) {
        throw e instanceof ShadcnReadSemanticsError ? new ShadcnReadSemanticsError(e.code, scrub(e.message)) : norm("contract", e.message);
    }
    ...
    try {
        ...
        if (baseline.hasSymlink)
            throw norm("baseline_symlink", "serviceCwd baseline에 symlink 존재 — spawn 전 차단");
        baselineHash = baseline.hash;
    }
    catch (e) {
        throw e instanceof ShadcnReadSemanticsError ? new ShadcnReadSemanticsError(e.code, scrub(e.message)) : norm("fs_walk_error", e.message);
    }
    void baselineHash;
    ...
    try {
        ...
        if (!entry || !("command" in entry))
            throw norm("config_server", "shadcn stdio 서버 엔트리 없음");
        command = entry.command;
        args = entry.args;
        if (command !== "npx" || JSON.stringify(args) !== JSON.stringify(["--yes", SHADCN_PACKAGE, "mcp"]))
            throw norm("config_command", "실행 명령이 정확히 npx --yes shadcn@4.13.1 mcp 아님");
        configHash = written.configHash;
    }
    catch (e) {
        if (e instanceof ShadcnReadSemanticsError)
            throw new ShadcnReadSemanticsError(e.code, scrub(e.message));
    ...
    try {
        collected = await new Promise((resolveP, reject) => {
            ...
            const fail = (code, msg) => settle({ kind: "err", err: norm(code, msg) });
            ...
            const sendNextCall = () => {
                ...
                try {
                    ...
                    return fail(e.code ?? "fs_walk_error", e.message);
                }
            ...
            const handleMessage = (msg, lineBytes) => {
                if (msg.jsonrpc !== "2.0")
                    throw new ShadcnReadSemanticsError("jsonrpc_version", "jsonrpc가 '2.0' 아님");
                if (!("id" in msg) || msg.id === undefined)
                    return; // 서버 notification 무시
                if (msg.id !== pendingId)
                    throw new ShadcnReadSemanticsError("jsonrpc_id_mismatch", "응답 id 불일치");
                if (phase === "init") {
                    if (msg.error !== undefined)
                        throw new ShadcnReadSemanticsError("init_error", "init error 응답");
                    const r = msg.result;
                    if (!isPlainObject(r))
                        throw new ShadcnReadSemanticsError("init_error", "init result 누락");
                    const pv = r.protocolVersion;
                    if (typeof pv !== "string" || !ALLOWED_PROTOCOL_VERSIONS.includes(pv))
                        throw new ShadcnReadSemanticsError("protocol_version", "protocolVersion 미허용");
                    if (!isPlainObject(r.capabilities) || !isPlainObject(r.capabilities.tools))
                        throw new ShadcnReadSemanticsError("capabilities", "capabilities.tools plain object 아님");
                    const si = r.serverInfo;
                    if (!isPlainObject(si) || typeof si.name !== "string" || !si.name || typeof si.version !== "string" || !si.version)
                        throw new ShadcnReadSemanticsError("server_info", "serverInfo name/version 누락");
                    protocolVersion = pv;
                ...
                if (phase === "tools") {
                    if (msg.error !== undefined)
                        throw new ShadcnReadSemanticsError("tools_error", "tools/list error 응답");
                    const r = msg.result;
                    if (!isPlainObject(r) || !Array.isArray(r.tools))
                        throw new ShadcnReadSemanticsError("tools_error", "tools/list result.tools 배열 아님");
                    for (const raw of r.tools) {
                        if (!isPlainObject(raw) || typeof raw.name !== "string" || !raw.name)
                            throw new ShadcnReadSemanticsError("tools_error", "tool.name 누락");
                        const full = `mcp__${SHADCN_SERVER}__${raw.name}`;
                        if (names.has(full))
                            throw new ShadcnReadSemanticsError("duplicate_tool", "중복 도구");
                        names.add(full);
                        if (names.size > MAX_TOOLS)
                            throw new ShadcnReadSemanticsError("too_many_tools", "도구 수 초과");
                    }
                    const nc = r.nextCursor;
                    if (nc !== undefined && nc !== null) {
                        if (typeof nc !== "string" || !nc)
                            throw new ShadcnReadSemanticsError("tools_error", "nextCursor 유효하지 않음");
                        if (cursorsSeen.has(nc))
                            throw new ShadcnReadSemanticsError("repeat_cursor", "반복 cursor");
                        cursorsSeen.add(nc);
                        if (toolsListPagesSent >= MAX_PAGES)
                            throw new ShadcnReadSemanticsError("too_many_pages", "페이지 초과");
                        sendToolsList(nc);
                        return;
                    }
                    if (JSON.stringify([...names].sort()) !== JSON.stringify(EXPECTED_NS_TOOLS))
                        throw new ShadcnReadSemanticsError("tool_name_mismatch", "도구 집합 불일치");
                    phase = "calls";
                    sendNextCall();
                    return;
                }
                ...
                if (perCallTimer)
                    clearTimeout(perCallTimer);
                if (msg.error !== undefined)
                    throw new ShadcnReadSemanticsError("tool_call_error", "tools/call error 응답");
                const analysis = analyzeResult(msg.result);
                const elapsedMs = clock() - callStartedAt;
                const afterHash = fsSnapshot(serviceCwd).hash;
                if (afterHash !== curBeforeHash)
                    throw new ShadcnReadSemanticsError("filesystem_changed", `호출이 serviceCwd를 변경함 @ ${CALL_PLAN[callIdx].name}`);
            ...
            const onLine = (line) => {
                ...
                if (lineBytes > SINGLE_RESPONSE_CAP)
                    throw new ShadcnReadSemanticsError("response_too_large", `단일 응답 ${SINGLE_RESPONSE_CAP} byte 초과`);
                let msg;
                try {
                    msg = JSON.parse(t);
                }
                ...
                if (!isPlainObject(msg))
                    throw new ShadcnReadSemanticsError("malformed_line", "JSON-RPC 메시지가 객체 아님");
            ...
            child.stdout.on("data", (d) => {
                ...
                if (stdoutBytes > STDOUT_CAP)
                    return fail("stdout_too_large", `stdout ${STDOUT_CAP} byte 초과`);
            ...
            child.stderr.on("data", (d) => {
                ...
                if (stderrBytes > STDERR_CAP)
                    return fail("stderr_too_large", `stderr ${STDERR_CAP} byte 초과`);
                stderr += d.toString();
            });
            ...
            child.on("close", (code) => {
                ...
                if (code !== 0)
                    reject(norm("nonzero_exit", `MCP 서버 비정상 종료 (code ${code}): ${stderr.trim() || "(stderr 없음)"}`));
                else
                    reject(norm(phase === "init" ? "no_init" : phase === "tools" ? "no_tools" : "calls_incomplete", `완료 전 종료 (code ${code})`));
            });
    ...
    try {
        ...
        if (existsSync(childHome))
            cleanupErr = norm("cleanup_failed", "임시 HOME/cache가 정리되지 않음(잔존)");
    }
    catch (e) {
        cleanupErr = norm("cleanup_failed", `임시 HOME/cache 정리 실패: ${e.message}`);
    }
    if (cleanupErr) {
        if (probeErr)
            cleanupErr = norm("cleanup_failed", `probe 실패(${probeErr.code ?? "unknown"}) 후 임시 HOME 정리도 실패`);
        throw cleanupErr;
    }
    if (probeErr)
        throw probeErr instanceof ShadcnReadSemanticsError ? probeErr : norm("probe", probeErr.message);
    ...
    try {
        ...
        throw norm("persist", `read-semantics snapshot 기록 실패 (기존 파일/symlink?): ${e.message}`);
    }
    const operationSummary = { initialize: 1, initialized: 1, toolsListPages: data.toolsListPages, toolCalls: data.calledTools.length, calledTools: data.calledTools, forbiddenToolCalls: 0 };
