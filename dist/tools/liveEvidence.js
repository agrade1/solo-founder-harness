import { chmodSync, closeSync, fchmodSync, fstatSync, fsyncSync, linkSync, lstatSync, mkdirSync, openSync, readSync, statSync, unlinkSync, writeSync, } from "node:fs";
...
export const LIVE_EVIDENCE_CONTRACTS = Object.keys(CONTRACT_SPECS).sort();
 ...
 */
export function isValidEvidenceTimestamp(ts) {
    if (typeof ts !== "string")
        return false;
...
function forbiddenFragment(key) {
...
function scanForbiddenKeys(value, path, depth, errors) {
    ...
    for (const key of Object.getOwnPropertyNames(value)) {
        ...
        if (frag)
            errors.push(`${path}.${key}: 금지 필드 이름(조각 '${frag}')`);
        scanForbiddenKeys(value[key], `${path}.${key}`, depth + 1, errors);
    }
}
/**
 ...
 */
export function validateLiveEvidence(value) {
    const errors = [];
    if (!isPlainObject(value))
        return ["evidence: plain object 여야 함"];
    ...
    for (const k of unknown)
        errors.push(`evidence.${k}: unknown field`);
    for (const k of missing)
        errors.push(`evidence.${k}: 필수 필드 누락`);
    // 3) 고정 필드.
    if (value.version !== LIVE_EVIDENCE_VERSION)
        errors.push(`evidence.version: '${LIVE_EVIDENCE_VERSION}' 여야 함`);
    if (value.status !== LIVE_EVIDENCE_STATUS)
        errors.push(`evidence.status: '${LIVE_EVIDENCE_STATUS}' 여야 함(성공 전용)`);
    ...
    for (const k of mKeys)
        if (!specKeys.includes(k))
            errors.push(`evidence.metrics.${k}: unknown metric`);
    for (const k of specKeys)
        if (!mKeys.includes(k))
            errors.push(`evidence.metrics.${k}: 필수 metric 누락`);
    for (const k of specKeys) {
        if (!mKeys.includes(k))
            continue;
        const v = metrics[k];
        if (spec[k] === "boolean") {
            if (typeof v !== "boolean")
                errors.push(`evidence.metrics.${k}: boolean 이어야 함`);
        }
 ...
 */
export function buildLiveEvidence(opts) {
    const candidate = {
...
export function serializeLiveEvidence(evidence) {
    const metrics = {};
    for (const k of Object.keys(evidence.metrics).sort())
        metrics[k] = evidence.metrics[k];
    const ordered = {
 ...
 */
export function findSensitiveResidue(text, secretValues = []) {
    for (const v of secretValues) {
        if (v && text.includes(v))
            return "secret_value";
    }
    if (redactSecrets(text, secretValues) !== text)
        return "redactable_residue";
    if (CRED_SHAPE_RE.test(text))
        return "credential_shape";
    if (UNEXPECTED_CHAR_RE.test(text))
 ...
 */
export function resolveEvidenceDir(opts) {
    const override = opts.overrideDir;
    if (typeof override === "string" && override.trim().length > 0) {
        if (!isAbsolute(override))
            throw new LiveEvidenceError("evidence_dir_relative", "evidence 디렉터리 override는 절대경로여야 함");
...
function ensureEvidenceDir(dir) {
    ...
    if (st) {
        if (st.isSymbolicLink())
            throw new LiveEvidenceError("evidence_dir_symlink", "evidence 디렉터리가 symlink — 거부");
        if (!st.isDirectory())
            throw new LiveEvidenceError("evidence_dir_not_directory", "evidence 디렉터리 위치가 디렉터리가 아님 — 거부");
    }
    ...
    for (let i = 0; i < MAX_ANCESTOR_SYMLINK_CHECKS; i++) {
        ...
        if (ancestor?.isSymbolicLink())
            throw new LiveEvidenceError("evidence_dir_symlink", "evidence 상위 경로에 symlink — 거부");
        if (ancestor && !ancestor.isDirectory())
            throw new LiveEvidenceError("evidence_dir_not_directory", "evidence 상위 경로가 디렉터리가 아님 — 거부");
    ...
    if ((statSync(target).mode & 0o777) !== 0o700)
        throw new LiveEvidenceError("evidence_dir_mode", "evidence 디렉터리 권한이 0700 아님");
 ...
 */
function assertDirIdentity(dir, identity) {
...
function unlinkIfSameObject(path, identity, label, problems, inject) {
    let st;
    try {
        st = lstatSync(path);
    }
    ...
    try {
        ...
        const code = e.code;
        if (code !== "ENOENT")
            problems.push(`${label}_unlink_${code ?? "unknown"}`);
    }
}
...
function tempNameFor(finalName) {
 ...
 */
export function writeLiveEvidence(opts) {
    ...
    if (!NONCE_RE.test(nonce))
        throw new LiveEvidenceError("invalid_nonce", "nonce 형식 위반");
    ...
    const cleanupTemp = () => unlinkIfSameObject(tempPath, tempIdentity, "temp", cleanupProblems, opts.testHooks?.unlinkTemp);
    let published = false;
    try {
        ...
        try {
            ...
            if (post.size !== buf.length)
                throw new LiveEvidenceError("evidence_size_mismatch", "evidence 임시 파일 크기 불일치");
            if ((post.mode & 0o777) !== 0o600)
                throw new LiveEvidenceError("evidence_mode", "evidence 임시 파일 권한이 0600 아님");
        }
        finally {
            try {
                closeSync(fd);
        ...
        if (closeFailed)
            throw new LiveEvidenceError("evidence_close", "evidence 임시 파일 close 실패");
        ...
        try {
            ...
            const code = e.code;
            if (code === "EEXIST")
                throw new LiveEvidenceError("evidence_exists", "동일 evidence 파일명이 이미 존재 — 덮어쓰지 않음");
        ...
        if ((fin.mode & 0o777) !== 0o600)
            throw new LiveEvidenceError("evidence_mode", "evidence 파일 권한이 0600 아님");
    }
    catch (e) {
