...
export const TOOL_USE_ID_HOOKS = ["PreToolUse", "PostToolUse", "PostToolUseFailure", "PermissionDenied"];
...
function redactAndCap(v, secretValues, maxBytes) {
    const s = str(v);
    if (s === null)
        return null;
...
function sanitizeInput(raw, secretValues) {
 ...
 */
export function validatePayload(hookKind, payload) {
    ...
    if (hookKind === "PermissionRequest") {
        if (!str(payload.tool_name))
            throw new HookPayloadError("missing_tool_name", "tool_name 필수");
    ...
    if (TOOL_USE_ID_HOOKS.includes(hookKind)) {
        if (!str(payload.tool_name))
            throw new HookPayloadError("missing_tool_name", "tool_name 필수(tool hook)");
        if (!str(payload.tool_use_id))
            throw new HookPayloadError("missing_tool_use_id", "tool_use_id 필수(tool hook)");
    }
}
/**
 ...
 */
export function normalizeHook(hookKind, deny, payload, config, now) {
    ...
    const server = toolName && Object.hasOwn(config.toolMap, toolName) && typeof config.toolMap[toolName] === "string"
 ...
 */
export function toRunEvent(rec) {
    switch (rec.event) {
        case "tool_requested":
