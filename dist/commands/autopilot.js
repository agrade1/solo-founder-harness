...
async function runAutopilotUnderLease(opts, kernel, ctx) {
    ...
    const offlinePlanDir = backend === LIVE_PLAN_BACKEND ? null : opts.planDir;
    if (offlinePlanDir === undefined) {
        return { blocked: "run_unavailable", iterations: 0, tasks, stoppedBecause: "plan_dir_required" };
    }
    const planFor = (taskId) => offlinePlanDir === null ? LIVE_PLACEHOLDER : readPlanDocument(offlinePlanDir, taskId);
 ...
 */
function reviewRoundtripGate(kernel, taskId, currentTurnId) {
    const spec = kernel.getManifest().reviewRoundtrip;
    if (spec === undefined || spec.verify !== taskId)
        return null;
...
export async function runAutopilotCommand(opts) {
    ...
    const backend = opts.workerBackend ?? OFFLINE_PLAN_BACKEND;
    if (!AUTOPILOT_WORKER_BACKENDS.includes(backend)) {
        process.stdout.write(`[autopilot] 거부: --worker-backend는 ${AUTOPILOT_WORKER_BACKENDS.join(" | ")} 중 하나여야 한다(받은 값: ${backend})\n`);
        process.exitCode = 2;
        return;
    }
    /**
