...
const sentinel = (field) => `${SENTINEL_PREFIX}${field}`;
 ...
 */
function draftExecutable(flagValue, field) {
 ...
 */
function draftAuthorities(node) {
    if (node.operations.length > 0 && node.operations.length !== node.provides.length) {
        throw new OrchestrationError("draft_authority_underivable", `${node.taskId}: operations ${node.operations.length}건과 provides ${node.provides.length}건의 개수가 달라 ` +
    ...
    return node.provides.map((path, i) => ({
        authorityId: node.operations.length > 0
            ? node.operations[i]
 ...
 */
export function buildApprovalDraft(opts) {
    ...
    for (const node of document.tasks) {
        ...
        for (const a of authorities)
            authorityMap.push({ taskId: node.taskId, authorityId: a.authorityId, path: a.path });
        if (authorities.length > 0 && node.operations.length === 0)
            tasksWithoutOperations.push(node.taskId);
    }
    const draft = {
        milestoneId,
    ...
    if (executable) {
        throw new OrchestrationError("draft_would_be_executable", "산출한 초안이 승인 검증기를 그대로 통과한다 — 이 명령은 실행 가능한 승인을 발행하지 않는다(버그다)");
    }
 ...
 */
export function sentinelPaths(value, at = "") {
    if (typeof value === "string")
        return value.startsWith(SENTINEL_PREFIX) ? [at] : [];
    if (typeof value === "number")
        return value === SENTINEL_NUMBER ? [at] : [];
    if (Array.isArray(value))
        return value.flatMap((v, i) => sentinelPaths(v, `${at}[${i}]`));
    if (value !== null && typeof value === "object") {
        return Object.entries(value).flatMap(([k, v]) => sentinelPaths(v, at === "" ? k : `${at}.${k}`));
    }
    return [];
}
/** 출력 파일 이름은 **초안임을 말해야 한다**(헌법 ④) — 이름이 거짓말하는 파일을 만들지 않는다. */
function assertDraftFileName(path) {
    if (!basename(path).toLowerCase().includes(DRAFT_NAME_MARKER)) {
        throw new OrchestrationError("draft_output_name_not_draft", `출력 파일 이름에 "${DRAFT_NAME_MARKER}"가 없다: ${basename(path)} — 초안 파일은 이름으로 초안이라고 말해야 한다`);
    }
    return path;
}
/** `harness draft-approval` 명령 본체. 거부는 다른 진입점과 같은 exit 2다. */
export function runDraftApprovalCommand(opts) {
    let built;
    let outPath;
    try {
        ...
        try {
            ...
            if (e?.code !== "EEXIST")
                throw e;
            throw new OrchestrationError("draft_output_exists", `출력 파일이 이미 있다: ${outPath} — 채우던 초안을 덮어쓰지 않는다. 다른 --out을 쓰거나 그 파일을 직접 치워라`);
        }
    }
    catch (err) {
    ...
    w(`[draft-approval] 초안을 썼다: ${outPath} · task ${Object.keys(built.draft.ownershipByTask).length}건 · ` +
    ...
    for (const p of built.ownedPaths)
        w(`  - ${p}\n`);
    w("[draft-approval] 승인한 write 권위(authorityId → path) — 짝이 네 의도와 맞는지 확인해라:\n");
    for (const a of built.authorityMap)
        w(`  - ${a.taskId}: ${a.authorityId} → ${a.path}\n`);
    if (built.tasksWithoutOperations.length > 0) {
        ...
        for (const t of built.tasksWithoutOperations)
            w(`  - ${t}\n`);
    }
 ...
 */
export function runValidateApprovalCommand(opts) {
    ...
    if (rejected !== null)
        process.stdout.write(`[validate-approval] 불통과: ${rejected}\n`);
    else
        process.stdout.write("[validate-approval] 불통과: 승인 검증기는 통과했으나 채우지 않은 자리가 남아 있다\n");
