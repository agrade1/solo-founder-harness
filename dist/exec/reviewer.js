...
export async function reviewDiff(inp) {
    ...
    try {
        ...
        }, MAX_REVIEW_EVENTS, (code, message) => new ReviewGateError(code, message));
    }
    finally {
...
function sealSubject(subject) {
    ...
    if (!s || typeof s !== "object")
        bad("객체가 아니다");
    const { revision, hash } = s;
    if (typeof revision !== "string" || revision.length === 0)
        bad("revision이 비어 있다");
    if (revision !== revision.trim())
        bad("revision에 앞뒤 여백이 있다(정규형이어야 한다)");
    if (/[\r\n]/.test(revision))
        bad("revision은 한 줄이어야 한다");
    if (revision.length > MAX_SUBJECT_REVISION_CHARS)
        bad(`revision은 ${MAX_SUBJECT_REVISION_CHARS}자 이하여야 한다`);
    if (typeof hash !== "string" || !SUBJECT_HASH_RE.test(hash))
        bad("hash는 소문자 16진 7~64자여야 한다");
    return Object.freeze({ revision: revision, hash: hash });
}
/**
