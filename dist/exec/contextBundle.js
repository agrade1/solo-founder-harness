 ...
 */
export function buildContextBundle(state, taskId) {
    ...
    lines.push(`- resourceClasses: ${task.resourceClasses.length > 0 ? [...task.resourceClasses].sort(byId).join(", ") : "(none — 병렬 안전)"}`);
 ...
 */
export function computeSnapshotDigest(state) {
