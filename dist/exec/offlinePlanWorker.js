...
export const OFFLINE_WORKER_INPUT_KEYS = ["backend", "planJson", "binding"];
export const WORKER_BINDING_KEYS = ["runId", "taskId", "attemptId", "turnId"];
/**
 ...
 */
function adoptPlanBytes(raw) {
    let byteLength;
    let byteOffset;
    let buffer;
    try {
        if (taTag.call(raw) !== "Uint8Array")
            throw new Error();
...
function adoptPlan(rawInput) {
    ...
    if (input.backend !== OFFLINE_PLAN_BACKEND) {
        ...
        throw new OrchestrationError("worker_backend_unsupported", `M5c의 backend는 "${OFFLINE_PLAN_BACKEND}" 하나뿐이다(live claude/codex 추론은 없다)`);
    }
...
async function* planEvents(plan) {
    let seq = 0;
    yield Object.freeze({ kind: "started", seq: seq++ });
    // **최소 1건의 인정되는 progress** — 이 backend는 최종 결과만 있는 스트림을 만들 수 없다.
    yield Object.freeze({ kind: "progress", seq: seq++, step: "plan_adopted" });
    for (let i = 0; i < plan.operations.length; i++) {
        // 라벨은 고정 형식이다 — 경로·본문·argv 같은 계획 내용을 진행 신호에 싣지 않는다.
        yield Object.freeze({ kind: "progress", seq: seq++, step: `operation_${i + 1}` });
    }
    yield Object.freeze({
        kind: "terminal",
        seq: seq++,
 ...
 * 쓰지 않고 아무것도 띄우지 않는다.
 */
export function startOfflinePlanTurn(rawInput) {
