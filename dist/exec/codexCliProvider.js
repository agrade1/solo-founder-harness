 ...
 */
export function compileCodexArgs(spec, resumeSessionId) {
    ...
    if (o.ephemeral && !resumeSessionId)
        args.push("--ephemeral");
...
export class CodexCliProvider {
    ...
    async start(spec, initialPrompt) {
        ...
        const state = { spec, queue: new AsyncEventQueue(), child: null, parser: null, codexSessionId: "", ephemeral: o.ephemeral };
    ...
    async send(handle, message) {
        ...
        if (!state)
            fail("codex_unknown_session", `없는 세션 ${handle.sessionId}`);
        if (state.ephemeral) {
        ...
        state.queue = new AsyncEventQueue(); // 새 invocation = 새 스트림
        ...
        child.stdout?.on("data", (chunk) => {
            for (const e of parser.push(chunk)) {
                if (e.kind === "init" && e.sessionId)
                    state.codexSessionId = e.sessionId;
                queue.push(e);
            }
        });
