 ...
 * **오직 이 값만** 실행한다.
 */
const PRODUCTION_SPAWN = nodeSpawn;
 ...
 */
export function verifyCodexExecutable(approved, pinned) {
 ...
 */
export const CODEX_CREDENTIAL_FILES = ["auth.json"];
/**
 ...
 */
export const CODEX_RUNTIME_DIRS = ["log", "tmp", "cache", "shell_snapshots", "sessions"];
/**
 ...
 * **관측된 이름만** 더한다.
 */
export const CODEX_RUNTIME_FILES = ["installation_id", "models_cache.json", ".sandbox_migration"];
 ...
 */
export const CODEX_CODE_LOAD_DIRS = Object.freeze({
...
export function verifyCodexHome(path, expect = {}) {
    ...
    if (approved) {
        for (const name of CODEX_CREDENTIAL_FILES)
            verifyCredentialFile(join(p, name));
        for (const name of CODEX_RUNTIME_DIRS)
            verifyRuntimeDir(join(p, name));
    }
    return { path: p, id };
}
 ...
 */
function verifyCredentialFile(file) {
    let st;
    try {
        st = lstatSync(file);
    }
...
export function attestReadOnlyCodexProvider(provider, expected) {
    ...
    for (const m of ATTESTED_METHODS) {
        const fn = provider[m]; // ← 이 property를 읽는 유일한 지점
        if (fn !== proto[m])
