 ...
 */
export const CODE_REVIEW_LENSES = ["code", "security", "test"];
 ...
 */
function inRoleFamily(roleId, family) {
...
function assertParticipantShape(p, what) {
...
function assertRoundtrip(spec) {
    ...
    for (const r of reviewers)
        assertParticipantShape(r, "reviewer");
    ...
    for (const r of reviewers) {
        if (r.provider !== "codex")
            fail("reviewer_provider", "review는 fresh Codex여야 한다");
        if (r.sandbox !== "read-only")
 ...
 */
export function assertCodeReviewRoundtrip(rt) {
