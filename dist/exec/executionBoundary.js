 ...
 */
export function verifyApprovedExecutable(approved, what, codes, pinned) {
    ...
    try {
        ...
        if (st.size > MAX_EXECUTABLE_BYTES)
            throw new OrchestrationError(codes.invalid, `${what}가 상한보다 크다`);
...
function digestOfFd(fd, what, codes) {
...
async function readCheckoutHead(gate, root, what) {
    ...
    if (!sameIdentity(identityOf(topReal, `${what}의 checkout 루트`), root.id)) {
        throw new OrchestrationError("boundary_not_checkout_root", `${what}는 checkout 루트 자신이어야 한다(주어진 경로: ${root.path}, 실제 루트: ${topReal} — 디렉터리 신원 불일치)`);
    }
