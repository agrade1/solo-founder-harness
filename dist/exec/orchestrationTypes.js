 ...
 */
export const PROCESS_LAUNCHING_KINDS = ["run_process", "git_worktree"];
/** 이 kind가 자식 프로세스 그룹을 여는가. `write_file`은 순수 파일 시스템 연산이라 자손이 없다. */
export function opensProcess(kind) {
    return PROCESS_LAUNCHING_KINDS.includes(kind);
}
/**
 ...
 */
export const CONTROLLER_ACTIONS = ["validate-plan", "run-tests"];
