...
import { attestReadOnlyCodexProvider, verifyCodexExecutable } from "./codexCliProvider.js";
...
export class StableController {
    ...
    constructor(opts) {
        ...
        const gitBin = atTrusted(() => verifyApprovedExecutable(manifest.executionAuthority.git, "승인된 git 실행 파일", CONTROLLER_GIT_CODES));
    ...
    async advanceOnce() {
        ...
        try {
            ...
            const outcomes = atKernel(() => this.#sealed.kernel.commitPreflightBatch({
                // 결정은 **이 batch를 고른 그 revision**에 대한 것이다 — 낡으면 kernel이 거부한다.
                baseRevision: plan.revision,
                actionId: this.#actionId("preflight"),
                decisions: plannedIds.map((taskId) => ({ taskId, outcome: "prepared", attemptId: this.#actionId("attempt") })),
            })).outcomes;
    ...
    async #runTask(taskId) {
        ...
        try {
            ...
            for (const raw of atKernel(() => kernel.listPendingInbox(taskId))) {
                ...
                atKernel(() => kernel.beginDeliveryAttempt({
                    taskId,
            ...
            atKernel(() => kernel.recordTerminal({
                taskId,
...
function captureKernel(kernel) {
    ...
    if (!attested) {
        fail("controller_kernel_not_genuine", "M5b bridge는 실제로 생성된 OrchestrationKernel만 상태 권위로 받는다(구조적 객체·delegate·proxy·subclass·override 거부)");
    }
    const raw = attested.methods;
    const bound = {};
    for (const m of KERNEL_METHODS)
        bound[m] = raw[m].bind(kernel);
    return {
        raw: raw,
        captured: Object.freeze({ workspaceRoot: attested.workspaceRoot, ...bound }),
    };
}
/**
 ...
 */
function captureProvider(provider, expected) {
    ...
    if (!attested.authorityMatches) {
        fail("controller_provider_authority_mismatch", "provider가 controller가 기대한 실행 권위(codex/git 실행 파일 신원 · checkout · 승인 · 시각 권위)와 다르게 발급됐다");
    }
    const raw = attested.methods;
    const bound = {};
