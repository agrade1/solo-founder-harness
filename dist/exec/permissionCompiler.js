...
export function loadPermissionPolicy() {
    ...
    if (!existsSync(abs))
        throw new Error(`권한 정책 파일 없음: ${POLICY_PATH}`);
    return JSON.parse(readFileSync(abs, "utf8"));
}
/** bash 명령 접두어 → Claude Code Bash 규칙. */
function bashRule(cmd) {
    return `Bash(${cmd}:*)`;
}
/** 중복 제거하며 순서 유지. */
function uniq(xs) {
 ...
 */
export function compilePermissions(spec, policy = loadPermissionPolicy()) {
 ...
 */
export function materializeSettings(dir, compiled) {
