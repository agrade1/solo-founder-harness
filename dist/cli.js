...
program
    ...
    .option("--worker-backend <id>", `worker backend (${AUTOPILOT_WORKER_BACKENDS.join(" | ")}). 기본 ${AUTOPILOT_WORKER_BACKENDS[0]}. ${AUTOPILOT_WORKER_BACKENDS[1]}은 실제 모델 세션을 돌리며(구독 한도 소모) 승인 manifest의 executionAuthority.claude가 없으면 시작하지 않는다`)
    .option("--workspace <path>", "orchestration workspace 루트 (기본: 현재 디렉터리)")
