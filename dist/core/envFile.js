...
export function envGitState(root) {
    const inRepo = gitProbe(root, ["rev-parse", "--is-inside-work-tree"]);
    if (inRepo === "unknown")
        return "unknown";
    if (inRepo === "no")
        return "no_repo"; // repo 밖 — 이 도구 경로에서 커밋될 길이 없다
 ...
 */
export function ensureEnvFileReady(root = WORKSPACE_ROOT) {
    const path = envFilePath(root);
    const notices = [];
