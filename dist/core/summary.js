 ...
 */
function pipelineActions(project, root) {
    const read = readPipelineStateAt(pipelineStatePath(root));
    if (read.kind === "absent")
        return null; // 파이프라인 미사용 — 기존 문구 그대로
 ...
 */
function nextActions(state, project, pipelineOwns = false) {
    if (!state) {
...
export function buildSummary(project, today) {
    ...
    if (pipeRead.kind === "ok") {
        ...
        lines.push(`- 단계 체크포인트: ${st.current_index + 1}/${DEFAULT_PIPELINE.length} '${currentStage(st)?.id ?? "(완료)"}' · ${st.status}` +
