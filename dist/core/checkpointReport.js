...
function decisionToken(root, artifacts) {
...
function criticalRisks(root, artifacts) {
 ...
 */
export function buildCheckpointReport(root, i) {
    ...
    L.push("");
    L.push(`> 이 문서는 **읽기 편하라고 만든 요약**이고 승인 대상 자체가 아니다. 승인은 아래 원문 바이트에 걸린다.`);
    L.push(`> checkpoint id가 위와 다르면 낡은 보고서다 — \`harness pipeline status\`로 확인하라.`);
    L.push("");
    L.push("## 판정 한 줄");
    L.push("");
    if (decision)
        L.push(`- CEO 판정: **${decision}**`);
    else
        L.push("- CEO 판정: (이 단계에는 판정 문서가 없다)");
    ...
    L.push(`      \`harness pipeline reject ${i.stage} --checkpoint ${i.checkpointId} --project ${i.project} --note "<이유>"\``);
    L.push("");
    ...
    L.push("");
    if (i.seeds.length === 0)
        L.push("- (판단 요약이 없다 — 이 단계는 판단 문서를 내지 않았다)");
    for (const s of i.seeds)
        L.push(`- ${s.line}`);
    L.push("");
    L.push("## 걸린 위험 (Critical만)");
    L.push("");
    if (risks.length === 0)
        L.push("- (없음 — red_team이 Critical을 남기지 않았다)");
...
/** 보고서를 쓸 절대경로. */
export function checkpointReportPath(root) {
    return join(root, CHECKPOINT_REPORT_REL);
}
/** 파일 크기(없으면 0) — 호출자가 안내에 쓴다. */
export function fileSize(abs) {
    try {
