...
import { loadRunState } from "./runWorkflow.js";
...
const APPROVE_MESSAGE = "판단 문서 생성 완료. 서비스 레포에서 Claude Code 대화형 세션을 여시겠습니까? (이후 파일 수정은 Claude Code 승인 프롬프트에서 진행됩니다)";
...
const HANDOFF_PREFLIGHT_PROFILE = {
...
function deriveSecretRefs(env) {
    const refs = [];
    for (const [k, v] of Object.entries(env)) {
        if (v && v.length > 0 && SECRET_NAME_RE.test(k) && isValidSecretRef(k))
            refs.push(k);
    }
...
function defaultSpawnInteractive(bin, argv, opts) {
    const res = spawnSync(bin, argv, { cwd: opts.cwd, env: opts.env, stdio: "inherit" });
    if (res.error)
        throw res.error; // spawn 실패만 오류. 종료코드는 기록하지 않는다.
}
function defaultStdinApprove(message, preview) {
...
export async function runHandoff(opts) {
    ...
    const scrub = (s) => redactSecrets(s, redactValues);
    ...
    // 1b) [B-41/2단] 단계 체크포인트 게이트 — 파이프라인이 있으면 **완료 후**에만 handoff한다
    ...
    try {
        ...
        if (!cstat.isFile())
            throw new Error("일반 파일이 아닙니다 (디렉터리/특수 파일)");
        accessSync(collector, constants.R_OK); // 읽기 권한 없으면 throw
    }
    catch (e) {
        const msg = scrub(`collector 산출물을 사용할 수 없습니다 (부재/디렉터리/읽기 불가): ${collector} — 'npm run build' 후 다시 시도하세요. (${e.message})`);
        log(msg);
    ...
    try {
        ...
        const msg = scrub(`handoff 산출물 준비 실패 (기존 파일/symlink?): ${e.message}`);
        log(msg);
        return { action: "setup_failed", message: msg };
    }
