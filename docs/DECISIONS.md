...
## 2026-08-30 (B-50 — **예산이 기계와 사람의 경계다: 소진 후의 `검증`은 사람 차례**)

- **결정 — 되돌림 예산이 소진된 상태에서 `검증` 판정이 나오면 `failed_reason`을 새 코드
  `ceo_decision_verify`로 낸다**(다른 판정의 소진은 그대로 `gate_jump_budget_exhausted`).
  게이트 분기의 사유 ternary 한 case이고 status·resume·영수증 기계는 기존 `failed` 경로 그대로다.
  그 사유에서는 `task-prompt`·`plan-dag`가 거부하고(`ceoVerifyGateStatus` 단일 판정 함수),
  `run`·`pipeline next`는 "사람이 확인할 차례"라는 사유별 복구 안내를 낸다.
- **`agents/founder_ceo_agent.md` §14.0 표의 `검증` 행을 고쳤다**: "(앞 단계로 되돌림)" →
  "(되돌림 1회 후, 소진되면 사람 확인 대기로 중단)". 계약 문서이므로 여기 남긴다. §8-C(추가 검증)와
  §14.1 산문 형식·정본 예시는 **건드리지 않았다** — 새 의미론에서 `검증`은 "먼저 기계, 소진 후 사람"
  둘 다를 뜻하고, 그것이 §8-C 문면("리서치, 인터뷰, 랜딩페이지, 수동 MVP")과 그대로 일치한다.
- **기각 ① `검증 → research` 매핑 제거 + 게이트가 `on` 조회 전에 가로채기**(설계 초판의 중심 수).
  전제였던 "2차 research 유효 0/4"가 **영수증 대조에서 거짓으로 드러났다**: `claimrep`은 아이디어를
  죽인 결정적 사실(KATECH)이 2차 attempt에만 있었고, `sellercs`는 2차 근거 9건 중 8건, `nuga`는
  10건 중 10건이 새 출처였다. 되돌림은 제 일을 하고 있었으므로 실증된 가치를 버리는 안이다.
  문제는 되돌림이 아니라 **되돌림이 끝난 뒤**였고, 그래서 경계를 예산에 걸었다.
- **기각 ② 새 durable status(`awaiting_human_verify` 등)** — `resume`이 `status !== "failed"`를 던지고
  `summary.ts`의 switch가 `never` 소진 체크라 컴파일부터 깨진다. 이 레포의 `failed`+`resume_from`은
  이미 "오류"가 아니라 "중단 — 사람 개입 필요·재개 가능"이다(`user_rejected`·`ceo_decision_hold` 선례).
  뜻은 사유 코드가 나른다. **기각 ③ `completed` 변형** — fail open이고 B-40이 닫은 구멍의 재개방이다.
- **기각 ④ 기계 재조사용 신규 토큰('재조사' 등)** — 수요 실측 0. 필요가 실증되면 registry 매핑 +
  프롬프트 표 한 줄이면 되고 jump 기계(`on`)는 일반형이라 그대로 남는다.
- **`pipeline` 안내는 decider 문서가 승인 manifest에 없을 때만 낸다.** 있으면 사람의 `## Decision`
  수정이 `pipeline_artifact_drift`로 막혀 안내가 거짓이 된다 — 그 경로에는 stale 판정 재생 문제도
  얽혀 있어 이번에 참인 문장을 쓸 수 없다(대장 등재 · 이 슬라이스에서 닫지 않는다).
- **주장하지 않는 것**: 이 변경이 CEO의 `검증` 사용 정확도를 바꾸는지는 재측정 전까지 모른다.
  증명한 것은 **`검증`에 대한 하네스의 응답**이지 판단 품질이 아니다. 종결도 "각 invocation은 반드시
  끝난다"까지이고, 사람이 무한히 resume하거나 새 run을 무한히 낼 수 있다는 사실은 그대로다.
