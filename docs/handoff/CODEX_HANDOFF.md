...
- **Task 3A는 독립 승인됐다. M5c와 M5 전체는 완료·승인되지 않았다.** 리비전 입력 권위는
  `/private/tmp/m5c-task3a-revision5-codex-review-output.txt`(범위 `7d3f547..e88c1ca` ·
  판정 `REVISE — A=1, B=2, C=3`). 최종 독립 권위는
  `/private/tmp/m5c-task3a-revision6-codex-review-output.txt`(fresh Codex `gpt-5.6-sol` xhigh
  read-only · 세션 `019fb6d4-cad0-7232-bd2e-a33ca1390362` · 범위 `e88c1ca..e0043ef` ·
  판정 **`APPROVE — A=0, B=2, C=3`**). 리뷰어는 정적 검토만 했고 테스트·빌드는 실행하지 않았다.
...
- **검증 실측**: 구현 세션과 supervisor 재실행 모두 `npx tsc --noEmit` 0 error · focused 5파일(`typedExecution` · `orchestrationKernel` ·
...
- **열린 미래 게이트**: `B-F1`(managed launcher 첫 소비자·첫 spawn 전) · `B-16`(첫 real typed-write
  ...
  과대였다. source/facade 주석도 최종 handoff에서 정정했다. 여전히 진짜 grant + 승인 경로/내용이
  필요하다 · M5d handoff 전) · `C2`(draft-07 실검증 · M5d 계약 handoff 전). 열린 **A는 없다**.
...
## 이전 상태 (2026-07-31 — V3 **M5c task 3A 5차 리비전 · 독립 재리뷰 `REVISE A=5·B=2·C=3`의 A 5건 닫음 · M5c·M5 여전히 미완료** · 위 현행 절이 이 기록을 대체한다)
...
- worktree `/private/tmp/solo-founder-harness-m5c` · branch `work/m5c-autopilot` · 시작 HEAD `7d3f547` ·
  ...
  작성자 transcript·자기평가 **미상속** · 재개 아님 · subagent/병렬 writer 0). Ponytail **level `full`**.
  원격 push/PR/merge · 네트워크 · `gh` · MCP · 패키지 설치 · 의존성/lockfile 변경 · live provider 추론 ·
  secret · deploy · DB · production · live billing · **프로세스 spawn 0** ·
  amend/rebase/reset/merge/stash 0 · **테스트 완화·삭제 0**(교체 assertion 2건 전수는 WORKLOG).
- **M5c도 M5도 승인 대상이 아니고 self-approved도 아니다.** 입력 권위는
  `/private/tmp/m5c-task3a-revision4-codex-review-output.txt`(세션 `019fb685-3f2f-7512-aa13-9d12f3e47585` ·
  범위 `20530b0..7d3f547` · 판정 `REVISE — A=5, B=2, C=3`).
- **변경 파일(삭제 1 · 변경 6)**: **삭제** `src/exec/writeFileEffect.ts`(4차 판이 만든 파일 —
  직접 import 우회의 원인) · `orchestrationKernel.ts`(+테스트) · `orchestrationStore.ts` ·
  `typedExecution.ts` · `schemas/orchestration_run_state.schema.json` · `autopilotLifecycle.test.ts`.
  `orchestrationTypes.ts` · `approvalManifest.ts` · `typedPlan.ts` · `agentMessage.ts` ·
  `executionBoundary.ts` · `offlinePlanWorker.ts` · `stableController.ts`(+테스트 — **수정 0**, red
  수치만 측정) · managed process/controller · trusted Git · reviewer · CLI · legacy exec/mission ·
  store 발행 내부 · `scripts/*` · package/lock · tracked `dist` · `AGENTS.md`/`CLAUDE.md` **전부 무변경**.
  두 번째 scheduler·대안 controller·신규 런타임/dev 의존성 **0**.
- **계약 요지(리뷰어가 확인할 것)**
  - **A1(정산 권위)** bare `chargeTurnUsage`는 ⓐ 이 task가 claim을 들고 있으면 `turn_conflict`이고
    ⓑ **그 turnId를 다른 task가 claim했으면** 역시 `turn_conflict`다(커밋 안에서 `draft.tasks`를 본다) →
    남의 생산 turn을 0 토큰으로 선점할 수 없다. `dispatchTurnSettled(task)`는 `accounting`을 **보지 않고**
    `execution.turnId === 열린 claim` + `chargedPlanDigest !== null` +
    `chargedPlanDigest === dispatchPlanDigest` + 그 turn의 미확정 0만 본다 → 정산은 **정확히 이
    run/task/attempt/turn/계획의 진짜 과금**에서만 나온다. claim 없는 turn의 safety-only 회계는 유지된다
    (`B-12`).
  - **A2(발급자 격리)** `PermitRecord`·`GrantRecord`·`ProgressChannelRecord`가 **발급 kernel 인스턴스
    객체**를 들고 있고 `chargeDispatchTurnUsage`·`beginOperation`·`recordOperationReceipt`·
    `failOperation`·`recordProgress`가 `this`와 `===`로 대조한다(평범한 durable 문자열 ID는 발급자 신원이
    아니다). `LIVE_GRANTS`는 `WeakMap<issuer, Map<pendingKey, GrantRecord>>`이므로 durable ID가 같은 두
    workspace가 서로의 live grant를 소비할 수 없다. grant의 `markAttempted`는 **발급 인스턴스의 private
    전이**를 부르는 클로저다. **같은 workspace의 두 번째 인스턴스도 남**이며, 권위 이전은 durable 경로
    2종뿐이다(정확한 `(turn, 계획)` permit **커밋 없는** 재발급 / handle-free `reconcileUncertainOperation`).
  - **A3(효과 표면)** `src/exec/writeFileEffect.ts`를 **삭제**했다. 파일 시스템 집행기
    (`judgeWriteFile`/`judgeWriteTransaction`/`walkParents`/`confirmDirDurability`)는 `GENUINE_GRANTS`와
    같은 모듈의 **사설 함수**이고 export되지 않는다 → 위조 가능한 구조적 `DispatchAuthority`로 효과에
  ...
  - **A5(용량 예약)** 불변식 `operationReceipts.length + pendingOperations.length <=
    LIMITS.maxOperationReceipts`를 `beginOperation` 커밋과 **store load**가 함께 본다. 정확한 재발급은
    커밋 밖에서 반환되므로 두 번째 자리를 쓰지 않는다 → 영수증 상한 위에서 열려 어떤 경로로도 닫히지 않던
    미아 pending이 존재할 수 없다.
  - **기존 폐쇄 유지**: brand된 단조 진행 채널 · durable 단조 시계(`updatedAt` 포함) · durable
    `attemptedAt`/`outcome_unknown` · 정확 신원 재시작 정합화 · A4 발행 fail closed(`B-16`) · B1 정리
    우선 · B2 닫힌 action/capability(**spawn 0**) · C1 커밋 없는 정확 재발급 · 적대적 manifest 단일 입양.
  - **schema 서술 정정**: 재시작한 `running` kernel도 `attemptedAt: null`이면 새 permit으로 grant를
    재발급한다 — handle-free 경로가 **필수인 것**은 attempted·`cleaning`·전진 게이트 폐쇄일 때다.
- **검증 실측**: `npx tsc --noEmit` 0 error · 파일 단독 `typedExecution` **62/62** ·
  `orchestrationKernel` **103/103** · `autopilotLifecycle` **28/28** ·
  (+`executionBoundary`·`offlinePlanWorker`) 동시 합계 **223/223** · `m4a` **PASS=32** ·
  `m4b` **PASS=45** · `m4c` **PASS=80**(전부 FAIL=0 · exit 0) · `git diff --check` 출력 0 ·
  mutation **10종**(MUT-1~8 + MUT-6+7 · MUT-8+1) — 각 게이트 red 확인 후 원복
  (`git status --short` = `?? node_modules` 한 줄 · `MUT[1-8]` grep 0 · 223/223 재확인).
  **정직한 예외**: `MUT-8`(정산 권위를 run 전역으로 되돌림)은 **단독으로는 red가 없다** —
  claimant 거부가 부패 상태를 도달 불가로 만드는 defense-in-depth이며, `MUT-8+1`이 4차 동작을 재현한다.
- **알려진 red**: `stableController.test.ts` **3 pass / 55 fail** — 이번 변경 **전후 동일**이고 실패 55건
  **전부** `manifest_pre_m5c_unsupported`(M5c 이전 manifest). 다음 DAG task 범위이며 이 세션은 그 파일을
  **수정하지 않았다**(측정만).
- **미실행**: `npm test` · `test:exec` · `test:core` · 전체 acceptance · stress · live · 반복 3회 ·
  build/dist · M5d. 최종 전체 suite 1회는 M5 최종 handoff에 예약돼 있다.
- **열린 미래 게이트**: `B-F1`(managed launcher 첫 소비자·첫 spawn 전) · `B-16`(첫 real typed-write
  산출물 배선 전, 늦어도 M5c 통합) · `C-1`(발행 seam export — 위치만 kernel로 옮겼고 표면 4종 그대로 ·
...
- **다음 DAG task**: `StableController` 재작성 + (permit → **권위 과금** → grant → 효과 → 영수증) 배선 →
  managed process supervisor(+자손 정리) → trusted Git → `autopilot` CLI. **이 과거 절의 모델 지시는
  사용자 의도의 오기였으며 폐기됐다. 현재 Task 3A 뒤 Codex는 중단하고 이후는 사용자 별도 지시를 따른다.**
...
- worktree `/private/tmp/solo-founder-harness-m5c` · branch `work/m5c-autopilot` · 시작 HEAD `2956ffc` ·
  ...
  재개 아님 · subagent/병렬 writer 0). Ponytail **level `full`**. 원격 push/PR/merge · 네트워크 · MCP ·
  패키지 설치 · 의존성/lockfile 변경 · live provider 추론 · secret · **프로세스 spawn 0**.
  amend/rebase/reset/stash 0 · **테스트 완화·삭제 0**(교체 assertion 전수는 WORKLOG).
- **M5c는 여전히 승인 대상이 아니고 self-approved도 아니다.** 입력 권위는
  `/private/tmp/m5c-task3a-revision2-codex-review-output.txt`(세션 `019fb5fb-89ec-7e40-90a9-4a4e7e66d3c2`).
- **변경 파일(신규 0 · 변경 10)**: `orchestrationKernel.ts`(+테스트) · `typedExecution.ts`(+테스트) ·
  `approvalManifest.ts` · `orchestrationTypes.ts` · `orchestrationStore.ts` · 두 schema ·
  `autopilotLifecycle.test.ts`. `stableController.ts`(+테스트 — **열지 않았다**) · managed process ·
  trusted Git · reviewer · CLI · legacy exec/mission · store 발행 내부 · `executionBoundary.ts`(+테스트) ·
  `typedPlan.ts` · `offlinePlanWorker.ts` · `scripts/m4*-offline-acceptance.mjs` · package/lock ·
  tracked `dist` **전부 무변경**. 두 번째 scheduler·대안 controller·신규 런타임/dev 의존성 **0**.
- **계약 요지(리뷰어가 확인할 것)**
  - **A1** 순서가 계약이다: **permit(claim) → `chargeTurnUsage` → `beginOperation`(grant) → 효과**.
    효과 게이트가 `accounting.chargedTurnIds.includes(turnId)`를 요구한다(신규
    `budget_turn_unaccounted`) → 토큰 판정이 한 turn 뒤처질 수 없다. **과금은 claim을 닫지 않는다** —
    끝난 claim(과금 + 미확정 0)만 다음 turn의 permit 요청이 교체한다(지연 해제). `recordProgress`는
    소진된 no-progress/wall 창을 되살리지 못하고 attempt `processLeaseMarker` + worker
    `{kind:"progress",seq,step}` 닫힌 읽기를 요구한다. 전진 시각은 `state.updatedAt`에 대해 단조다.
  - **A2** durable pending 신원당 **live grant 1개**(새 발급이 이전 것 폐기). `consumeExecutionGrant`
    export **삭제** → `executeUnderGrant(grant, op, effect)`(진입 일회용 · 정상 반환 시에만 canonical
    결과를 굳혀 **opaque handle**). `recordOperationReceipt({outcome, actionId})`는 저장된 값을 적고
    호출자 필드를 채택하지 않는다. 미시도·실패는 신규 `failOperation({grant, marker: denied|failed})`.
    영수증·event에 attemptId·turnId·**planDigest**가 durable하다.
  - **A3** 영수증 정합화는 **safety-only**(`requireReconcilableTask` — `running|cleaning`, 만료·예산·
    wall·no-progress·drift 미검사, 신원 전수 확인). attempt를 떠나거나 리셋하는 전이
    (preflight/pause/resume/settle)는 pending이 있으면 전부 거부. 런타임·schema 불변식으로 pending을
    attempt/turn/planDigest에 묶고 그 밖의 state에서 금지.
  - **A4** **신규 발행은 `write_publish_unsupported`로 fail closed**(temp도 만들지 않는다).
    `process.chdir` 우회는 평가 후 채택하지 않았다(전역 상태·worker thread·자식 cwd 오염).
    → **`applyWriteFile`은 새 파일을 만들지 못한다**(대장 신규 `B-16`).
  - **B1** temp 소멸로 unlink/truncate durability·고아 plaintext가 성립하지 않는다. fd 반납 실패는
    `write_cleanup_unconfirmed`이고 1차 오류와 동시에 나면 **정리 미확인이 이긴다**(1차 코드는 메시지에).
  - **B2=`B-10`** `data: string[]` → action별 `{planPath}`(정규화 항등·surrogate·상한·writableRoots·
    ownership). 공개 `ProcessLaunchSpec` **삭제** → opaque `ProcessLaunchCapability`(실행 대상·argv·
    digest **비노출**). 권능 발급은 순수 판정 → spawn 없는 계획이 성공을 만들 수 없다. **spawn 0.**
  - **C1/C2/C3** 멱등 재발급(커밋 0) · manifest 단일 입양(accessor·proxy 거부) · schema 주장 정정
    (**draft-07 validator는 추가하지 않았다** — 지시대로).
- **검증 실측**: `npx tsc --noEmit` 0 error · 파일 단독 `typedExecution` **47/47** ·
  ...
  바이트 일치 · `if (false &&` 잔재 grep 0).
- **알려진 red**: `stableController.test.ts` — **이 세션은 실행하지도 열지도 않았다**(다음 DAG task 범위).
  직전 세션 실측은 3 pass / 55 fail이었고 이번 계약 변경으로 **더 늘었을 것으로 예상**한다(미측정).
- **미실행**: `npm test` · `test:exec` · `test:core` · 전체 acceptance · stress · live · 반복 3회 ·
  build/dist · M5d. 최종 전체 suite 1회는 M5 최종 handoff에 예약돼 있다.
- **다음 DAG task**: `StableController` 재작성 + (permit → 과금 → grant → 효과 → 영수증) 배선 →
  managed process supervisor(+자손 정리) → trusted Git → `autopilot` CLI. **이 과거 절의 모델 지시는
  사용자 의도의 오기였으며 폐기됐다. 현재 Task 3A 뒤 Codex는 중단하고 이후는 사용자 별도 지시를 따른다.**

## 이전 상태 (2026-07-30 — V3 **M5c task 3A 2차 리비전 · 그 시점 기록 · 위 절이 이를 정정한다**)
...
- worktree `/private/tmp/solo-founder-harness-m5c` · branch `work/m5c-autopilot` · 시작 HEAD `16cdc87` ·
  ...
  자기평가 **미상속** · 재개 아님 · subagent/병렬 writer 0). Ponytail **level `full`**.
  원격 push/PR/merge · 네트워크 · MCP · 패키지 설치 · 의존성/lockfile 변경 · live provider 추론 · secret ·
  **프로세스 spawn 0**. amend/rebase/reset/stash 0 · **테스트 완화·삭제 0**(변경 assertion 전수는 WORKLOG).
- **M5c는 여전히 승인 대상이 아니고 self-approved도 아니다.** 이 리비전이 닫은 것은 독립 재리뷰의
  Category A **4/4**와 B **2/2**(`B1` 인접 filesystem · `B2`=`B-10` `run_process` 코드 권위)다.
- **변경 파일(신규 0 · 변경 14)**: `orchestrationKernel.ts`(+테스트) · `typedExecution.ts`(+테스트) ·
  `approvalManifest.ts` · `orchestrationTypes.ts` · `orchestrationStore.ts` ·
  `schemas/milestone_approval_manifest.schema.json` · `schemas/orchestration_run_state.schema.json` ·
  `autopilotLifecycle.test.ts` · `executionBoundary.test.ts` · `scripts/m4{a,b,c}-offline-acceptance.mjs`.
  `stableController.ts`(+테스트) · managed process · trusted Git · reviewer · CLI · legacy exec/mission ·
  store 발행 내부 · `executionBoundary.ts` 런타임 · `typedPlan.ts` · `offlinePlanWorker.ts` · package/lock ·
  tracked `dist` **전부 무변경**. 두 번째 scheduler·대안 controller·신규 런타임 의존성 **0**.
- **계약 요지(리뷰어가 확인할 것)**
  - **A1** `issueOperationDispatchPermit()`은 이제 **커밋**이다 — `execution.dispatchTurnId` +
    `dispatchPlanDigest`를 claim한다. durable turn이 `null`인 동안 두 turn이 각각 permit을 받는 경로가
    사라졌다. `chargeTurnUsage()`가 claim을 닫고 `chargedTurnIds`에 기록한다(재개방 불가). permit 레코드가
    private `#state`를 직접 읽는다. 효과·명세 발급 직전마다 **토큰 등호 · attempt wall 등호 · no-progress
    등호**를 추가 확인한다.
  - **A2** `beginOperation()`(신규 공개 메서드 1개) → durable `pendingOperations` 커밋 + **일회용 grant**.
    `applyWriteFile`/`resolveProcessLaunchSpec`은 permit이 아니라 **grant**를 요구하고 정확히 한 번 소진한다.
    `recordOperationReceipt({grant, actionId, receipt})`가 정확히 한 번 소비하며 pending을 닫는다.
    성공 marker는 효과 게이트를 지난 grant에서만 나온다. 미확정 pending이 있으면 turn·완료·차단 전부 막힌다.
  - **A3** 기존 경로 교체는 **temp 생성 전에** `write_replace_unsupported`. `renameSync` 발행 **삭제**.
  - **A4** `already_applied`도 부모 fsync 성공 뒤에만.
  - **B1** close/unlink 실패 → `write_cleanup_unconfirmed`. 지울 수 없는 temp는 소유 fd로 `ftruncate(0)`.
  - **B-10** `executionAuthority.controllerEntrypoint`(digest 고정) 필수 · 승인 레코드는 닫힌 `action` +
    데이터 전용 `data`(`-` 시작 금지) · argv는 `[entrypoint, action, ...data]`로 파생.
- **검증 실측**: `npx tsc --noEmit` 0 error · `typedExecution` **46/46** · `offlinePlanWorker` **10/10** ·
  `autopilotLifecycle` **28/28** · `orchestrationKernel` **103/103** · `executionBoundary` **20/20**
  (합계 **207/207**) · `git diff --check` clean · mutation **8종** 전부 실패 확인 후 원복.
- **알려진 red**: `stableController.test.ts` **3 pass / 55 fail**(변경 전후 동일 · v1 manifest fixture +
  stale `startScheduledBatch()` — **다음 DAG task 범위**).
- **미실행**: `npm test` · `test:exec` · `test:core` · 전체 acceptance · M4 acceptance 3종 · stress · live ·
  반복 3회 · build/dist · M5d. 최종 전체 suite 1회는 M5 최종 handoff에 예약돼 있다.
- **다음 DAG task**: `StableController` 재작성 + grant/pending lifecycle 배선 → managed process supervisor
  (+자손 정리) → trusted Git → `autopilot` CLI.

## 이전 상태 (2026-07-30 — V3 **M5c task 3A 리비전 · 독립 리뷰 A 4건 + 인접 filesystem B 닫음** · 그 시점 기록)
...
## 이전 상태 (2026-07-30 — V3 **M5c green-recovery slice · v2 schema 정본화 + kernel/M4 검증면 green · M5c 여전히 미완료** · **그 시점 기록** — 현행은 맨 위 절이다)
...
## 이전 상태 (2026-07-30 — V3 **M5b stable controller · 7차 리비전 완료** · **독립 8차 리뷰 대기 · M5 미완료** · **그 시점 기록** — 현행은 맨 위 절이다)
...
- **M5b 7차 리비전** — 시작 HEAD `ff5e035` 위에 code/tests/dist/schema 커밋 + docs 커밋.
  ...
  `409dee2`. fresh Claude Opus 5 단일 세션(공유 dirty 체크아웃 → 단일 writer · subagent·병렬 writer 0).
  원격 push/PR/merge·네트워크·MCP·패키지 설치·live provider 추론·secret 0.
  `node_modules`(supervisor symlink) stage 0 · 세션 끝에 `unlink`.
- **⚠ 아래 "6차 리비전이 A 2건을 둘 다 닫았다" 서술은 부분적이었다.** 7차 독립 fresh Codex `gpt-5.6-sol`
  xhigh read-only 재리뷰(`409dee2..ff5e035`)가 **REVISE · A/P1=2 · B=7 · C=12**를 냈고 **6차 A1 PARTIAL ·
  6차 A3 PARTIAL**(초기 A2 CLOSED · 초기 A4 PARTIAL)으로 판정했다. 공통 뿌리는 **"검증을 트랜잭션 1회
  단위로 잡았다"** 이다 — *무엇이 권위인가*는 6차가 바로잡았지만 *언제 다시 보는가*가 넓었다.
  7차 리비전이 닫은 것:
  - **A1 — git 내용이 spawn마다 재검증되지 않았다.** 경계 진입에서 한 번 해싱한 뒤 `readCheckoutHead()`가
    `--show-toplevel`·`rev-parse HEAD` **두 자식 프로세스**를 각각 await했으므로, 첫 프로세스를 기다리는
    동안 owner-writable 승인 실행 파일을 **같은 inode에 제자리 덮어쓰면** 두 번째 프로세스가 **SHA-256이
    승인되지 않은 바이트**를 실행했다(그 payload는 임의 작업 후 기대 HEAD를 출력하고 원 바이트를 되돌릴 수
    있어 뒤 검사도 통과한다). `revalidateSync()`도 루프 **앞에서** 한 번 해싱하고 checkout마다 `spawnSync`
    했다. → ① `GitGate = () => string` 하나를 두고 **모든 git spawn이 자기 `runProcess`/`spawnSync`
    직전에** 같은 fd(`O_RDONLY|O_NOFOLLOW`)에서 정규·비symlink·일반 파일·실행 비트·타인 쓰기 없음·
    **pin된 dev+ino**·**승인 SHA-256**을 다시 판정한다 ② 게이트↔spawn 사이에 **`await`가 없다**
    (`runProcess`는 Promise executor 안에서 동기로 `spawn`한다) ③ `readCheckoutHead()`의 **두 호출 각각** ·
    `revalidateSync()`의 **controller/target 회차 각각**이 지나고 루프 앞 1회 검증은 제거했다
    ④ 남는 창은 **fd 해싱→exec syscall 몇 개**뿐이며 그 한계를 코드·문서에 적었다(`fexecve` 없음 — `C-5`
    종류) ⑤ 호출자 경로·PATH 조회·ambient env·대체 trust root·신규 의존성 0 · manifest-only 실행 권위와
    provider/controller attestation 계약 그대로.
  - **A2 — body 발행이 검증 이후 교체본을 link하고 복구 journal을 삭제할 수 있었다.** `publishOwnedBodies()`가
    **전수 preflight** 뒤 hook을 부르고 staging **경로 이름 그대로** `linkSync`했으므로, hook이나 durable
    journal을 읽는 **같은 UID 동시 writer**가 그 사이 staging을 갈아끼우면 교체본이 최종 body가 되고
    staging·journal이 삭제됐다(같은 digest → 남의 inode 입양 / 다른 digest → "성공한 커밋 + 잘못된 body",
    reopen은 증거가 사라진 뒤에야 탐지). 이미 발행된 body의 소유를 dev/ino/**size**로만 봤으므로 **같은
    inode·같은 크기 제자리 내용 변경** 뒤에도 journal이 지워졌다. → ① `ownershipOf()`가 **열린 fd 하나로**
    dev+ino·정확한 바이트 수·내용 SHA-256을 판정한다(`absent`/`ours`/`foreign` · `O_NOFOLLOW` · ENOENT만
    "아직 없다") ② **hook 이후·link 직전** staging 재증명(같은 digest의 다른 inode도 거부 — 입양 금지)
    ③ **link 직후** 최종 이름 재증명 → EEXIST 경합·교체본을 그 자리에서 `journal_body_foreign`으로 접고
    **staging(= body 바이트의 유일한 사본)을 지우지 않는다** ④ **`finishJournal()` 하나가 journal 삭제의
    유일한 경로**(정상 커밋 + "이미 목표 state" 복구 공용): `journal:cleanup` hook을 **먼저** 울린 뒤
    journal이 고정한 **모든** 최종 body를 전수 재증명하고(앞선 시도가 발행한 것까지) 어긋나면 **journal을
    남기고** fail closed다 ⑤ roll back은 최종 body가 애초에 없으므로 자기 경로 유지(주석에 이유 명시) ·
    초기 생성·기준 rollback 동작 불변 ⑥ 남의 최종 body를 지우거나 덮거나 채택하지 않고 실패를 조용히
    성공으로 바꾸지 않으며 재시도는 결정론적·멱등.
- **정직한 한계**: git은 **해싱한 fd를 그대로 exec하지 않고** body 발행도 **증명한 fd를 link하지 않는다**
  (`link(2)`는 pathname · Node에 `AT_EMPTY_PATH` 없음) → 두 창 다 0이라고 주장하지 않고 syscall 몇 개로
  줄인 뒤 **사후 재증명**으로 탐지한다 · hard-link 발행은 같은 파일 시스템·POSIX dev+ino 전제 ·
  `C-7`(키 없는 state↔event digest) · `C-37`(caller가 본 실패와 durable 진실의 갈림) · `C-36`(export된
  fault seam)은 그대로 열려 있다.
- **낡은 주석 정정**: store `recoverPendingCommit`의 "`C-37` 닫힘" · kernel `completeTaskWithArtifacts`와
  ...
  일뿐"(hook은 **동기 파일 변경도 한다** — `C-36` 증거 갱신). **`C-37`·`C-36`은 닫지 않았다.**
- **B 7건은 하나도 닫지 않았다**(`B-7`·`B-9`·`B-10`·`B-11`·`B-12`·`B-13`·`C-12`→B — 기한·트리거 원문 유지).
  **C 12건**: 11건(`C-35`·`C-5`·`C-17`·`C-29`·`C-19`·`C-36`·`C-37`·`C-30`·`C-38`·`C-39`·`C-26`) 상태 유지 +
  ID 없던 **승인 실행 파일 경로 schema regex와 runtime의 갈림**(`/a//b`·`/a/./b`·`/a/../b`)을 **`C-40`** 로
  등록하고 **이번에 정렬해 닫았다**: 정본 `APPROVED_PATH_PATTERN` 하나를 runtime validator와 schema가
  공유 · 동치를 pattern 항등 + **양/음성 표 전수 + 길이 상한**으로 고정 · 정렬 전 **1021 케이스** 실측으로
  명령형 판정과의 동치를 먼저 증명 · **양방향 mutation**(schema만/runtime만 옛 regex) 전부 kill.
- **테스트(worker 자기보고 — 독립 실측 아님)**: kernel **103/103**(98 → 103) · controller **58/58** ·
  ...
  race-sensitive subset(경계+kernel) **3회 직렬 123/123** · `npm run test:exec` **361/361**(353 → 361) ·
  `tsc --noEmit --pretty false` clean · `build` + `git diff --check` clean · `node --check` emitted 5파일 ·
  **dist 런타임 프로브 2종**(A1 제자리 교체 → `boundary_git_digest_mismatch`·spawns=1·sentinel 미실행 +
  C-40 비정규 경로 `invalid_manifest` / A2 `journal:cleanup` 같은 크기 제자리 변경 → `journal_body_foreign`·
  **journal 보존**·파일 미삭제·미덮어쓰기·**reopen도 완료된 run으로 보고하지 않음**) ·
...
- **다음 게이트는 fresh Codex `gpt-5.6-sol` xhigh read-only 독립 8차 리뷰**(`409dee2..HEAD` 전 범위)이고
  위 fixed 판정 **전부가 재확인 대상**이다. **A=0**일 때만 M5b가 M5c로 전진한다.

## 이전 상태 (2026-07-28 — V3 **M5b stable controller · 6차 리비전 완료** · dated history · **7차 리뷰가 A1/A3를 PARTIAL로 다시 열었다**)
...
- **⚠ 아래 "2차 리비전이 A=5를 전부 닫았다" 서술 중 A2·A5b는 부분적이었다.** 3차 독립 fresh Codex
  `gpt-5.6-sol` xhigh read-only 재리뷰(`409dee2..38b8d32`)가 **REVISE · A/P1=3**을 냈다. 공통 뿌리는
  **"공개 표면을 권위로 신뢰했다"** 이다. 3차 리비전이 닫은 것:
  - **A1 — attested read-only provider가 공개 `spawn` seam으로 위조 가능.** `CodexCliProviderOpts.spawn`은
    공개 임의 callback인데 생성자가 `opts.spawn ?? nodeSpawn`을 포착한 **모든** 인스턴스를 증명했다 →
    증명을 통과한 callback이 argv·env를 무시하고 임의 write/command/network를 할 수 있었다. TS
    `private readonly spawnFn`도 emitted JS에서 writable own field였고 **controller 테스트가 실제로 그것을
    덮어썼다**. → ① 모듈 사설 `PRODUCTION_SPAWN`(적재 시점의 진짜 `node:child_process.spawn`)
    ② `#spawn`·`#sessions`를 **ECMAScript `#private`** 로 봉인(대입·`defineProperty` 불가)
    ③ **`opts.spawn`을 준 인스턴스는 증명하지 않는다**(하위 계층 provider 단위 테스트용 untrusted seam으로
    유지 — `codexCliProvider.test.ts` **58건 커버리지 그대로**) ④ 함수 아닌 `spawn`은 `codex_config_invalid`
    ⑤ **controller 성공 경로 테스트를 실제 OS 자식 프로세스로 전환**: 기존 `src/exec/__fixtures__/fake-codex.mjs`를
    절대 `process.execPath` shebang 래퍼(0700)로 감싸 default `nodeSpawn`이 직접 실행 → production
    생성·증명·봉인·경계·argv·env·stdin·파서를 전부 지난다(codex/claude 추론·네트워크 **0**).
    세션 종료 관측도 내부 map 교체를 버리고 **공개 API 프로브**(같은 id `start`가 `codex_session_exists`인지
    `codex_prompt_invalid`인지)로 바꿨다.
  - **A2 — error taxonomy provenance가 exported class로 위조 가능.** `ControllerError`가 public
    constructible이고 `atBoundary`가 `instanceof ControllerError`를 보존했으므로 handoff가
    `new ControllerError("result_accepted", …)`만 던지면 `status:"failed"` + `marker:"result_accepted"`가
    만들어졌다. → 모듈 사설 `ISSUED_HERE` WeakSet만 provenance이고 경계는 **예외 없이** 고정 코드로 접는다.
    호출자 콜백 전수 차단: handoff · provider start/send/events · **`opts.nowMs` 시계**
    (`controller_clock_unreadable`) · **`opts.kernel` 전 메서드**(`atKernel`) · **kernel 반환값의 throwing
    getter**. kernel native 코드는 **닫힌 허용 집합 `KERNEL_MARKERS`(23종)** 일 때만 입양하고 나머지는
    `kernel_rejected`다 — `result_accepted`는 어떤 경로로도 실패 marker가 될 수 없다. 신뢰된 정적
    import(`verifyArtifactFile`·`verifyExecutionBoundary`)만 `atTrusted`로 코드를 입양한다(근거는 호출 지점).
    `consumeExactlyOneTerminal`은 클래스 대신 **factory**를 받아 소비자가 자기 provenance를 붙인다.
  - **A3 — 다중 artifact 등록과 완료가 비원자적.** `registerArtifact`를 산출물마다 durable commit한 뒤 별도
    `submitResult`였으므로 뒤쪽 실패 시 **앞선 artifact·event·revision만 남고** 재시도가 revision을 쌓았다.
    → kernel 원자 트랜잭션 **`completeTaskWithArtifacts`**: 한 `#mutate` 안에서 envelope(`artifactRefs`는
    비어 있어야 함) · summary · body · 전이를 먼저 닫고, 산출물 전체를 소유권 · writableRoots · 파일/hash/
    symlink · role · **개수 상한 `LIMITS.maxArtifactRefs`=16** · **경로 중복 `artifact_path_duplicate`** 로
    검증·등록한 뒤 그 포인터로 envelope를 채워 `acceptMessage`가 재대조하고 record+event+메시지+`completed`를
    **한 커밋**으로 반영한다. 집행 불변식은 `registerArtifact`와 **같은 헬퍼**(`addArtifact`) 공유.
    기존 `registerArtifact`/`submitResult` API·테스트는 호환 유지.
- **같은 라운드의 B 2건도 유예하지 않고 닫았다**: **`B-14`**(첫 terminal 뒤 늦은 이벤트·중복 종료·iterator
  throw 경로의 usage 회계 — 원 기한 *M5c 두 번째 provider/retry 배선 전*) · **`B-15`**(`ReviewSubject`
  한 줄·정규형·bounded·정규 16진 hash closed 검증 + frozen 스냅샷 — 원 기한 *M5c/`C-19`*).
  리뷰 C 1건은 **`C-32`**로 등록 후 닫았다(inbox 항목 단일 읽기). **신규 open C**: `C-33`(`KERNEL_MARKERS`가
  수동 유지 목록 — M5c marker 분기 로직 도입 시) · `C-34`(`codeOf`가 대부분 경로에서 중복 방어 — mutation
  실측 기반 정직 표기). **`C-31` 축소 재기술**(white-box 관측 2곳 제거, 남은 절충은 "자식 프로세스 회수까지는
  보지 않는다" — 기한 그대로 `B-13`) · **`C-30` 범위 축소**(usage 회계는 `B-14`가 닫았고 marker e2e만 남음).
- **3차 리비전 검증(worker 자기보고 — 독립 실측 아님)**: `stableController` **52/52** ·
  `orchestrationKernel` **74/74** · `codexCliProvider` **58/58** · `reviewer` **21/21** ·
  `npm run test:exec` **322/322** · authority/atomicity/timing subset **3회 직렬 205/205** ·
  `npx tsc --noEmit` clean · `npm run build` + source/dist parity · `git diff --check` clean.
  **mutation**: A1/A2/A3 핵심 게이트를 각각 되돌려 회귀가 죽는 것을 확인하고 정확히 원복했다.
  **살아남은 mutation 1건**(`codeOf` provenance 느슨화)은 실제 도달 경로(kernel 반환값 getter) 회귀를
  추가해 죽였고 남은 중복성은 `C-34`로 등록했다 — 숨기지 않는다.
  **`npm test` 전체 suite·acceptance·stress·live는 실행하지 않았다**(최종 M5d handoff에서 supervisor가 직렬 1회).
- **다음 독립 리뷰가 특히 볼 지점**: ① `#private` 봉인과 증명 등록 분기가 **정말로** 임의 executor를 배제하는가
  (subclass·proxy·`defineProperty`·모듈 재적재) ② `atBoundary`/`atKernel`/`atTrusted` **누락된 seam이 없는가**
  (특히 새로 추가한 kernel 반환값 읽기 경로) ③ `KERNEL_MARKERS`가 성공 marker를 담지 않고 **닫혀 있는가**
  ④ `completeTaskWithArtifacts`가 **모든** 실패 경로에서 부분 적용 0인가(특히 `commitRun`의 body 파일 쓰기)
  ⑤ 새 controller 테스트 harness가 실제로 production 경로를 지나는가(주입 seam 잔존 여부).
- **아래 2차 리비전 서술은 dated history로 보존한다** — 충돌 시 이 절이 현행이다.

## 이전 (2026-07-28 — V3 **M5b 2차 리비전**)
...
- **인접 감사에서 새로 등록한 C**: `C-26` — 경계 **밖의** `runProcess` git 호출자(worktree 유틸 등)는 여전히
  `git`을 이름으로 부르고 env를 상속한다. 리뷰 지시대로 범위를 경계로 한정했고, 이들은 **승인 커밋을 증명하지
  않으므로 경계 우회는 아니다**. 기한: **controller가 worktree 조작을 자동화 경로로 쓰기 전(M5c)**.
- **열린 항목 정본(현행)**: B = `B-7` · `B-8` · `B-9`(live/배선 하드 게이트, offline M5b는 막지 않는다).
  C = `C-17` · `C-18` · `C-19` · `C-21` · `C-22` · `C-24` · `C-25` · `C-26`(`C-23`은 이번에 fixed).
  **열린 A는 없다. 새 B도 없다.**
