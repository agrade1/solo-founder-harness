...
`npm test` → **exec 75 pass / core(=test:core: core+tools+providers+commands) 157 pass / acceptance 71 PASS**, 실패 0. (M3b.2 offline 최종 보완 2026-07-20 반영)
...
> **live 검증 범위 구분**: M3a는 **non-empty MCP profile**(expected 서버 1개 + `mcp__expected__read_thing`)로 headless preflight의 strict 격리·canary 차단을 **실제 Claude 2.1.215에서 실측 통과**했다. 반면 **M3b.2의 empty MCP config + hook-settings + 대화형(TUI) Hook 경로는 아직 live 미검증**(offline seam만) — 아래 M3b.2 live acceptance 참조.
...
- **M3b.2 live acceptance(남음, 수동 — empty MCP/settings/Hook 대화형 경로 live 미검증)**: 실제 Claude Hook로 `--setting-sources ""` 수용, exec-form Hook 6종 등록, 6 payload(PreToolUse/PostToolUse/PostToolUseFailure/PermissionRequest/PermissionDenied/SessionEnd), trace redaction·0600, TUI 유지·stream-json 미사용 확인. **M3a(non-empty MCP strict)와 달리 이 경로는 아직 실제 Claude로 미검증.** **M3c(shadcn read)는 이 live 통과 후.**
