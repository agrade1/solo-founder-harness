...
## 6.5 M7 범위 경계 (2026-08-12 로드맵 배치 정정 반영)

아래는 **M7이 하는 일이 아니다** — 다른 마일스톤에 배치돼 있으니 M7 세션이 손대지 마라.

| 항목 | 어디로 | 왜 M7이 아닌가 |
|---|---|---|
| `run_process` action enum 확장(테스트 실행) | **M9 선결(하드 게이트)** | 현재 `["validate-plan"]` 하나·읽기 전용. M9의 "test review"가 요구한다 |
| `B-16` 신규 파일 발행 | **M9 선결** | 구현 파이프라인의 기본 동작 |
| `B-17` inbox 전달 소비(ack) | **M9 선결** | "검토 → 수정" 자동 전달의 전제 |
| `F2` 진행률·스피너·ETA | **M9 선결** | 병렬 worker가 다수 도는 첫 마일스톤에서 통증이 최대 |
| `F3` 문서 완료 → Claude Code 핸드오프 | **M10** | 문서 파이프라인이 전부 선 뒤에야 의미 |
| `F1` cross-project 학습 | **§11 선택적 확장** | 프로젝트 3개 축적 조건 |

M7이 **하는** 일은 §4 표의 완료 조건뿐이다. 그 표에 없는 기능은 대장에 적고 넘긴다.
