...
| 열린 대장 | A **0** · 등급 B **10** · 등급 C **100** · id **110** |
...
3. **명확화된 목표(2026-08-26)의 최소 골격 3개가 전부 닫혔다** — `B-40`(판정 ⑪) · `B-41`(⑫) ·
   `C-126`(⑬). **남은 1순위 = `C-138` live 실측**: 사용자가 `.env`에 Tavily 키를 채우면 즉시
   최소 시나리오(크레딧 1 · 검증 9항목 — `docs/handoff/C126_RESEARCH_ADAPTER_DESIGN.md`)를 돌려
   ⓐ 실제 external 모드 ⓑ receipt·raw 해시 대응 ⓒ **키가 산출물·프롬프트·자식 env에 없음**
   ⓓ **malformed 오탐 없음**(A-4 수정의 회귀 신호) ⓔ ≈64KB 프롬프트 수용을 확인한다.
   그 뒤 `C-127`(v1 검증 차단) · `C-125`(아이디어 개정 루프) · `B-43`/`B-44`(autopilot 게이트) ·
