...
> **[대체됨 2026-08-27] `docs/handoff/M13_ORCHESTRATION_KICKOFF.md`가 현행이다.** 이 문서는 판정
> ⑬ 시점의 스냅샷으로 남긴다(§4 함정 열은 M13 §4가 승계·확장했다).
...
| 항목 | 값 |
|---|---|
| `main` | 판정 ⑬까지(B-40 kill · B-41 체크포인트 · C-126 리서치 — 착수 전 `git log`로 실측하라) |
...
| live 실측 | 판정 ⑦(단일 task 산출물) · ⑧(12-task DAG 초안) · ⑨(multi-task 부분) · ⑩(**문서 단계 4종 완성**) · ⑪·⑫(v1 층 — mock로 충분) · **⑬ ⓔ-live(외부 리서치 실호출 · Tavily 2 크레딧 · 검증 9항목)** |
...
3. **명확화된 목표(2026-08-26)의 최소 골격 3개가 전부 닫혔고 live까지 실측됐다** — `B-40`(판정 ⑪) ·
   `B-41`(⑫) · `C-126`+`C-138`(⑬ · 외부 검색 실호출 · 검증 9항목 통과). **남은 순서**: `C-127`(v1 검증 차단) · `C-125`(아이디어 개정 루프) · `B-43`/`B-44`(autopilot 게이트) ·
   `C-123`(consumes 계약) · `C-136`(summary/vault에 research mode).

4. (이전 순서 기록) **최소 골격 3개 중 둘이 닫혔던 시점의 서술** — ~~`B-40` kill 게이트~~(판정 ⑪) ·
