...
| ~~`B-54`~~ **closed (M16 · 결정으로 종결)** | **`pipeline_artifact_drift` 안내가 제시하던 두 탈출구가 둘 다 막혀 있었다**(live 실측 2026-09-01). M15가 안내의 거짓 절반을 걷어냈고, **M16이 남은 둘을 처리해 닫는다**: **ⓐ 형제 안내 하나가 더 있었다** — `approveCheckpoint`의 drift 거부가 여전히 *"파일을 복원하거나"* 라고만 했다. 그 상태에선 pending이 있어 `reject`가 실제로 열려 있으므로 거짓은 아니지만, 사람이 ⓐ를 먼저 시도하고 나서야 되살릴 방법이 없음을 안다. 같은 사실(**하네스는 내용을 보관하지 않는다**)을 적고 통하는 길을 명령까지 적었다(함정 27 · 테스트로 고정). **ⓑ 승인 산출물 내용 스냅샷은 보관하지 않기로 결정했다.** 근거: (1) `B-47`·`A-4`·`B-53`이 닫힌 뒤 남는 drift는 **사람이 승인 문서를 직접 고친 경우**뿐이고, 그 되돌리기는 사람이 만든 변경이라 git·백업이라는 **보편 도구가 이미 푼다** (안내가 이제 그것을 명시한다) (2) 문서 전체 사본을 durable에 두면 저장·회전·checkpoint와의 정합이라는 새 상태가 생기고, 그 정합이 어긋나면 **영수증이 두 벌**이 된다 — 이 레포가 반복해 다친 계열이다 (3) `awaiting_run`에서 `restart`를 여는 대안은 설계가 명시적으로 기각한 **체크포인트 회피 통로**다. **뒤집는 조건**: 프로젝트가 git 밖에서 운용되는 사용례가 생기면 재검토한다 | **closed 2026-09-07** | `M14 진행 판정 ⑮` ⓝ · `M16` |
| ~~`B-64`~~ **closed (M16)** | **`maxProcessesPerRun`(32)이 attempt 롤오버마다 리셋된다** — `launchedProcesses`가 `task.execution.operationReceipts`만 세는데 롤오버가 그것을 비운다. **M16에서 kernel fixture로 관측했다(코드 대조에서 승격)**: 32까지 채운 뒤 pause→resume 롤오버 1회에 run 집계가 **28로 떨어졌다**(`managedProcess.test.ts`의 [B-64]). 주장이 그대로 맞았다. **수정**: `task.execution.priorAttemptProcesses`(누적·단조)를 두고 롤오버 지점 둘(preflight 재준비 · `resumeTask`)이 그 attempt의 수를 이월한다 — 상한이 attempt 밖에 산다. schema·validator·golden 동반 갱신. mutation: 이월분 무시 → 1건 RED | **closed 2026-09-07** | `M15 판정 ⑯` ⓠ · `M16` |
| ~~`C-156`~~ **반증 (M16)** | 등재 문구는 *"`accounting.elapsedMsUsed`가 합계가 아니라 running max인데 소비량으로 렌더된다"* 였다(코드 대조만). **M16에서 kernel fixture로 관측하니 max인 것은 맞지만 결함이 아니다**: 경과 상한 `maxElapsedMs`는 `budgetStartedAt + maxElapsedMs = budgetDeadlineAt`을 **now와** 비교해 집행되는 **wall-clock**이라(`seedAccounting`), 그와 비교 가능한 값은 "예산 시작 이후 경과"이고 그 누적기는 max가 맞다. **turn 합계로 바꾸면 병렬 task의 같은 wall time을 중복 계산한다.** `remainingBudget()`은 이 값을 쓰지도 않는다. **남는 진짜 잔여는 다른 것이다** → 신규 `C-158`. 렌더에는 무엇인지 적었고(`orchestrationStore.ts`) 관측 테스트를 남겼다(`autopilotLifecycle.test.ts`의 [C-156]) | **반증 2026-09-07** | `M15 판정 ⑯` ⓠ · `M16` |
| `C-158` | **`chargeTurnUsage`/`chargeDispatchTurnUsage`의 `elapsedMs`가 호출자 값이고 계약이 주석에만 있다** — 누적기가 max이므로 호출자가 **turn 단위 delta**를 넘기면 회계가 조용히 과소 집계된다. 지금은 **production 호출자가 0**이라(테스트만 부른다) 관측되지 않는다. `C-156`을 반증하는 과정에서 갈라낸 진짜 잔여다. 수정 후보: 인자를 없애고 **kernel이 `now - budgetStartedAt`으로 직접 계산**한다(이 파일의 선례: attempt deadline은 *"kernel이 계산한다 — 호출자 값 아님"*). 공수는 호출부 ~30곳(전부 테스트) | **첫 production 호출자가 생기기 전** | `M16` |
...
| `C-154` | **파이프라인 입구·아이디어 결박·취소 전이 5종** — **ⓐ closed(M16)**: `init`이 콘솔에서도 4단계 파이프라인 경로를 말한다(예전엔 생성된 `CONTEXT_SUMMARY.md` 안에만 있어 `init`을 돌린 사람은 그 파일을 열기 전까지 아무 안내도 못 받았다 · 문구는 그 템플릿과 같은 명령이다). **ⓒ closed(M16)**: 승인 뒤 `00_IDEA.md`를 통째로 바꿔도 2단계가 진행했다(실측 재현) — `runStateSources`가 산출물만 담고 **심사받은 입력**을 안 담았다. 결박했고 drift가 이제 본다. 부작용을 결정으로 고정했다: **아이디어가 아예 없으면 승인 대기로 넘어가지 못한다**(`buildManifest` fail closed) — 결박 전에는 넘어갔고 그때 승인자는 무엇을 심사했는지 알 수 없는 영수증을 받았다. 골든 2곳을 '왜 늘었나·왜 이 자리인가'와 함께 갱신. mutation: 결박 제거 → 3건 RED. **남은 것 ⓑⓓⓔ**: ⓑ 97B 자리표시자 아이디어가 1단계를 통과한다(프롬프트 층이 빈 아이디어를 **명시적으로 허용**한다 — 고치려면 최소 내용 기준을 정해야 하고 과차단 위험이 실질적이라 손대지 않았다) · ⓓ 사용자가 조종할 수 있는 활성 파이프라인 취소가 없다(새 명령이 필요하다 · 승인 게이트와의 이음새를 먼저 설계해야 한다) · ⓔ 단독 `task-prompt` 산출물에 경로 루트 선언이 없다(계약이 `handoff.ts:193-207`에만 있다). 심각도 C | **파이프라인을 남에게 쥐여 주기 전** | `M15_CODEX_AUDIT.md` §5 · `M16` |
| ~~`C-155`~~ **closed (M16)** | **`briefGenerator`의 `asStrings`가 잘못된 항목을 조용히 버렸다 — `deps`에서 의미가 바뀐다** — `deps: "task-a"`(스칼라)가 `undefined`가 되고 스케줄러 둘(`mission.ts` · `parallelMission.ts`)이 **의존성 없음**으로 읽어 선행 완료 전에 실행했다(병렬 모드는 자동 병합까지). **수정**: 형태 오류를 throw한다(`id`/`role`/`task`가 이미 그러므로 비대칭을 없앴다) + **없는 id를 가리키는 deps를 파싱에서 막는다**(스케줄러는 이미 fail closed지만 오타 하나가 mission 전체를 말없이 dep_unmet으로 만들었다). **순환은 검사하지 않는다** — 같은 이유로 스케줄러가 닫고 검사를 더해도 막는 사고가 없다(기각 근거를 코드에 남겼다). mutation: 무음 필터로 되돌리면 2건 RED | **closed 2026-09-07** | `M15_CODEX_AUDIT.md` §5 · `M16` |
| `C-152` | **fanout 하위 에이전트가 직렬로 돈다** — `P1` 실측(2026-09-03): step 소요 합 1,142.2초 = wall clock 1,142.2초(**차이 0.0초** = 겹침 0). 4개는 `tech_lead` 계획을 서로 독립적으로 나눠 받고 서로의 산출물을 입력으로 받지 않는데도 한 줄로 선다. spawn 구간 합 722.6초 · 최댓값 282.3초 → **병렬이면 440.3초 절감(run 전체 -39%)**. 심각도 C · 확률 = fanout을 쓰면 항상 · 영향 반경 = 벽시계 시간(정확성·영수증 무관) · 유예 비용 = fanout 단계마다 누적 · 수정 공수 = **중** (프로세스 회계 `maxProcessesPerRun`·부분 실패 처리·영수증 순서와 얽힌다 — `B-10` 계열의 열린 결함 위에 얹힌다) | **fanout 단계 시간이 실사용에서 문제가 될 때** | `M15 진행 판정 ⑯` ⓛ · `M15_P1_LIVE_MEASUREMENT.md` §3ⓐ · **M16 보강 — 선행 조건이 하나 늘었고 하나는 풀렸다**: ⓐ `B-64`는 closed다(풀림). ⓑ 그러나 **`B-48`이 같은 세션에서 예산 가드를 모든 모델 호출 직전으로 내렸다** — 지금 fanout을 `Promise.all`로 바꾸면 하위 N개가 **동시에 같은 잔량을 보고 전부 통과**해 초과분이 호출 1개에서 **N개**로 돌아간다. 병렬화는 예산 **예약(reservation)** 을 함께 설계해야 한다(호출 전에 몫을 미리 차감). ⓒ 그 밖에 순서 결정성도 함께 봐야 한다: `spawned_agents`·`usagePerAgent`·`completed_steps` push 순서와 콘솔 로그가 현재 spec 순서에 결박돼 있고 골든이 그것을 잰다 — 결과를 모아 **spec 순서로 커밋**해야 한다. ⓓ 부분 실패 의미가 미정이다(현재는 throw 하나가 루프를 끊는다). **M16은 이 넷을 확인만 하고 착수하지 않았다** — 이득이 wall-clock뿐인데 방금 세운 예산 가드를 되돌릴 위험이 있어서다 |
| `C-153` | **입력 토큰의 98%가 호출마다 반복되는 고정 바닥이다** — `P1` 실측: 에이전트별 입력 69,800~77,087(편차 **9.5%**) · 입력:출력 **6.9:1**. 체인 뒤쪽으로 가도 입력이 커지지 않는다. **M16이 그 바닥을 갈랐다**(대장이 요구한 빈 프롬프트 대조 · `claude -p` 1회씩 2회): **ⓐ 36,148** = provider 자체(claude-code system prompt + 도구 스키마 · 중립 빈 디렉터리) · **ⓑ ~4,016** = **하네스 레포의 `CLAUDE.md`**(같은 프롬프트를 레포 안에서 부르면 40,164 — provider를 `cwd` 없이 spawn해 하네스 개발 지침이 **모든 agent 호출에 실린다**) · **ⓒ ~29k** = 하네스 몫(공통+역할 프롬프트+아이디어+findings+출력 형식). M16 live 실측 재확인: 호출당 입력 68,846·70,425·82,490. **그래서 "영어화로 1.46배 절감"은 과대주장이다** — 레버가 닿는 것은 ⓒ뿐이라 상한이 29k×(1−1/1.46)≈9.1k = 호출당 입력의 **약 13%**다(46%가 아니다). 대신 **ⓑ가 더 싼 레버로 드러났다** → 신규 `C-159`. 심각도 C · 영향 반경 = 모든 live run의 비용 · **남은 미증명**: 표본이 각 1회이고 ⓒ는 뺄셈으로 얻은 값이다(직접 계측하지 않았다) | **live 비용이 제약이 될 때, 또는 공통 프롬프트를 손대기 전** | `M15 진행 판정 ⑯` ⓛ · `M15_P1_LIVE_MEASUREMENT.md` §3ⓑ · `M16` · **M16 live 보강 — "98%·편차 9.5%"는 비평 루프·게이트 점프가 **없는** run에 한정된 수치다.** 비평 2회 + 게이트 점프 1회가 든 run의 호출당 입력은 **68,846~311,236(편차 352%)** · 입력:출력 **10.7:1**(P1은 6.9:1). 바닥 자체(69k~83k)는 재확인됐지만 **"비용은 몇 번 부르는가의 함수"는 이 run에서 거짓**이다 — 호출 하나가 바닥의 4.5배였다(비평 R2 · 4분 4초 · **원인 미규명** · `conclusion_only`가 의도대로 좁히는지 확인 대상) |
| `C-159` | **하네스 레포의 `CLAUDE.md`가 모든 live agent 호출에 실린다** — `claudeCodeProvider.runClaude()`가 `spawn(claudeBin(), args, {stdio})`를 **`cwd` 없이** 부르므로 자식 `claude`가 하네스를 실행한 디렉터리(= 이 레포)를 프로젝트 컨텍스트로 읽는다. 실측(같은 `.` 프롬프트 1회씩): 레포 안 **40,164** vs 중립 빈 디렉터리 **36,148** = **~4,016 토큰/호출**. 그 내용은 하네스 **개발 지침**이지 심사 대상 아이디어와 무관하다 — 비용이자 프롬프트 오염이다. 수정 후보: `cwd`를 프로젝트 루트로 넘긴다(한 줄). **먼저 확인할 것**: 프로젝트 디렉터리에 `CLAUDE.md`가 생기면 그것이 대신 실린다 — 그 경로도 함께 닫아야 한다. `C-153`을 가르는 과정에서 나왔다 | **다음 live 계측 전**(비용·오염 둘 다) | `M16` |
| `C-160` | **live 4단계 계측의 선행 조건이 `B-47`이 아니었다** — `B-47`은 closed이고 **mock 4단계는 완주한다**(설계 게이트 거부 후 resume 포함 · M16 실측). 그런데 live는 **1단계 게이트를 못 넘는다**: `founder_ceo`가 '검증'을 두 번 내고 되돌림 예산이 소진돼 `ceo_decision_verify`로 멈춘다(M16 · 호출 16회 · 입력 1,582,482). CEO가 요구한 것은 **3시간짜리 실물 테스트**(카드 CSV 가맹점 문자열 식별률 · 기존 무료 대안 화면 확인)이고 **하네스는 그 증거를 만들 수 없다** — `B-50`의 사람 검증 레버가 정상 작동하지만 그 사람이 오프라인 작업을 해야 한다. **'진행' 판정은 누적 0회이고, 이 run은 P1보다 구체적인 아이디어였는데도 3라운드 연속 '검증'이었다** → 가설 ⓐ(아이디어가 약했다)는 이 표본으로 약해졌다(ⓑ와 아직 못 갈랐다). 선택지: ⓐ 사람이 실물 테스트 후 `## Decision`을 고친다(모델 0회 재판정) ⓑ 게이트 없는 경로로 2~4단계 기계 동작만 계측한다(그것은 "live 4단계 완주"가 아니다) | **다음 live 계측 계획을 세우기 전** | `M16` · `M16_LIVE_MEASUREMENT.md` |
| `C-161` | **`init` 템플릿을 그대로 따르면 아이디어 한 줄이 지시문에서 조용히 사라진다** — `taskPrompt.ts:120`이 `## 아이디어 한 줄 정의` 아래의 **첫 불릿**을 뽑는데(`extractSectionBullets`), `init.ts:12-13`이 만드는 템플릿은 그 자리에 `(여기에 아이디어를 한 문장으로 적는다)`라는 **맨 문장**이다. 사용자가 템플릿 형식을 그대로 따라 문장으로 쓰면 추출이 `undefined`가 되고 작업 지시문의 `- 아이디어:` 줄이 **말없이 빠진다**(`if (oneLine)` 가드가 삼킨다). 불릿이어도 **여러 줄이면 첫 줄만** 실린다. 생성기와 소비자가 같은 형식을 안 쓰는 계열(`C-130` 부류)이고, 실패가 조용하다. 심각도 C · 수정 방향 = 템플릿을 불릿으로 바꾸거나(한 줄) 추출이 문단도 받게 하고, 둘 중 무엇이든 **형식 계약을 한 곳에서** 정한다 · 발견 경위: M16에서 테스트용 아이디어 문서를 쓰다가 추출을 직접 돌려 봤다 | **작업 지시문을 근거로 구현을 시작하기 전** | `M16` |
| ~~`C-164`~~ **closed (M16 · 사용자 요구)** | **승인 게이트가 존재 이유인데 그 승인이 읽을 수 없는 양이었다** — 체크포인트 하나에서 사람이 읽어야 할 원문이 **66KB(한글 약 2만 자)**이고 그것이 4번 반복된다(실측: `01_RESEARCH` 18.7KB + `02_PRD` 19.3KB + `05_RED_TEAM` 13.1KB + `06_CEO_DECISION` 14.0KB). `harness summary`는 기계 상태(workflow·실패·경고 수)만 내고 내용 요약이 아니었다. **수정**: 체크포인트마다 `outputs/CHECKPOINT_REPORT.md`를 만든다 — 판정 한 줄 → 지금 결정할 것(실행 가능한 명령) → 담당별 핵심 판단 → 걸린 Critical → 원문 목록 순. **live 산출물 실측 66KB → 5.8KB(91% 감축)**. **새 추출 규칙을 만들지 않았다**: 판정 토큰은 게이트의 `CEO_DECISION_TOKENS`, Critical은 비평 루프의 `extractCriticalRisks`, 판단 한 줄은 checkpoint manifest의 `seeds`를 그대로 쓴다 — 따로 파싱하면 보고서가 게이트와 다른 말을 한다. **결박하지 않는다**(증거가 아니라 뷰다 · 결박하면 이 파일을 쓰는 행위가 곧 drift다) 대신 머리에 checkpoint id를 박아 낡은 보고서를 알아볼 수 있게 했다. **자르지 않는다**(조용한 절단 금지 · 테스트로 고정). 업계 조사에서도 harness 생태계는 승인을 제약 집행 문제로만 다루고 **사람이 읽을 UI/UX는 공백**이었다 | **closed 2026-09-08** | `M16` · 사용자 요구 |
| ~~`C-163`~~ **closed (M16 · 사용자 live가 잡았다)** | **예산 소진이 리서치 실패로 둔갑해 틀린 복구 안내가 나갔다** — `B-48`이 예산 가드를 모델 호출 직전(`runStepWithRegen`)으로 내리면서, 리서치 **2차 호출을 감싼 catch**(B-2가 모든 예외를 `research_second_pass_failed`로 안정화한 자리)가 그 예외까지 삼켰다. 결과: `--max-tokens` 소진인데 `failed_reason`이 `research_second_pass_failed`가 되고 *"원인(키 오류·네트워크·크레딧)을 고친 뒤"* 라는 **틀린 원인**을 안내했다 — B-1이 만든 진짜 안내(예산을 올려 재개 / 상한을 비우고 재개 · 같은 상한 재개는 호출 0회로 다시 막힌다)는 **한 줄도 나오지 않았다**. **2026-09-08 사용자 live 실행이 그대로 맞았다**(실제 명령·실제 크레딧). 수정: `TokenBudgetExceeded`는 봉인만 하고(`research_budget_stopped`) **다시 던진다** — 예산은 리서치의 문제가 아니라 run의 문제이고, step loop의 예산 처리가 제 코드와 제 안내를 내야 한다. **교훈**: 예외를 안정 코드로 바꾸는 catch는 **자기 도메인 밖의 예외를 통과시켜야 한다** — 안 그러면 나중에 추가되는 모든 횡단 관심사(예산·취소·타임아웃)가 그 자리에서 도메인 오류로 둔갑한다. mutation: 통과 제거 → 1건 RED | **closed 2026-09-08** | `M16` · 사용자 live |
| ~~`C-162`~~ **closed (M16 · 사용자 요구)** | **`init`이 작업 폴더의 아이디어 문서를 인수한다** — 예전엔 템플릿만 만들고 사람이 자기 문서를 `projects/<name>/docs/00_IDEA.md`로 **직접 복사**해야 했다. "작업 폴더에 문서 두고 설치해서 실행"이라는 사용 모양(사용자 원 기획)에서 그 복사가 유일한 수작업이었다. 후보 6종(`docs/00_IDEA.md` · `00_IDEA.md` · `docs/idea.md` · `idea.md` · 대문자 2종)을 순서대로 찾아 **템플릿 대신 그 내용으로** 만든다. **옮기지 않고 복사한다**(`init`은 되돌릴 수 없는 일을 하지 않는다) — 대신 정본이 어디인지 인쇄한다(사본이 둘이면 사람이 엉뚱한 쪽을 고친다). 이미 프로젝트 문서가 있으면 덮어쓰지 않고 **무시했다는 사실과 내용이 다른지**를 말한다(조용한 무시 금지). **구현 중 실물 실행으로 거짓 안내 하나를 잡았다**: 문서를 담아 놓고도 "아이디어를 채우세요"라고 지시했다(함정 27) — 테스트로 고정. 소문자 후보를 대문자보다 앞에 둔 이유도 같은 계열이다(대소문자 무시 FS에서 인쇄 이름이 실제 이름과 달라진다). mutation: 인수 제거 → 1건 RED | **closed 2026-09-08** | `M16` · 사용자 요구 |
...
C-130 C-131 C-132 C-133 C-134 C-136 C-137 C-139 C-141 C-142 C-143 C-145 C-146 C-147 C-148 C-151 C-152 C-153 C-154 C-157 C-158 C-159 C-160 C-161
```
