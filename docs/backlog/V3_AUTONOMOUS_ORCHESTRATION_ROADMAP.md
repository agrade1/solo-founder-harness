...
| `B-59` | **`exec`/`mission` 세션에 집행되는 쓰기 경계가 없다 (2026-09-03 검수에서 확인 · 등재는 09-04 — 앞선 등재에서 빠뜨렸다)** — `permissionCompiler`가 `ownership`(담당 경로 glob)과 `hookDenyPatterns`(규칙으로 표현 못 하는 T3 정규식: curl|sh 등)을 만들지만 `sessionRunner.ts:126-136`이 소비하는 것은 `permissionMode`/`allow`/`deny`/`settingsPath`뿐이고, **두 필드는 비-테스트 소비자가 0이다**(grep 확정 · Codex 재검수도 CONFIRMED). `ownership`은 자연어 프롬프트에만 실린다. worktree는 **관례적 격리이지 파일시스템 봉쇄가 아니다.** `ownership` 필드 주석은 스스로 *"향후 훅에서 강제"*라고 적어 미구현을 밝히지만 `hookDenyPatterns`에는 그런 표시조차 없다. **v3 커널 경로(`autopilot`)는 무관하다** — 거기선 `stableController.ts:364`가 `task.ownership`을 실제로 집행한다. 이 항목은 `exec`/`mission` 경로 한정이다. 심각도 **B** · 확률 = 그 경로를 쓰면 항상 · 영향 반경 = 에이전트가 담당 밖 파일을 고쳐도 막히지 않는다(실제 방어선은 Claude Code의 allow/deny뿐이고 그 end-to-end 검증은 `permissionCompiler.ts:10-12` 주석 자신이 **미검으로 표시**한다) · **미검 잔여**: 실제로 경계 밖에 쓸 수 있는지 **돌려 보지 않았다**(정적으로 소비자 0인 것만 확정) · 수정 방향 = `ownership`을 훅으로 집행하거나, 집행하지 않는다면 task 문서의 "소유(쓰기 허용) 경로" 문구에서 **강제성 주장을 빼고** 그 사실을 세션 시작 시 알린다 | **`exec`/`mission`을 실사용하기 전** | `M15_CODEX_AUDIT.md` §5 #1 · `M15_UNVERIFIED.md` §2 |
