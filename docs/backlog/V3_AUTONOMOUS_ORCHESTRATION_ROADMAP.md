...
**등급 B (13).** (M14: **`B-49`·`B-46`·`B-50`·`B-52` closed** · `B-51` 신규(게이트 우회 실증) · **`B-46`·`B-49` closed** — 판정 ⑮ · `B-48`은 `C-125` 머지로 **트리거 도래**하여 열린 채 기한만 갱신 · `B-50` 신규.) (M13: **`B-42`는 판정 ⑭에서 closed** · `B-45`~`B-49` 신규.)
...
| `C-149` | **`last_failure.written`이 실패마다 통째로 덮인다** — 연속 resume에서 **이전 attempt가 정당하게 덮은** 승인 경로가 두 번째 resume부터 drift로 거부될 수 있다(`pipeline.ts` 실패 분기). `B-52` 수정 전후 동일한 기존 성질이고 `B-52` 구현자가 코드로 추론해 보고했다 — **재현은 하지 않았다**. 심각도 C · 확률 낮음(연속 실패 필요) · 영향 반경 = 파이프라인 복구 마찰 · 수정 방향 = `written`을 누적(합집합)으로 바꾸거나 attempt별로 보존 | 파이프라인 **연속 실패 복구를 주장해야 할 때** | `M14 진행 판정 ⑮` ⓚ · `docs/handoff/B52_DESIGN.md` |
...
**등급 C (103).** — 각 id의 **마지막 등재 행**이 현행 서술이다. (M14: **`C-125`·`C-140` closed** · `C-147`·`C-148`·`C-149` 신규.) (M13: **`C-127`·`C-135` closed** · `C-141`~`C-143`·`C-145`·`C-146` 신규(`C-144`는 live 증거로 **`B-49`로 승격**).)
...
C-130 C-131 C-132 C-133 C-134 C-136 C-137 C-139 C-141 C-142 C-143 C-145 C-146 C-147 C-148 C-149
```
