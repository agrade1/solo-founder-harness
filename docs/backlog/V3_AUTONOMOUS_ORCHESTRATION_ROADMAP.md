...
정본(이 절)      →  id 78   ← `C-104` closed(−1) · `C-105` 신규(+1) 반영
...
##### 열린 항목 — id 78건 (A 0 · **등급 B 6** · 등급 C 72)
...
| `B-34` | codex 홈의 코드·지시 로드 면이 **최상위 이름까지만** 판정된다 | CLI feature 차단을 **codex live로 실측**할 때 | 2261 |
| `B-35` | claude 격리 홈에 **내용 allowlist 없음** + `CLAUDE_CONFIG_DIR`/`USER` 공존 시 **계정 우선순위 미실측** | 승인된 홈에 **사람이 1회 로그인한 직후** | 2122 |
| `B-36` | 그룹을 **탈출한 자손을 탐지할 커널 능력이 없다**(`B-18`에서 분리) | **linux(cgroup)** 또는 sandbox/컨테이너 채택 | 2125 |
| `B-37` | 닫힌 action 집합으로 **회복 불가한 worktree 잔재** 두 모양(`B-31`에서 분리) | **현장에서 실제로 관측될 때만** | 2123 |
...
**등급 C (70).** — 각 id의 **마지막 등재 행**이 현행 서술이다.
...
##### **M11 진행 판정 ③ — 대장 전수 재판정(정본 절 신설) + `C-104`** (2026-08-23 · **이 절이 현행이며 아래 ②보다 최신이다**)
