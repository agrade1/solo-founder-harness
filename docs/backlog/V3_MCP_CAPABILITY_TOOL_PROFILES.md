...
> **M3 완료(재개방 금지)** · **M4a·M4b·M4c 완료 → M4 전체 완료(offline 검증)** · **M5는 미완료**
...
> M4b `11775fd`+`ab63eac`). **원격 push/PR/merge는 없다.** 부하·live 재실행은
> **nonblocking release-readiness backlog**(`B-1`/`B-2`)이며
...
> **M4c 추가 — 승인 manifest가 dependency pin 규칙의 집행 지점이 됐다**: 이 문서의 **MCP 패키지
> `@latest` 금지**는 이제 `MilestoneApprovalManifest.allowedDependencies`에서 런타임으로 강제된다.
> `latest`·범위(`^`/`~`/`>=`)·`x`·2자리 버전·dist-tag는 `dependency_not_pinned`로 거부하고,
> M5 executor는 순수 술어 `dependencyAllowed(manifest, name, version)`로 **정확히 pin된 조합만**
> 조회 통과시킨다(deny-by-default). 네트워크 도메인도 같은 방식이며 **하위 도메인은 자동 허용하지 않는다**.
> 다만 **M4c는 아무것도 실행하지 않는다** — 설치·네트워크 접근·MCP 기동은 여전히 M5 범위이고
> 레포 hard deny는 manifest 내용과 무관하게 항상 더 강하다.
>
...
>
> **M3d는 아직 완료가 아니다.** M3d.2는 **여덟 번째 리비전**까지 구현·offline 검증만 끝났고
> **live runner 3종·evidence 3건은 pending, 부하(stress) acceptance도 미충족이며
> (여섯 번째 리비전 세션 FAIL — 호스트 외부 부하 + 고정 5초 child startup deadline 2건, lock 계층 계약은 정상 ·
> 일곱·여덟 번째 리비전 세션에서는 재실행하지 않았다), M3d 완료를 승인한 리뷰도 없다.**
> 리뷰 이력(2026-07-26 갱신): fresh Codex Sol xhigh **REQUEST_CHANGES 8회**(리비전 1~8) +
> **진행 승인 `APPROVE_FEATURE_PROGRESSION` 1회**(여덟 번째 리비전 재검토, Category A 0건) ·
> **M3d 완료 APPROVE 0회**. 이 진행 승인은 **M3d APPROVE가 아니며** 그렇게 축약해 적지 않는다 —
> 기존 완료 게이트는 전부 유효하다.
> 부하 acceptance 통과는 **M3d 완료의 차단 게이트**이며 비차단 잔여 위험이 아니다.
> 현행 offline 수치: exec **75/75** + core **374/374** + acceptance **71/71**(연속 3회 PASS),
> focused lock 테스트 **75/75**, liveEvidence **24/24**.
> **M4 구현은 not started — 별도 사용자 마일스톤 승인 전 착수 금지.** 배송 우선 원칙에 따라 **M4 계획 준비만**
> 가능하다(로드맵 §9.1 · M4 절).
>
> **2026-07-26 작업 방식 정책(전 마일스톤 적용, 상세는 로드맵 §9.1~§9.3):** 리뷰 finding은 **A(지금 차단) /
> B(지정 마일스톤·트리거 전 필수) / C(개선 backlog)** 로 분류하고 **C만으로는 리비전 루프·진행 정지가 없다.**
> 우선순위는 **유예 비용 대 수정 공수**로 정하며, 유예 항목은 심각도·확률·영향 반경·유예 비용·공수·기한/트리거·
> 담당·증거·상태를 유지하는 대장에 남긴다(**조용한 폐기 금지**). 테스트는 위험 비례(변경마다 focused ·
> handoff 전 전체 1회 · 반복/stress/live는 게이트에서만, 해당 계약 변경 시 예외)이되 **완화·삭제는 금지**다.
> 병렬 Claude Opus 5 세션은 **격리 worktree + disjoint 파일 소유권**일 때만 쓰고, 공유 schema/API·통합·상태
> 마이그레이션·최종 전체 테스트·배타 자원/stress/live는 **직렬**이며 **원격 쓰기는 계속 hard deny**다.
> MCP 도구 감사·profile 계약 작업에도 같은 분류·대장·병렬 규칙을 적용한다.
>
> **여덟 번째 리비전에서 확정된 계약**(이후 마일스톤에도 그대로 적용한다):
> ⑭ **모듈 지정자는 URL이다.** 소스 계약 감사에서 지정자를 문자열로 비교하지 않고 URL 문법 순서로 정규화한 뒤
> 비교한다(query·fragment 분리 → percent 디코드). 정규화 불가는 "대상 아님"이 아니라 **판정 불가 = fail closed**다.
> ⑮ **동적 로딩 route도 감사 대상이다.** 지정자 식을 bounded하게 접고(리터럴·치환 없는 template·`+` 연결·
> 정확히 한 번 선언된 `const`), 접히지 않으면 도달 가능한 문자열 조각으로 판정한다 — 조각 0개는 fail closed,
> 대상 token 포함은 문제, 그 밖은 `safe`. 이 `safe`는 **명시적으로 bounded한 트레이드오프**이며
> whole-program 증명이 아니다(정상 동적 로딩이 잡히지 않음을 실제 대조군으로 고정한다).
> ⑯ **재수출은 import-then-export까지 포함한다.** 별칭·default·namespace 파생 노출도 같은 도달 경로로 보고,
> 수집을 두 패스로 나눠 소스 순서로 우회할 수 없게 한다.
> ⑰ **scope는 보수적으로 fail closed 한다.** 전역(`process`)·추적 바인딩을 가릴 수 있는 선언이 하나라도 있으면
> 감사를 실패시키고, shadow된 식별자를 사용으로 인정하지 않으며, namespace import도 direct와 같은 검사를 받는다.
> ⑱ **부분 파싱된 소스를 안전의 근거로 쓰지 않는다**(파싱 진단 fail closed).
>
> **일곱 번째 리비전에서 확정된 계약**(이후 마일스톤에도 그대로 적용한다):
> ⑬ **"호출부 발견"은 구문 인식·재귀 감사로 한다.** 문자열 일치 + 한 겹 디렉터리 스캔은 중첩 경로 호출부,
> 식별자와 `(` 사이의 공백·주석, 별칭(`as`)·namespace import를 놓친다. 대상 트리를 모든 깊이까지 재귀 열거하고
> (symlink 파일·디렉터리는 production 소스로 신뢰하지 않고 따라가지 않는다), AST로 해당 모듈에서 온 바인딩을
> 추적해 호출 형태(인자 개수·첫 인자 정규형)를 검사하며, 미사용 바인딩·다중 호출·동적 로딩·재수출·비호출 참조도
> 문제로 보고한다. 그 감사의 비공허성은 **합성 우회 소스**로 증명한다(임시 파일·production 훼손 없이).
> 감사 도구는 테스트에서만 쓰고 production 의존성·주입 표면을 늘리지 않는다.
>
> **여섯 번째 리비전에서 확정된 계약**(이후 마일스톤에도 그대로 적용한다):
> ⑨ **신원 검사 대상과 파괴적 조작 대상은 같은 최종 엔트리여야 한다.** 상태 파일(lock/guard 등) 읽기는
> `O_RDONLY|O_NOFOLLOW`로만 열고, 최종 엔트리가 symlink면 대상이 정상 파일이어도 **거부**하며 엔트리·대상 모두
> 손대지 않는다. `O_NOFOLLOW` 미지원 플랫폼은 그 전이를 전부 거부한다(fail closed).
> ⑩ **성공 상태는 전이 완결 뒤에만 공표한다.** 자원 해제·격리 같은 종결 상태는 guard 반납까지 성공한 다음에만
> handle·요약 지표에 반영한다. 파일이 사라졌다는 사실만으로 "해제됨"이라 부르지 않으며, 소비자 요약도
> 미완결을 명시한다(거짓 성공 신호 금지).
> ⑪ **소스 계약 회귀는 "호출부 발견 + 인자 형태" 두 단계로 만든다.** 호출부 목록을 하드코딩하지 않고 스캔으로
> 발견해 기대 목록과 비교한 뒤 각 호출의 인자를 검사한다 — 새 호출부가 생기면 테스트가 먼저 깨진다.
> ⑫ **부하 acceptance 실패는 테스트 완화로 없애지 않는다.** 원인(고정 startup deadline, 호스트 부하)을 수치와 함께
> 기록하고 사용자 판단에 남긴다.
> 보안 계약 보강(전 마일스톤 적용): **외부** 테스트 주입 seam은 **argv `--fixture-config <절대경로 .json>` 하나**뿐이며
> production 코드는 lock 경로·`ps` fixture·pause/injection·evidence 디렉터리를 `process.env`에서 읽지 않는다
> (`HARNESS_LIVE_EVIDENCE_DIR` 폐기 — env는 자손에 암묵 상속되어 production 동작을 조용히 바꾼다).
> 임의 명령을 주입하는 테스트 seam은 만들지 않는다.
>
> **다섯 번째 리비전에서 확정된 계약**(이후 마일스톤에도 그대로 적용한다):
> ⑤ **파괴적 syscall(unlink/rename) 직전에 신원을 한 번 더 확인한다.** 확인과 syscall 사이에 동기화 지점이나
> 지연이 끼면 그 사이 교체가 가능하므로, 재확인을 통과하지 못하면 **남의 파일을 보존**하고 실패로 올린다.
> Node 18에는 `unlinkat`·compare-and-unlink가 없어 창을 0으로 만들 수 없다는 한계를 문서화하고,
> 창 최소화 + 사후 탐지 + fail closed로 대응한다.
> ⑥ **전이는 "완결"까지 성공이다.** 상태 전이 guard 반납 실패, 임시 파일 정리 실패, 읽기 fd close 실패 등
> guard 취득 **이후의 모든 I/O·cleanup 실패**는 mechanism 실패로 전파하며 **성공 handle을 돌려주지 않는다**.
> 정리 대상도 신원이 일치할 때만 지운다(신원 없으면 blind unlink 금지).
> ⑦ **token은 "우리 계열"만 증명하고 "이 파일"을 증명하지 못한다.** 재진입 성공 시점의 기본 record + (dev,ino)를
> 신뢰 기준으로 고정해 cleanup까지 명시 전달하고, 같은 token을 아는 행위자의 교체 파일은 인정하지 않는다.
> ⑧ **syscall 결과를 바꿔야 하는 테스트 경로는 in-process 함수 인자 seam으로만 연다.** production 진입점에서
> 도달 불가능하고 표면이 최소여야 하며(예: fs 함수 4개), 회귀 테스트로 호출부 인자 개수를 고정한다 —
> 위 "**외부** 주입은 argv 하나뿐" 계약과 충돌하지 않는다.
>
> **네 번째 리비전에서 확정된 파일 신원·주입 계약**(이후 마일스톤에도 그대로 적용한다):
> ① 상태 파일 발행은 "임시 파일 fd `fstat` → `link` → 최종 경로 `lstat` 신원 일치"까지 확인해야 성공이며,
> 신원 확인 실패·불일치 시 **최종 경로를 지우지 않고** 실패한다(우리 파일이라는 증거가 없으면 손대지 않는다).
> ② 전이 실패는 `refusal`(상태 변경 없음)과 `mechanism`(**기본값**, guard 유지)으로 **명시 분류**하고,
> 분류를 적지 않은 새 오류 경로는 자동으로 fail closed 쪽에 선다.
> ③ 주입 설정 파일은 경로를 **1회만** 열고(`O_RDONLY|O_NOFOLLOW`) 검사·읽기를 같은 fd에서 수행한다
> (검사–사용 경합 금지, 상한은 실제 읽은 바이트로 판정).
> ④ 소비자별 allowlist는 **자기가 해석하는 key만** 담는다 — 상위 프로세스는 하위에게 최소 설정만 명시 전달하고,
> 하위는 남의 권한 key를 해석하지 않는다(confused deputy 금지).
