...
> 계획은 M0~M3 당시 설계 기록이며, 후속 구현 순서로 사용하지 않는다.
>
> **M3d는 완료가 아니다.** M3d.2(live evidence 영속화 + 반복/부하 acceptance)는 **여덟 번째 리비전**까지
> 구현·offline 검증만 끝났고 **live runner 3종 실행과 evidence 3건은 pending, 부하(stress) acceptance도
> 미충족이며(여섯 번째 리비전 세션에서 FAIL — 호스트 외부 부하 + 고정 5초 child startup deadline 2건,
> lock 계층 계약 자체는 정상 · 일곱·여덟 번째 리비전 세션에서는 재실행하지 않았다), M3d 완료를 승인한 리뷰도 없다.**
> 리뷰 이력(2026-07-26 갱신): **REQUEST_CHANGES 8회**(리비전 1~8) + **진행 승인
> `APPROVE_FEATURE_PROGRESSION` 1회**(여덟 번째 리비전 재검토 · Category A 0건) · **M3d 완료 APPROVE 0회**.
> 이 진행 승인은 **M3d APPROVE가 아니고** 그렇게 축약해 적지 않는다 — 기존 완료 게이트는 전부 유효하다.
> **M4 구현은 not started**이며 별도 사용자 마일스톤 승인이 필요하다(계획 준비만 가능 — 로드맵 §9.1 · M4 절).
> 부하 acceptance 통과는 **M3d 완료의 차단 게이트**이며 비차단 잔여 위험이 아니다.
>
> **2026-07-26 작업 방식 정책(F1/F2/F3 후속 구현에도 적용, 상세는 로드맵 §9.1~§9.3):** 리뷰 finding은
> **A(지금 차단) / B(지정 마일스톤·트리거 전 필수) / C(개선 backlog)** 로 분류하고, **C만으로는 리비전 루프를
> 다시 돌리거나 기능 진행을 멈추지 않는다.** 우선순위는 **유예 비용 대 수정 공수**로 정하고, 유예 항목은
> 심각도·확률·영향 반경·유예 비용·공수·기한/트리거·담당·증거·상태를 유지하는 대장에 남긴다(조용한 폐기 금지).
> 테스트는 위험 비례이되 **완화·삭제 금지**이며, 병렬 Claude Opus 5 세션은 **격리 worktree + disjoint 소유권**
> 조건에서만 쓰고 통합·공유 schema 변경·최종 전체 테스트·배타 자원 테스트는 **직렬**이다.
> 현행 offline 수치: `npm test` 연속 3회 PASS = exec **75/75** + core **374/374** + acceptance **71/71**,
> focused lock 테스트 **75/75**, liveEvidence **24/24**.
> M3d.2 관련 **외부** 테스트 주입은 argv `--fixture-config` 하나뿐이며 production은 테스트용 env seam을 읽지 않는다
> (`HARNESS_LIVE_EVIDENCE_DIR` 폐기). 로더는 경로를 1회만 열어(`O_NOFOLLOW`) 검사·읽기를 같은 fd에서 하고,
> 소비자는 **자기가 해석하는 key만** 계약에 담는다(상위는 하위에게 최소 설정만 명시 전달).
> suite lock에는 stale/orphan 자동 회수가 없다(항상 거부 → 수동 제거). 상태 파일 발행은 최종 경로 신원(dev+ino)
> 일치를 확인해야 성공이고, 전이 실패는 기본적으로 guard를 남기는 fail closed다 — F2 진행 상태·F3 handoff 산출물을
> 디스크에 쓸 때도 같은 계약을 적용한다.
>
> **다섯 번째 리비전에서 더해진 계약**(디스크에 상태를 쓰는 F2/F3 산출물에도 동일 적용):
> ① **unlink/rename 직전에 신원을 한 번 더 확인**하고, 재확인을 통과하지 못하면 남의 파일을 **보존**한 채
> 실패로 올린다(Node 18에는 compare-and-unlink 원자 연산이 없어 창을 0으로 만들 수 없다 — 창 최소화 + 사후 탐지).
> ② **정리(cleanup)·close 실패도 실패다.** 임시 파일은 열자마자 확보한 (dev,ino)와 일치할 때만 지우고,
> 정리·반납·close 실패는 삼키지 않으며 **전이가 완결되지 않았으면 성공 handle을 돌려주지 않는다.**
> ③ **소유권은 비밀 token만으로 성립하지 않는다.** 재진입/재개 시점에 확인한 기본 record + (dev,ino)를 기준으로
> 보존해 이후 조작까지 명시 전달한다.
> ④ syscall 실패 경로를 테스트해야 하면 **production에서 도달 불가능한 in-process 함수 인자 seam**만 쓰고,
> 회귀 테스트로 production 호출부를 고정한다(외부 주입 표면은 늘리지 않는다).
>
> **여섯 번째 리비전에서 더해진 계약**(디스크에 상태를 쓰는 F2/F3 산출물에도 동일 적용):
> ⑤ **상태 파일 읽기도 `O_NOFOLLOW`다.** 최종 엔트리가 symlink면 대상이 정상 파일이어도 신원으로 인정하지 않고
> 거부하며 엔트리·대상 모두 손대지 않는다(그렇지 않으면 검사 대상과 unlink/rename 대상이 다른 파일이 된다).
> `O_NOFOLLOW` 미지원 플랫폼은 그 전이를 전부 거부한다.
> ⑥ **종결 상태(해제·격리·완료)는 전이가 완결된 뒤에만 공표한다.** 파일이 사라졌다는 사실만으로 성공이라 부르지
> 않으며, 완결 전 실패는 `failed` + 문제 보고 + 사람이 볼 잔재(guard)로 남긴다. 요약 지표도 거짓 성공을 내지 않는다.
> ⑦ **회귀는 호출부를 스캔으로 발견해 전수 검사한다**(하드코딩 목록 대신) — 새 호출부가 계약을 우회하지 못한다.
> ⑧ **부하 조건 acceptance가 실패하면 완화하지 않고 원인·수치를 기록해 사람 판단에 남긴다.**
>
> **일곱 번째 리비전에서 더해진 계약**(소스 계약 회귀 전반에 적용):
> ⑨ **"스캔으로 발견"은 구문 인식·재귀여야 한다.** 문자열 일치 + 한 겹 디렉터리 스캔은 중첩 경로·공백/주석이 낀
> 호출·별칭(`as`) import를 놓쳐 계약이 공허해진다. 대상 트리를 **모든 깊이까지** 재귀 열거하고
> (symlink 파일·디렉터리는 신뢰하지 않고 따라가지 않는다), AST로 **모듈에서 온 바인딩**을 추적해 호출 형태를
> 검사하며, import했지만 쓰지 않는 바인딩·다중 호출·동적 로딩·재수출·비호출 참조도 조용히 넘기지 않는다.
> 그리고 그 감사가 실제로 잡는지 **합성 우회 소스**(파일을 남기지 않고 production을 훼손하지 않는)로 증명한다.
>
> **여덟 번째 리비전에서 더해진 계약**(소스 계약 회귀 전반에 적용):
> ⑩ **모듈 지정자는 문자열이 아니라 URL로 다룬다.** query·fragment·percent 인코딩을 URL 문법 순서대로 정규화한 뒤
> 비교하고(Node ESM은 file URL을 디코드해 같은 파일로 해석한다), 정규화할 수 없으면 "대상이 아니다"라고 결론내지
> 않는다(판정 불가 = fail closed).
> ⑪ **동적 route는 bounded 규칙으로 판정하고 그 한계를 명시한다.** 지정자 식을 접을 수 있는 범위까지 접고,
> 접히지 않으면 도달 가능한 문자열 조각으로 판정한다 — 조각이 없으면 fail closed, 대상 token이 있으면 문제,
> 그 밖은 `safe`. `safe` 분기는 **정상 동적 로딩을 깨지 않기 위한 트레이드오프**이며 whole-program 증명이 아니라고
> 코드·문서·리뷰 보고에 함께 적는다. 정상 코드가 잡히지 않는지는 실제 대조군으로 고정한다.
> ⑫ **"재수출"은 형태가 아니라 결과로 본다.** 직접 `export … from`뿐 아니라 import-then-export·별칭·default·
> namespace 파생 노출까지 잡고, 수집을 **두 패스**로 나눠 소스 순서(hoisting)로 우회할 수 없게 한다.
> ⑬ **scope는 정확히 계산하는 대신 보수적으로 실패시킨다.** 전역·추적 대상 바인딩을 **가릴 수 있는 선언이 하나라도
> 있으면** 감사를 실패시키고, shadow된 식별자를 "사용"으로 인정하지 않는다.
> ⑭ **부분 파싱된 소스를 "안전"의 근거로 쓰지 않는다.** 파싱 진단이 있으면 fail closed로 보고한다.
