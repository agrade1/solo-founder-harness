...
4. **[v2.5 신기능 실전 검증 현황]**
   ...
   - critique_loop 편향분리(0-4, conclusion_only) → ⬜ 미검증. **mvp-planning**(critique_loop 포함) 실행 시 발동. (남음)
   - resume(0-1)/budget(0-2) → ⬜ 미검증. 실제 실패/상한 상황에서 확인 필요. (mock acceptance로는 검증됨)
...
- [ ] (버전 승격 원칙) v3 착수 전 하네스 자신에게 Red Team 워크플로우 1회 — Phase 2 착수 직전 수행 예정.
