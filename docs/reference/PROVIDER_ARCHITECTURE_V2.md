...
| `anthropic` ✅ | **A안** — Anthropic API 직접 (`@anthropic-ai/sdk`) | 종량 과금 | @anthropic-ai/sdk (설치됨) |

3종 모두 구현 완료. 프롬프트 구성은 `src/providers/promptParts.ts`(system+user)를 claude-code/anthropic이 공유한다 — claude-code는 합쳐서 stdin, anthropic은 system/user 분리.
