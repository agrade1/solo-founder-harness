...
실행 계층(execution layer) 아키텍처 설계 확정본 — v0.2.1

- **작성일**: 2026-07-08 (v0.1 → v0.2 → v0.2.1: 모델 정책 확정)
- **입력**: EXECUTION_LAYER_DESIGN_BRIEF.md §8의 질문 7개 + 창업자 결정 3건
- **상태**: 설계안 (구현 전). 승인 후 v3 스펙으로 승격
- **v0.2 개정 요지**:
...
호출 형태: `claude -p --output-format stream-json --permission-mode acceptEdits --allowedTools <컴파일 목록> --model <spec.model> --append-system-prompt <역할>`, 후속 지시는 `--resume <session_id>`. **구현 1번 작업 = 설치된 CLI 버전에서 플래그 실측** (기동 시 `claude --version` 체크 + 버전 핀 주석).
...
| L3 리뷰어 세션 | **Opus, 신선한 컨텍스트**(코더 세션과 대화 이력 미공유 — 기존 conclusion_only 편향 분리 원칙 재사용). diff + SPEC + API_CONTRACT만 입력 | critique_loop 패턴의 실행 계층 이식: Critical → 코더에 revise 주입, max_rounds=2, 초과 시 보류 목록행 |
...
### 6.2 rate limit 대응 (밤샘의 실질 제약)
무인 금지의 원 사유는 원칙이었지 토큰이 아니다 — 그러나 구독은 5시간 롤링 윈도우+주간 한도가 있어 밤샘 연속 실행은 중간에 한도 도달 가능성이 높다. 대응 2단: ① **강등 사다리**(§1.1) — `degrade_on_limit: auto`면 B→C→A로 자동 강등해 진행 유지(리뷰어 Opus는 고정, MISSION_REPORT에 강등 이력 기록) ② 강등으로도 막히면(주간 한도 등) **체크포인트 저장 → 윈도우 리셋까지 대기 → 자동 재개** 스케줄러 (기존 run_state/resume 재사용).
...
1. CLI 실측 스크립트 (stream-json/resume/permission 플래그 확인)
2. ExecutionProvider(CLI) + 이벤트 파서 + 단일 세션 수명
3. 권한 컴파일러(티어→플래그+훅) — PERMISSION_POLICY를 기계가 읽는 JSON으로 병행 정리
4. worktree 수명 + L1 기계 게이트 파이프라인
5. 대화형 게이트(stdinApprover→ApprovalQueue) + diff 미리보기 + tell  → **여기까지 v3 acceptance**
6. Opus 리뷰어 세션(L3) + revise 루프 (critique_loop 이식)
7. 미션 브리프 생성기 + 사전승인 패키지 + defer 보류 체계 + 모델 강등 사다리(degrade_on_limit)
8. develop 자동 병합·푸시 + rate limit 체크포인트/재개 + MISSION_REPORT  → **v3.5 acceptance**

## 10. 미결정/백로그
- **첫 미션 = 정책 B(전부 Opus)로 소모 프로파일 계측** → 강등 임계값(누적 대기 1h/2회) 기본값 보정. Opus 상시 운용 가능하면 C/A는 비상용으로만 유지
