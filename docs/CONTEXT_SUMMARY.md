...
- **`B-59` 신규**: `ownership`·`hookDenyPatterns`가 컴파일 후 소비자 0 — `exec`/`mission`에 **집행되는
  쓰기 경계가 없다**(v3 커널 경로는 무관, 거기선 실제로 집행한다). **앞선 등재에서 빠뜨린 것을 넣었다.**
  **정적으로만 확정** — 실제로 경계 밖에 쓸 수 있는지는 **안 돌려 봤다**.
- **실측**: typecheck 0 · **core 691/691 · exec 653/653 · acceptance 272/272** (전부 단독 실행).
...
- **다음**: `B-59`(쓰기 경계 — 실물 확인 먼저) · `C-154` 잔여 4종 · `C-155`(deps) · `B-47`·`B-48`·`B-51` ·
