...
## 최신 (2026-09-09 M16 종료 — 상세는 `docs/handoff/M16_SESSION_REPORT.md`)

- **세션 보고 정본**: `docs/handoff/M16_SESSION_REPORT.md` (다음 세션은 그것부터 읽는다).
- **main `28de769`** · typecheck 0 · core **718**/0 · exec **662**/0 · acceptance **272**/0 ·
  `npm test` 전체 체인 exit 0 · 대장 **125**(등급 A **0**).
- **사용자 실사용이 3건을 낳았다**: `C-162`(init이 작업 폴더 아이디어 문서를 인수) ·
  **`C-163`(내가 이 세션에 만든 결함 — 예산 소진이 리서치 실패로 둔갑해 틀린 원인을 안내)** ·
  `C-164`(확인 요청서 · live 실측 **66KB→5.8KB**).
- **live 4단계는 코드가 아니라 사람이 막고 있다**(`C-160`) — CEO가 3시간짜리 실물 테스트를 요구했고
  하네스는 그 증거를 만들 수 없다. 수행하지 않고 `## Decision`을 고치는 것은 검증 위조라 하지 않았다.
- **다음**: 확인 요청서 형식 피드백 대기 → MCP 서버(①-A). 리뷰 모델은 **`gpt-6-astra`**
  (codex CLI를 0.154.0-alpha.8로 올렸다).

---

## 이전 (2026-09-07 M16 — 머지 정상화 + 필수 3건 + 개선 A 2건 + 70k 바닥 가름 + live 계측)

- **머지 사고 하나를 발견하고 고쳤다.** 스택 #116~#122를 순서대로 머지했는데 `--delete-branch`를
  쓰지 않아 GitHub이 base를 재타겟하지 않았고, **#117~#122가 각자의 스택 base로 머지돼 main에는
  #116만 반영됐다.** tree 해시로 유실 없음을 확인한 뒤 superset 브랜치를 PR #123으로 올려 정상화했다.
  → **다음 세션 교훈: `--delete-branch`를 안 쓰면 재타겟도 안 된다. 머지 후 main의 tree를 스택
  tip과 대조하라.**
- **머지된 main에서 typecheck가 red였다**(3건). 회귀가 아니라 `8f21fdc`(A-4) 이후 줄곧 있던 것 —
  **그 브랜치에서 typecheck를 단독으로 재지 않았다는 뜻이다.** TS CFA가 nested function 안의 대입을
  추적하지 못해 `assert.ok` 뒤 타입이 `never`가 됐다(런타임은 정상).
- **`B-47` closed** — `reject`가 인쇄하는 재실행 명령이 `pipeline_artifact_drift`로 그 자리에서
  거부됐다(fixture 재현). 성공한 run이 `last_failure`를 null로 내려 "이 단계가 그 경로를 덮었다"는
  영수증이 사라지기 때문. **되돌림된 checkpoint의 artifacts**를 두 번째 근거로 쓰고, 소비자 셋을
  `stageWrittenBytes()` 하나로 모았다(사전 검증만 고치면 다음 실패가 영수증을 비워 되살아난다).
- **`B-48` closed** — 예산 검사를 step 경계에서 **모든 모델 호출 직전**(`runStepWithRegen`)으로 내렸다.
  재현: `--max-tokens 700`에 **1,200 소비**(+71%) → 수정 후 800(초과분 = 호출 하나).
  critique_loop뿐 아니라 fanout·게이트 점프·리서치 2차가 같은 문을 지난다.
- **`B-64` closed · `C-156` 반증** — 둘 다 "코드 대조만" 등재였고 **kernel fixture로 관측했다.**
  `B-64`는 맞았다(32 → 롤오버 1회에 **28**). `C-156`은 **틀렸다**: 경과 상한이 wall-clock이라
  누적기가 max인 것이 맞고 합계로 바꾸면 병렬 task의 wall time을 중복 계산한다. **코드가 아니라
  주장을 고쳤다.** 갈라져 나온 진짜 잔여는 `C-159`가 아니라 `C-158`(호출자가 넘기는 `elapsedMs`).
- **`C-151` closed** — `test:core`·`test:exec`·`acceptance`를 **직접 불러도** lock을 지나게 했다
  (`*:inner`로 분리 · `test:inner`는 token으로 재진입). 겹쳐 부르면 이제 **exit 2 + 거부 사유**이지
  거짓 실패 28건이 아니다(실측). spawn 슬랙 5s→20s(이 값을 재는 테스트는 전부 짧은 값을 명시 override).
- **`B-51` closed(프롬프트 층)** — 아이디어 문서가 게이트 심사 범위를 좁히던 경로를 **아이디어를
  싣는 그 한 자리**(`promptParts.ts`)에서 막았다. 범위 제약은 정당한 입력으로 통과시키고 결론·심사
  항목 지정만 막는다. **미증명: live 재현 실험은 하지 않았다** — 프롬프트에 실린다는 것만 관측했다.
- **`C-153` 가름 완료 — "영어화 1.46배 절감"은 과대주장이었다.** 빈 프롬프트 대조 2회:
  **36,148**(provider 자체) + **~4,016**(이 레포의 `CLAUDE.md`가 실린다) + **~29k**(하네스 몫).
  영어화가 닿는 것은 ⓒ뿐이라 상한이 호출당 입력의 **약 13%**다. 대신 **더 싼 레버**가 나왔다 →
  **`C-159` 신규**(provider를 `cwd` 없이 spawn해 하네스 개발 지침이 모든 agent 호출에 4k씩 실린다).
- **live 4단계 — 선행 조건이 `B-47`이 아니었다(`C-160` 신규).** `B-47`은 닫혔고 **mock 4단계는 완주한다**
  (설계 게이트 거부 후 resume 포함). 그런데 **live는 1단계 게이트를 못 넘는다**: `founder_ceo`가 '검증'을
  두 번 내고 되돌림 예산이 소진돼 `ceo_decision_verify`로 멈췄다(호출 16회 · 입력 1,582,482 · 출력 148,213).
  CEO가 요구한 것은 **3시간짜리 실물 테스트**라 하네스가 만들 수 없는 증거다 — **수행하지 않은 채
  `## Decision`을 '진행'으로 고치는 것은 검증 위조이므로 고치지 않았다.** 상세: `docs/handoff/M16_LIVE_MEASUREMENT.md`.
- **`'진행'` 판정 누적 0회.** 이 run은 P1보다 **구체적인 아이디어**였는데도 3라운드 연속 '검증'이었다 →
  가설 ⓐ(아이디어가 약했다)는 **약해졌다**(ⓑ와 아직 못 갈랐다).
- **`C-153` 보강**: "98%·편차 9.5%"는 **비평 루프·게이트 점프가 없는 run에 한정**된 수치다. 이 run은
  호출당 입력 **68,846~311,236(편차 352%)** · 입력:출력 **10.7:1**. 바닥(69k~83k)은 재확인됐지만
  "비용은 몇 번 부르는가의 함수"는 거짓이었다 — 호출 하나가 바닥의 4.5배(**원인 미규명**).
- **실측(main 기준 · 전부 단독 실행)**: typecheck 0 · core **705**/0 · exec **662**/0 · acceptance **272**/0.
  **`npm test` 전체 체인도 exit 0**(658 → 703 → 272) — `C-151`의 wrapper 중첩이 실제로 재진입한다는 증거다.
- **대장 124**(등급 A **0**). 이번 세션: `B-47`·`B-48`·`B-64`·`C-151`·`B-51` closed · `C-156` **반증** ·
  `C-155`·`C-154ⓐⓒ`·`B-54` closed · `C-158`·`C-159`·`C-160` 신규 · `C-153` 가름 완료 · `C-152` 선행 조건 갱신.
- **머지 후 이어서(#125)**: `C-155` closed(형태 오류를 조용히 버리던 파서가 `deps`의 **의미**를 바꿨다 —
  스칼라 `deps`가 '의존성 없음'이 되어 선행 완료 전에 실행했다. 순환 검사는 **일부러 안 넣었다** —
  스케줄러가 이미 dep_unmet으로 닫는다) · `C-154ⓐⓒ` closed(**승인 영수증이 심사받은 입력을 안 담았다** —
  승인 뒤 아이디어를 통째로 바꿔도 2단계가 돌았다. 결박했고, 그 부작용인 "아이디어 없으면 승인 대기로
  못 간다"를 결정으로 고정했다) · `B-54` closed(**형제 안내 하나를 더 찾았다** + 스냅샷 보관은
  근거 셋·뒤집는 조건과 함께 **"하지 않는다"로 결정**).
  `IDEA_REL`을 leaf로 옮겼다 — `pipeline.ts`가 값으로 필요한데 반대 방향이 런타임 순환이 된다.
- **`C-152`는 착수하지 않았다** — 선행 조건 `B-64`는 풀렸지만 **같은 세션의 `B-48`이 새로 걸렸다**
  (병렬화하면 하위 N개가 동시에 같은 잔량을 보고 전부 통과 → 초과분이 N개로 돌아간다). 예산 예약·
  순서 결정성·부분 실패 의미 넷을 대장에 적고 멈췄다.

---

## 이전 (2026-09-04 M15 — 죽어 있던 **design 계약을 살렸다**(`B-4`=`C-70`) + `C-157`)

- **`C-157` 먼저 닫았다** — a11y "공허함 방지"가 `semantic.color` 하나만 훑는데 프롬프트 §77이
  다크 모드를 `semantic.light`/`dark`로 분기하라고 **지시한다**. 그 지시를 따르면 검사가 **0회 돌고**
  AA 미달 텍스트가 `ok:true`로 통과했다(실측 대비 2.9). **계약이 지시하는 모양이 검사를 무력화**하고
  있었다. `semantic`의 **모든 하위 group**을 훑게 고쳤다. 깨진 계약을 먼저 고치고 배선했다.
- **`B-4`(=`C-70`) 배선** — `validateDesignArtifacts`는 **비-테스트 호출자 0개**였다(유일 호출자
  `buildDesignHandoff`도 0 — 체인 전체가 production에서 죽어 있었다). v1이 하던 검증은 **헤더 존재**뿐.
  **기존 재생성 루프에 합류**시켰다(별도 루프 0 · 새 상태 0): 미달이면 `persistFinalOutcome`이
  **채택도 저장도 하지 않는다**. 경계 고정: 색 하나를 AA 미달로 바꾸면 `failed`/`failed_agent: design` ·
  `DESIGN.md` 미생성인데, **같은 산출물이 옛 검증으로는 통과한다.**
- **배선하자 test provider들이 곧바로 red** — 그것이 계약이 한 번도 안 돌았다는 증거다.
  `mockProvider`의 인벤토리·focus 토큰을 계약에 맞추고, `progress.test.ts`의 scripted provider는
  design만 **mock에 위임**하게 했다(토큰 JSON 복사본을 만들지 않는다).
- **대장 서술 정정**: `C-70`이 근거로 적던 *"handoff·구현으로는 아직 쓰이지 않는다"* 는 **사실이 아니었다** —
  `task-prompt`가 구현자에게 그 문서를 읽고 따르라고 지시하고 있었다(`taskPrompt.ts:176-184`).
- **실측**: typecheck 0 · **core 697/697 · exec 656/656 · acceptance 272/272** (전부 단독 실행).
- **대장 125**(등급 A **0** · B 14 · C 108 + nonblocking 2).
- **다음**: `B-64`(프로세스 상한 attempt 리셋)·`C-156`(elapsedMsUsed) — **둘 다 코드 대조만 한 커널
  항목**이라 수정 전 kernel fixture 관측이 먼저다. 그 외 `B-47`·`B-48`·`B-51`·`B-54` 잔여분.

---

## 이전 (2026-09-04 M15 — 수색 보고서 **19건 전수 검증 완료** · 잔여 5건 수정)

- **수색 잔여 11건 전수 검증**(`C-150` closed): **8건 확인 · 3건은 이미 닫혀 있었다 · 반증 0.**
  재현 6건(`B-2`·`B-3`·`B-6`·`C-1`·`C-4` + `B-7`은 코드 대조) · 코드 대조 2건(`B-10`·`C-3`).
  **수색 워크플로 19건이 전부 맞았다** — 그것이 그 워크플로를 신뢰할 근거이지 다음 보고서를
  무검증으로 받을 근거는 아니다(같은 M15가 보고서에 없던 거짓 안내를 스스로 셋 만들었다).
- **수정 5건 closed**: `B-61`(상한 검사를 유료 호출 앞으로 — resume마다 크레딧을 사서 버렸다) ·
  `B-63`(`handoff`의 `run_state.json` 비원자적 write — `C-135`가 빠뜨린 네 번째 writer) ·
  `B-62`(손상을 "미실행"으로 접던 지역 리더 교체) · `B-65`(완료+drift에서 "거부된다"와 "생성"이
  같은 화면에 나오던 모순) · `B-60`(리서치 step 없는 단계가 앞 단계 영수증을 자기 것으로 증언).
- **가드 하나 추가**: `guidance.test.ts`가 `run_state.json`에 쓰는 **모든** 자리의 tmp+rename을 전수로 잰다.
  `B-63`은 "패턴을 복사하다 하나를 빠뜨린" 결함이라 그 자리만 고치면 다섯 번째를 못 막는다.
- **기각**: 영수증(`ResearchAttempt`)에 `workflow_id`를 넣는 안 — content-addressed라 모든
  `receipt_path`가 바뀌고 checkpoint 결박이 깨진다. run_state에 경계 index 하나만 뒀다.
- **실측**: typecheck 0 · **core 695/695 · exec 656/656 · acceptance 272/272** (전부 단독 실행).
- **대장 126**(등급 A **0** · B 14 · C 109 + nonblocking 2).
- **남는 셋**: `B-64`·`C-156`은 **코드 대조만 한 커널 항목** — 수정 전 kernel fixture 관측이 먼저다.
  `C-157`은 `B-4`(design 계약 production 호출자 0)와 함께 다뤄야 한다.

---

## 이전 (2026-09-04 M15 — **등급 A 잔량 0** · `B-56` 재현 후 closed · Codex 검수 반영)

- **`B-56` closed (등급 A · 재현하고 닫았다).** `M15_UNVERIFIED.md`가 적어 둔 재현 방법
  (스크립트 없는 임시 레포 + 중간에 죽는 provider stub · **live Claude 불필요**)이 그대로 통했다.
  **두 실패 모드(`exit_error` · `result.isError`) 모두 `merged`였고 부분 산출물이 develop에 병합됐다.**
  → `consumeTurn`이 실패 신호를 **반환**하고 호출자가 **게이트 전에** 멈춘다(`coder_failed`).
  **빈 게이트 통과는 건드리지 않았다** — 그건 계약이다(happy-path 테스트가 스크립트 없이 병합한다).
  대신 `GateResult.vacuous` + 미션 리포트가 *"게이트 체크 0개 — 검증되지 않았다"*를 병합 옆에 적는다.
  **순차·병렬 양쪽에 넣었다**(고치기 전에 grep했다 — `B-58`의 교훈).
- **`B-59` closed (등재 당일 재현).** 정적: `ownership`이 `settings.json` 어디에도 없고 `Edit`/`Write`가
  **경로 제약 없이** allow에 있으며 `permissionMode`는 `acceptEdits`. 동적: 담당을 선언한 세션이
  담당 밖 파일을 쓰고 **`merged`** 됐다. → `runSession`이 diff에서 담당 밖 변경을 찾아 **리뷰·승인·병합
  앞에서** 멈춘다(`ownership_violation` · revise 루프에도 같은 검사 · `ownership` 미지정이면 무영향).
  **부수 발견**: `parallelMission` fixture가 담당을 `a/**`로 선언해 놓고 루트에 쓰고 있었다 —
  **경계가 장식이었다는 증거**다. fixture를 계약에 맞추고 단정을 추가했다(완화 0).
- **`exec` exit code**도 고쳤다 — `coder_failed`·`ownership_violation`이 0으로 나가고 있었다.
  성공 집합의 **여집합**으로 적어 상태가 늘면 기본이 실패다.
- **실측**: typecheck 0 · **core 691/691 · exec 656/656 · acceptance 272/272** (전부 단독 실행).
- **대장 124**(등급 A **0** · B 13 · C 108 + nonblocking 2).
- **미검증은 `docs/handoff/M15_UNVERIFIED.md` 하나가 정본이다** — 등급(코드만 읽음/적대적 검증만/주장만)
  으로 나눠 적고, 이 세션이 **스스로 틀린 판정 5건**도 같은 무게로 남겼다.
- **다음**: `C-154` 잔여 4종 · `C-155`(deps) · `B-47`·`B-48`·`B-51` ·
  `B-54` 잔여분 · 수색 보고서 **11건 미재검**(`C-150`).

---

## 이전 (2026-09-03 M15 — **A급 넷 전부 closed** + **live 3단계 첫 완주**(`P1`) + `B-54` 축소)
