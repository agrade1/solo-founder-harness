...
## 최신 (2026-09-03 M15 — **A급 넷 전부 closed**(`A-1`~`A-4`) + live 계측 전 필수 둘 + `B-54` 축소)
...
- **다음 1순위**: **§3 `P1`** — 단독 `harness run dev-preflight --project <새 프로젝트> --allow-spawn`.
  게이트도 `founder_ceo`도 없어 날조할 사업 판정이 없고, 파이프라인을 안 건드린다. **`--yes` 필요**
  (`dev-preflight`에 내부 승인 게이트가 있어 비대화면 `approval_approver_missing`으로 시작조차 안 한다 — 실측).
