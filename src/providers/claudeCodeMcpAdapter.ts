...
function isTrustedLauncher(v: string): boolean {
  return (TRUSTED_LAUNCHER_IDS as readonly string[]).includes(v);
}

/** shadcn_read_proxy의 고정 스크립트 경로(override 불가). */
function trustedLauncherPath(): string {
 ...
 */
export function verifyTrustedProxyFile(scriptPath: string): void {
  let st;
  try {
  ...
  if (st.isSymbolicLink()) throw new McpConfigError("launcher_proxy_symlink", `신뢰된 proxy 경로가 symlink입니다(거부): ${scriptPath}`);
  if (!st.isFile()) throw new McpConfigError("launcher_proxy_not_file", `신뢰된 proxy 경로가 일반 파일이 아닙니다(디렉터리/특수): ${scriptPath}`);
  try {
...
function compileLauncherServer(name: string, decl: McpServerDecl, secretValues: string[]): StdioEntry {
 ...
 */
export function buildEmptyMcpConfig(): McpConfigResult {
  return { config: { mcpServers: {} }, expectedServers: [], expectedTools: [] };
}

/** buildEmptyMcpConfig 결과를 최소 권한(dir 0700 / file 0600)으로 기록한다. */
export function writeEmptyMcpConfig(runtimeDir: string): WrittenMcpConfig {
 ...
 */
function persistMcpConfig(built: McpConfigResult, runtimeDir: string): WrittenMcpConfig {
