...
const LAYERS = ["primitive", "semantic", "component"] as const;
type Layer = (typeof LAYERS)[number];
/** 접근성 선언 블록 key. */
const A11Y_KEY = "a11y";
/** 대비비 최소값으로 허용되는 값 — WCAG AA 본문 4.5 · 큰 텍스트/비텍스트 3. 임의 값(1 등)은 거부. */
const ALLOWED_MIN_RATIOS = [3, 4.5] as const;
/** focus 표시 토큰을 요구하는 대화형 컴포넌트(닫힌 목록). inventory 이름 기준. */
const INTERACTIVE_COMPONENTS = ["Button", "Input", "Select", "Checkbox", "Radio", "Switch", "Textarea", "Link", "Tab", "MenuItem"] as const;

/** 참조 형식 `{primitive.color.blue-500}`. */
const REF_RE = /^\{([a-z]+)\.([A-Za-z0-9_-]+)\.([A-Za-z0-9_-]+)\}$/;

/** 각 계층이 참조해도 되는 상위 계층(primitive는 참조 금지 = raw 값만). */
const ALLOWED_REF_LAYER: Record<Layer, Layer | null> = {
  primitive: null,
  semantic: "primitive",
  component: "semantic",
};
...
export interface DesignContractError {
  /** 안정 코드 — acceptance/mutation 판정이 문구가 아니라 이 코드에 붙는다. */
  code: string;
  /** 위반 위치(토큰 경로 또는 헤더 이름). 원문 값을 담지 않는다. */
  where: string;
  message: string;
}
...
export interface ContractResult {
  ok: boolean;
  errors: DesignContractError[];
}

function err(errors: DesignContractError[], code: string, where: string, message: string): void {
 ...
 */
export function validateTokens(raw: unknown): ContractResult {
  const errors: DesignContractError[] = [];
...
export function resolveToken(tokens: unknown, path: string): string | null {
  let cur = path;
  for (let hop = 0; hop < 3; hop++) {
    const parts = cur.split(".");
    if (parts.length !== 3 || !isPlainObject(tokens)) return null;
    const layer = (tokens as Record<string, unknown>)[parts[0]];
 ...
 */
export function parseInventory(designMd: string): { result: ContractResult; components: InventoryComponent[] } {
  const errors: DesignContractError[] = [];
  ...
  for (let i = idx + 1; i < lines.length; i++) {
    ...
    if (!/^[-*]\s/.test(line)) continue; // 산문 설명은 허용(계약은 bullet에만 걸린다)
  ...
  if (components.length === 0) err(errors, "inventory_empty", "컴포넌트 인벤토리", "유효한 컴포넌트 bullet이 0개다");
...
export function validateDesignHeaders(designMd: string): ContractResult {
  const errors: DesignContractError[] = [];
...
const PAIR_KEYS = ["fg", "bg", "min"] as const;
 ...
 */
export function validateA11y(tokens: unknown): ContractResult {
  const errors: DesignContractError[] = [];
  ...
  const a11y = tokens[A11Y_KEY] as Record<string, unknown>;
  ...
  const declaredFg = new Set<string>();
  pairs.forEach((p, i) => {
    ...
    const { fg, bg, min } = p as { fg: unknown; bg: unknown; min: unknown };
    if (typeof min !== "number" || !(ALLOWED_MIN_RATIOS as readonly number[]).includes(min)) {
      err(errors, "a11y_min_ratio", where, `min은 ${ALLOWED_MIN_RATIOS.join(" 또는 ")} 여야 한다`);
      return;
    }
    for (const [label, path] of [["fg", fg], ["bg", bg]] as [string, unknown][]) {
      if (typeof path !== "string" || !/^(semantic|component)\./.test(path)) {
        err(errors, "a11y_pair_path", `${where}.${label}`, "semantic/component 색 토큰 경로가 아니다");
        return;
      }
    }
    const fgHex = resolveToken(tokens, fg as string);
    const bgHex = resolveToken(tokens, bg as string);
  ...
  const semanticColor = isPlainObject(tokens.semantic) && isPlainObject((tokens.semantic as Record<string, unknown>).color)
    ? ((tokens.semantic as Record<string, unknown>).color as Record<string, unknown>)
    : {};
 ...
 */
export function validateFocusTokens(components: InventoryComponent[], tokens: unknown): ContractResult {
  const errors: DesignContractError[] = [];
  const groups = isPlainObject(tokens) && isPlainObject(tokens.component) ? (tokens.component as Record<string, unknown>) : {};
  for (const c of components) {
    if (!(INTERACTIVE_COMPONENTS as readonly string[]).includes(c.name)) continue;
 ...
 */
export function validateDesignArtifacts(designMd: string, tokens: unknown): ContractResult {
