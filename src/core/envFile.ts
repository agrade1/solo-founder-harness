...
function trackedByGit(root: string): boolean {
 ...
 */
function ignoredByGit(root: string): boolean {
...
export interface KeyResolution {
  ...
  refusedCode?: "env_file_tracked_by_git" | "env_file_not_ignored";
...
export function resolveResearchKey(opts: ResolveKeyOptions = {}): KeyResolution {
  ...
  if (insideWorkTree(root)) {
    ...
    if (!ignoredByGit(root)) {
      ...
      if (changed) notices.push(`.gitignore에 ${GITIGNORE_ENTRIES.join(" · ")} 규칙을 추가했습니다 (${join(root, ".gitignore")}).`);
      if (!ignoredByGit(root)) {
        return {
          envPath,
          key: null,
