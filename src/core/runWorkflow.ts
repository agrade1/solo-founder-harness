...
import {
  ...
  RESEARCH_DECLARATION_INSTRUCTION,
  RESEARCH_DIR_REL,
...
export interface GateJumpEntry {
  ...
  /**
   * [B-40/A-2] 이 게이트가 **실제로 한 일**. `jumped_to === null`을 "진행"으로 읽는 소비자가
   * 폐기와 실패(보류·unmapped·예산 소진·대상 부재)를 전부 "진행"으로 적고 있었다 —
   * durable status는 failed인데 영수증은 진행이라고 증언하는 모순. 결과를 추론하지 말고 여기서 읽어라.
   */
  outcome: "proceed" | "jump" | "kill" | "failed";
  /** outcome이 "failed"일 때의 안정 사유 코드 (run_state.failed_reason와 같은 값). */
  reason?: string;
  /**
   * [B-49] 게이트가 판정을 읽은 문서가 **이번 invocation에서 실행된 decider의 출력이 아니라**
   * 디스크에서 복원된 산출물일 때만 존재한다(=resume이 게이트 인덱스부터 재개해 decider를
   * 재실행하지 않은 경우).
   *
   * **판정 저자의 증명이 아니다** — 사람이 고친 문서 · 스크립트가 고친 문서 · 무편집 resume의
   * 원래 모델 출력을 구분하지 못한다. 증언하는 것은 "이번 invocation이 decider를 실행하지 않고
   * 디스크 바이트로 판정했다" 하나뿐이다. (resume은 started_at을 이어받아 같은 논리적 run이므로
   * "이번 run 미실행"이라고 읽으면 안 된다.)
   *
   * 그래도 남기는 이유: `cleared_idea_sha256`은 **아이디어 바이트**의 영수증이라 모델이 이번에
   * 판정했는지 여부를 담지 못한다. 사람이 "## Decision"을 고쳐 재개하는 복구 경로는 이미 계약인데
   * (아래 ceo_decision_absent 주석), 그 경로로 발급되는 해제가 무표시였다.
   */
...
export interface RunState {
  ...
  project: string;
  provider: string;
  status: "completed" | "failed" | "killed"; // 정상 완주 / 중단 / kill 게이트 폐기 판정
  completed_steps: string[];
  failed_agent: string | null;
  failed_reason: string | null; // 중단 사유 (실패 시)
  /**
   * kill 게이트가 run을 죽인 판정 (없으면 null). 폐기는 실패가 아니므로 failed_reason에 넣지 않는다 —
  ...
  /**
   * [C-126] 리서치 영수증. **additive optional**이다 — 구버전 run_state는 이 필드 없이 그대로 통과한다.
   *
   * `attempts`는 `kill_history`(:546)와 **같은 자리·같은 규율로 carry forward**한다: resume이든
   * 새 run이든 이전 state에서 이어받는다. 이어받지 않으면 mvp-planning의 run_state 하나가
...
export interface RunWorkflowArgs {
  ...
  /**
   * [B-40] workflows 파일 경로 override (기본: registry/tool_profiles.json 옆의 registry/workflows.json).
   * 실제 세 workflow는 kill 게이트가 모두 마지막 step이라 "게이트 뒤 step이 안 돈다"를 증명할 수 없다 —
  ...
  /**
   * [C-126/A-1] 이 run의 리서치 진행 방식. **`runWorkflow`는 키를 모른다** — 이 인자가 유일한 입력이고
   * 값은 `resolveResearchRuntime()`이 만든다(`runRun`과 파이프라인 `nextLocked` 양쪽이 부른다).
   *
   * 미지정 = `{kind:"self"}`와 같다(기존 테스트·exec 경로 · 외부 호출 0회). self에서도 attempt와
   * receipt는 남는다 — 승인자가 "이 문서가 어떤 리서치 모드에서 나왔나"를 승인 바이트 안에서
   * 보는 것이 §6.1의 요구다.
   */
 ...
 */
function lockFieldsProblem(s: RunState): string | null {
  const raw = s as unknown as Record<string, unknown>;
  const hist = raw.kill_history;
  if (hist !== undefined) {
    if (!Array.isArray(hist)) return "kill_history가 배열이 아니다";
    for (const [i, e] of hist.entries()) {
      if (typeof e !== "object" || e === null) return `kill_history[${i}]가 객체가 아니다`;
      if (!("idea_sha256" in (e as object))) return `kill_history[${i}]에 idea_sha256 키가 없다`;
    }
  }
...
export function readRunStateAt(abs: string): RunStateRead {
  ...
  let state: RunState;
  try {
    state = JSON.parse(readFileSync(abs, "utf8")) as RunState;
  } catch (err) {
 ...
 * "심사한 바이트"와 "기록한 digest"가 갈라지지 않게 한다(TOCTOU).
 */
export interface IdeaSnapshot {
  path: string;
  sha256: string | null; // 파일이 없으면 null
  text: string; // 없으면 빈 문자열
}

/** 아이디어 문서를 한 번 읽어 snapshot을 만든다. 파일이 없으면 sha256=null, text="". */
export function snapshotIdea(ideaAbs: string): IdeaSnapshot {
  if (!existsSync(ideaAbs)) return { path: ideaAbs, sha256: null, text: "" };
...
export function snapshotProjectIdea(project: string): IdeaSnapshot {
 ...
 */
export function ideaGateStatus(read: RunStateRead, idea: IdeaSnapshot, allowReevaluation = false): IdeaGateStatus {
 ...
 */
export function devSurfaceGateStatus(
  projectRoot: string,
): { ok: true } | { ok: false; code: "ceo_decision_not_proceed"; message: string } {
  const registry = loadAgentRegistry();
  const seenDocs = new Set<string>();
  for (const deciderId of gateDeciderIds()) {
    const rel = findAgent(registry, deciderId)?.default_output;
    if (rel === undefined || seenDocs.has(rel)) continue;
    seenDocs.add(rel);
    const abs = join(projectRoot, rel);
    if (!existsSync(abs)) continue; // 이 decider는 이 프로젝트에서 판정한 적이 없다
    const parsed = extractCeoDecision(readFileSync(abs, "utf8"));
    if ("token" in parsed && parsed.token === "진행") continue;
    const what =
      "token" in parsed
        ? `'${parsed.token}'입니다`
        : parsed.error === "absent"
 ...
 */
function devSurfaceMessage(deciderId: string, rel: string, what: string): string {
  const rerun = reevaluationWorkflowIds().join(" | ") || "(게이트가 있는 workflow 없음)";
  return (
    `${deciderId} 판정이 ${what} — **개발 표면(작업 지시문·DAG 초안)은 '진행' 판정에서만 열립니다.**\n` +
...
export async function runWorkflow(args: RunWorkflowArgs): Promise<RunWorkflowResult> {
  ...
  if (new Set(gateDeciders).size !== gateDeciders.length) {
    throw new Error(
      `gate_duplicate_decider: workflow '${workflowId}'에 같은 decider의 게이트가 2개 이상 있습니다 (${gateDeciders.join(", ")}) — ` +
  ...
  const isJump = (g: GateJumpEntry): boolean => {
    ...
    const outcome: GateJumpEntry["outcome"] | undefined = g.outcome;
    return outcome === "jump" || (outcome === undefined && g.jumped_to !== null);
  };
  const remainingJumps = (gate: GateDef): number =>
    Math.max(0, (gate.max_jumps ?? 0) - gate_jumps.filter((g) => isJump(g) && g.decider === gate.decider).length);
  ...
  const research: ResearchRuntime = args.research ?? { kind: "self", envPath: envFilePath(), notices: [] };
   ...
   */
  const budgetExceeded = (): number | null => {
  ...
  const budgetMessage = (spent: number, where: string): string =>
    `  ✗ 토큰 예산 초과: ${spent}/${maxTokens} — ${where} (모델 호출 없이 중단했습니다)\n` +
  ...
  async function runStepWithRegen(
    ...
    opts: {
      ...
      /** [C-126] 1차에 주입하는 검색 선언 지시 (키가 있을 때만). */
      researchRequest?: string;
      /** [C-126] 명시 digest — 리서치 2차 전용. 미지정이면 아래 수신자 allowlist 규칙이 적용된다. */
    ...
    if (overBudget !== null) throw new TokenBudgetExceeded(overBudget);
   ...
   * 그 호출의 사실이므로 남긴다.
   */
  function recordOutcomeTelemetry(agent: AgentDef, o: StepOutcome): void {
    if (o.sawUsage) {
   ...
   */
  async function runWebResearchStep(
    agent: AgentDef,
    i: number,
    nextAgentId: string | undefined,
    spawnRequest: string | undefined,
  ): Promise<{ ok: true; saved: string; elapsedMs: number; mode: string } | { ok: false; reason: string; detail?: string }> {
  ...
  let runStatus: RunState["status"] = "failed";
  try {
    ...
    try {
      ...
      if (isGate(step)) {
        ...
        if ("error" in parsed) {
          ...
          console.error(
            `  ✗ 게이트: ${decider} 출력의 "## Decision" 정본 판정을 읽을 수 없음 (${failed_reason}) — ` +
              `중단. 허용 토큰: ${CEO_DECISION_TOKENS.join(" | ")} 중 정확히 하나`,
          );
          endGate(false);
          break;
        }
        const decision: string = parsed.token;
        ...
        if (decision === "진행") {
          ...
          if ((kill ?? []).length > 0) cleared_idea_sha256 = idea.sha256;
          endGate(true);
          continue;
        }
        failed_agent = decider;
