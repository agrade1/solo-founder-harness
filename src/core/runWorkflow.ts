 ...
 */
export function ceoVerifyGateStatus(read: RunStateRead): { ok: true } | { ok: false; code: "ceo_decision_verify"; message: string } {
  if (read.kind !== "ok") return { ok: true }; // 부재·손상은 폐기 잠금(ideaGateStatus)이 판정한다 — 규칙을 두 벌로 만들지 않는다
  const s = read.state;
  if (s.status !== "failed" || s.failed_reason !== "ceo_decision_verify") return { ok: true };
  const doc = (s.failed_agent && findAgent(loadAgentRegistry(), s.failed_agent)?.default_output) || "(decider 산출 문서)";
  return {
    ok: false,
    code: "ceo_decision_verify",
    message:
      `CEO 판정이 '검증'이고 되돌림 예산이 소진됐습니다 (workflow '${s.workflow_id}') — ` +
