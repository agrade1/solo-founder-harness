...
import {
  DAG_DOCUMENT_KEYS,
  DAG_NODE_KEYS,
  DAG_NODE_OPTIONAL_KEYS,
  MAX_DAG_CONTRACT_PATHS,
  MAX_DAG_TASKS,
  TASK_DAG_SCHEMA_VERSION,
  validateTaskDag,
  type TaskDagNode,
} from "../exec/taskDag.js";
import { assignmentBodyFor } from "../exec/taskDagMaterialize.js";
import { createRunFromDocuments, readJsonDocument, type AutopilotCreateResult } from "./autopilotCreate.js";
...
export interface PlanDagCliOptions {
  workspace?: string;
  run: string;
  milestone: string;
  /** 승인 manifest 파일 경로(사람이 쓴다 — 이 명령은 필드를 만들지 않는다). */
  approval: string;
  /** 아이디어 문서 경로(예: `docs/00_IDEA.md`). 내용이 지시 본문에 그대로 실린다. */
  idea: string;
}
  ...
  try {
    ...
    if (size > LIMITS.maxBodyBytes) {
      throw new OrchestrationError(
        "text_too_long",
        `아이디어 문서가 ${LIMITS.maxBodyBytes}바이트 상한을 넘는다(${size}) — 자르지 않는다. 문서를 줄여라: ${path}`,
      );
    }
...
const inline = (items: readonly string[]): string => items.map((i) => `\`${i}\``).join(" · ");
 ...
 * 거부된다).
 */
export function dagContractBriefing(): string {
  const required = DAG_NODE_KEYS.filter((k) => !(DAG_NODE_OPTIONAL_KEYS as readonly string[]).includes(k));
  return [
    ...
    // **C-117 결정 ⓐ — 계획층에서 푼다.** 실측: 문서 전문 2개를 한 task의 `provides`에 담은 live task가
    // `worker_plan_absent`로 2/2 실패했고 같은 run의 1-file task는 성공했다(17,969B). transcript는 설계상
    // 미저장이라 원인은 가설로만 남는다(긴 문서 2개의 JSON escape·출력 길이가 계획 추출을 깨뜨린다) —
    // 그러나 **계획을 나누면 그 조건 자체가 성립하지 않는다**. 그래서 고치는 자리를 planner 지침으로 골랐다.
    //
    ...
    "- `provides`는 **task당 1개**로 하라 — 산출물이 여러 개인 단계는 task를 나눠 각각 1개씩 `provides`하고 " +
      "`dependsOn`/`consumes`로 이어라. (**검증기 규칙이 아니라 live 실행층 실측 제약이다**: 계약은 여러 개를 " +
      "허용하지만 live worker는 한 turn의 계획 JSON에 산출물 전문을 전부 담아야 하고, 2개 이상은 실측에서 " +
      "계획 추출이 깨졌다 — C-117.)",
...
function planDagNode(
  manifest: ReturnType<typeof validateApprovalManifest>,
  idea: string,
  ideaPath: string,
): { node: TaskDagNode; approvedOps: ApprovedOperation[] } {
  ...
  if (ownership === undefined || ownership.length === 0) {
    throw new OrchestrationError(
      ...
      `승인 manifest의 ownershipByTask에 "${PLAN_DAG_TASK_ID}"가 없다 — 초안을 쓸 소유 경로가 승인되지 않았다`,
    );
  }
  const approvedOps = manifest.operationAuthorityByTask[PLAN_DAG_TASK_ID] ?? [];
  const writes = approvedOps.filter((op): op is Extract<ApprovedOperation, { kind: "write_file" }> => op.kind === "write_file");
  if (writes.length === 0) {
    throw new OrchestrationError(
      "dag_materialize_seed_rejected",
      `승인 manifest의 operationAuthorityByTask["${PLAN_DAG_TASK_ID}"]에 write_file 권위가 없다 — 초안을 쓸 곳이 없다`,
    );
  }
  ...
  const node: TaskDagNode = {
    ...
    scope:
      "산출물은 아래 승인된 경로의 task DAG 문서 JSON **초안** 하나다. " +
...
export function runPlanDagCommand(opts: PlanDagCliOptions): void {
  let result: AutopilotCreateResult & { draftPaths: string[] };
  try {
  ...
  process.stdout.write(
    `[plan-dag] ${result.created ? "run 생성" : "기존 run 이어받기"}: ${result.runId}@${result.milestoneId} · ` +
      `planner task ${result.taskCount}건 · 초안 산출 경로 ${result.draftPaths.join(",")}\n`,
  );
  process.stdout.write(
    `[plan-dag] 다음: harness autopilot --run ${result.runId} --milestone ${result.milestoneId} --worker-backend <backend> ...\n`,
  );
  // **초안은 초안이다.** 검증은 사람이 자기 손으로 돌린다 — 여기서 자동으로 실행에 넘기는 통로는 없다.
  process.stdout.write(
    `[plan-dag] 그 다음: harness validate-dag ${result.draftPaths[0]} 로 초안을 판정하고, 사람이 고친 뒤 autopilot-create에 넘겨라\n`,
  );
}
 ...
 */
export function runValidateDagCommand(opts: { file: string }): void {
  let document: ReturnType<typeof validateTaskDag>;
  try {
  ...
  process.stdout.write(
    `[validate-dag] 통과: task ${document.tasks.length}건 — ${document.tasks.map((t) => `${t.taskId}(${t.roleId})`).join(" · ")}\n`,
  );
