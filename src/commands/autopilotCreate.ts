...
import { createOrchestrationRun, openOrchestrationRun, type OrchestrationKernel } from "../exec/orchestrationKernel.js";
...
export interface AutopilotCreateCliOptions {
  workspace?: string;
  run: string;
  milestone: string;
...
export interface AutopilotCreateResult {
  runId: string;
  milestoneId: string;
  /** run을 **이 호출이** 만들었는가. `false`면 기존 run을 이어받았다. */
  created: boolean;
  /** **이 호출이 실제로 만든** task(멱등 재호출이면 빈 배열이다 — 만들지 않은 것을 만들었다고 적지 않는다). */
  createdOrder: string[];
  /** 문서가 선언한 task 수(= run에 실재하는 수). */
  taskCount: number;
}
...
function openBoundRun(workspaceRoot: string, runId: string, milestoneId: string, rawManifest: unknown): OrchestrationKernel {
  ...
  if (state.milestoneId !== milestoneId) {
    throw new OrchestrationError(
      ...
      `이미 있는 run ${runId}은 milestone ${state.milestoneId}에 bind돼 있다(요청: ${milestoneId})`,
    );
  }
  ...
  if (JSON.stringify(validateApprovalManifest(rawManifest)) !== JSON.stringify(state.manifest)) {
    throw new OrchestrationError(
      "run_already_exists",
      `이미 있는 run ${runId}에 bind된 승인과 이 파일의 승인이 다르다 — 바뀐 승인은 새 run이다`,
    );
  }
  return kernel;
}
...
export function runAutopilotCreateCommand(opts: AutopilotCreateCliOptions): void {
  let result: AutopilotCreateResult;
  try {
  ...
  process.stdout.write(
    `[autopilot-create] ${result.created ? "run 생성" : "기존 run 이어받기"}: ${result.runId}@${result.milestoneId} · ` +
      `task ${result.taskCount}건 · 이번에 만든 task ${result.createdOrder.length}건${result.createdOrder.length === 0 ? "" : ` (${result.createdOrder.join(",")})`}\n`,
  );
  process.stdout.write(
    `[autopilot-create] 다음: harness autopilot --run ${result.runId} --milestone ${result.milestoneId} --plan-dir <계획 디렉터리>\n`,
  );
}
