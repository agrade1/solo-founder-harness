import type { ProgressReporter, RunEvent, StepKind } from "../core/progress.js";
...
const FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
const TICK_MS = 100;
const SPIN_KINDS: ReadonlySet<StepKind> = new Set<StepKind>(["agent", "critic", "revise", "spawn"]);
...
export function createProgressReporter(): ProgressReporter {
  ...
  return {
    emit(e: RunEvent): void {
      switch (e.type) {
        case "step_start":
          if (SPIN_KINDS.has(e.kind)) startSpinner(e.label ?? `[${e.index}/${e.total}] ${e.agentId}`);
          break;
        case "step_end":
          if (SPIN_KINDS.has(e.kind)) stopSpinner();
          break;
        case "note":
          note(e.message);
          break;
