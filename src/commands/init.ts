...
const IDEA_CANDIDATES = ["docs/00_IDEA.md", "00_IDEA.md", "docs/idea.md", "idea.md", "docs/IDEA.md", "IDEA.md"] as const;

/** 워크스페이스 루트에서 아이디어 문서를 찾는다. 없으면 null. */
export function findWorkspaceIdea(): { rel: string; abs: string; content: string } | null {
  for (const rel of IDEA_CANDIDATES) {
    ...
    if (content.trim().length === 0) continue; // 빈 파일은 템플릿보다 나을 것이 없다
...
export function runInit(name: string): void {
  ...
  if (adoptedFrom) templates[ideaFile] = adoptedFrom.content;
