...
export async function runRun(
  ...
  for (const line of researchOutcomeLines(state.research?.attempts, state.research?.carried_attempts ?? 0)) console.log(line);
    ...
    if (state.failed_reason === "gate_jump_budget_exhausted") {
      ...
      console.log(
        `  ↳ 되돌림 예산이 소진됐습니다 — 예산은 gate_jumps 영수증에서 파생하므로 --resume으로 되살아나지 않습니다.\n` +
          ...
          `       [A-2] **개발 표면(task-prompt·plan-dag)을 여는 것은 '진행' 하나뿐입니다** — '보류'·'폐기'는 결론이지만 개발은 열지 않습니다.\n` +
    ...
    if (state.failed_reason === "ceo_decision_verify") {
      ...
      console.log(
        `  ↳ 되돌림을 다 쓰고도 판정이 '검증'입니다 — **하네스가 아니라 사람이 확인할 차례입니다** (개발하지 않습니다).\n` +
          ...
          `    ② 확인 결과로 같은 문서의 "## Decision"을 **결론 판정**으로 고친 뒤 --resume —\n` +
          `       게이트가 그 문서를 다시 읽어 재판정합니다(모델 호출 0회 · 영수증에 "판정 출처: 복원 문서").\n` +
          `       [A-2] 결론은 셋이지만 **개발 표면(task-prompt·plan-dag)을 여는 것은 '진행' 하나뿐입니다** —\n` +
          `       '보류'는 백로그, '폐기'는 종료라서 둘 다 개발을 열지 않습니다(초판 안내는 '보류'를 우회로로 권했다).\n` +
