...
function commitAfterRun(
  ...
  if (result.state.status === "failed") {
    ...
    if (reason.startsWith("research_")) {
      ...
      console.error(
        (capExhausted ? `리서치 복구 경로 (ⓐ는 막혔습니다 — 남은 것은 ⓑ 하나):\n` : `리서치 복구 경로 두 개:\n`) +
          (capExhausted
...
function commitPending(
  ...
  let reportRel: string | null = null;
  try {
    const idx = DEFAULT_PIPELINE.findIndex((x) => x.id === pending.stage);
    writeFileSync(
      checkpointReportPath(root),
      buildCheckpointReport(root, {
        project,
        stage: pending.stage,
    ...
    console.warn(`  ⚠ 확인 요청서를 만들지 못했습니다 (${(err as Error).message}) — 원문은 그대로 있습니다.`);
  }
    ...
    if (problem) {
      return reject(
        ...
        `${problem}\n확인한 바이트가 아니므로 승인하지 않았습니다 (상태 불변).\n` +
          ...
          `  ⓑ 'harness pipeline reject ${pending.stage} --checkpoint ${pending.checkpoint_id} --project ${state.project}'로 되돌린 뒤 다시 실행한다 — 이 상태에서는 실제로 열려 있습니다(확인 대기라 pending이 있습니다).`,
        1,
      );
    }
