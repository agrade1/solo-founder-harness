...
export const REQUEST_DECISION_REQUEST_KEYS = ["kind", "question", "safeDefault"] as const;
...
/** 계획이 반드시 실려야 하는 **controller/kernel 소유** 실행 신원. worker가 고르는 값이 아니다. */
 ...
 */
export function readOwnData(raw: unknown): Record<string, unknown> | null {
  try {
    if (typeof raw !== "object" || raw === null || Array.isArray(raw) || isProxy(raw)) return null;
 ...
 */
export function readOwnArray(raw: unknown): unknown[] | null {
  try {
    if (!Array.isArray(raw) || isProxy(raw)) return null;
    if (Reflect.getPrototypeOf(raw) !== Array.prototype) return null;
    const n = (raw as unknown[]).length;
    if (typeof n !== "number" || !Number.isInteger(n) || n < 0) return null;
    if (Reflect.ownKeys(raw).length !== n + 1) return null; // 인덱스 n개 + "length"
    const out: unknown[] = [];
    for (let i = 0; i < n; i++) {
...
function closedArray(raw: unknown, what: string): unknown[] {
  const items = readOwnArray(raw);
  if (items === null) throw planInvalid(`${what}는 여분 property 없는 순수 데이터 배열이어야 한다`);
  return items;
}
...
function planRequest(raw: unknown, index: number): AgentRequest {
  ...
  if (isSameKeySet(keys, REQUEST_DECISION_REQUEST_KEYS)) {
    if (read.kind !== "request_decision") throw planInvalid(`${what}.kind가 key 집합과 맞지 않는다`);
    return Object.freeze({
 ...
 */
export function validateTypedExecutionPlan(raw: unknown, binding: unknown): TypedExecutionPlan {
  ...
  if (rawOps.length > MAX_PLAN_OPERATIONS) throw planInvalid(`plan.operations는 ${MAX_PLAN_OPERATIONS}건 이하여야 한다`);
