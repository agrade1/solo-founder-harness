 ...
 * - **발행은 열려 있다**: 신규 파일 생성은 M9 선결 2에서 **완전 개방**됐고(`O_CREAT|O_EXCL` 빈 파일 →
 *   부모 경로 재해석 검증 → inode 도달성 검증 → 고정한 fd 전용 쓰기) 기존 대상 교체는 M5d에서 열렸다.
 *   `already_applied`는 **디렉터리 fsync 뒤에만** 나온다.
 *
 *   **정정(V3 M11 판정 ⑥ · 2026-08-24)**: 이 항목은 한때 "**발행은 예방 불가라서 존재하지 않는다** —
 *   신규는 `write_publish_unsupported`, 교체는 `write_replace_unsupported`"라고 적었고 **M5d·M9 이후
 *   거짓이었다**(같은 파일의 `[M9]` 테스트들이 실제로 신규 발행을 한다 = 자기모순). 이 거짓 주석이
 *   오진의 원천이 됐다: M11 판정 ⑥의 첫 판이 live pause의 원인을 `B-16`이라고 잘못 적었다.
