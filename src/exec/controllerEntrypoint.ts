...
function resolveWithin(cwd: string, raw: string): string | null {
  if (raw.length === 0 || raw.includes("\0") || isAbsolute(raw)) return null;
  if (raw.split("/").some((seg) => seg === "." || seg === "..")) return null;
  const abs = resolve(cwd, raw);
  return abs === cwd || abs.startsWith(cwd + sep) ? abs : null;
}

function validatePlan(planPath: string): number {
  let size: number;
  try {
  ...
  if (size > MAX_PLAN_BYTES) return EXIT_FAIL;
  let raw: unknown;
  try {
  ...
  const b = raw as { runId?: unknown; taskId?: unknown; attemptId?: unknown; turnId?: unknown };
  try {
...
function runTests(projectPath: string): number {
  ...
  if (r.error !== undefined || r.status === null) return EXIT_FAIL;
...
export function runControllerAction(argv: readonly string[], cwd: string): number {
  if (argv.length !== 2) return EXIT_CONTRACT;
  const [action, rawPath] = argv;
  const abs = resolveWithin(cwd, rawPath);
  if (abs === null) return EXIT_CONTRACT;
  if (action === "validate-plan") return validatePlan(abs);
  if (action === "run-tests") return runTests(abs);
