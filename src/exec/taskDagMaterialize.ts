 ...
 */
function planOperationTemplate(approved: ApprovedOperation): Record<string, unknown> {
 ...
 */
function assignmentOperationsSection(taskId: string, approvedOps: readonly ApprovedOperation[]): string {
  ...
  try {
    ...
    throw new OrchestrationError(
      "dag_materialize_seed_rejected",
      `${taskId}의 지시에 실을 operation이 계획 계약 밖이다(${e instanceof OrchestrationError ? e.code : "unknown"})`,
    );
  }
  return (
 ...
 */
function assignmentBriefingSection(briefing: string): string {
 ...
 */
export function assignmentBodyFor(node: TaskDagNode, approvedOps: readonly ApprovedOperation[] = []): string {
  ...
  const seeds = ordered.map((node) => {
    ...
    const approvedOps = node.operations.map((authorityId) => {
      ...
      if (approved === null) {
        throw new OrchestrationError(
          "dag_materialize_seed_rejected",
          `${node.taskId}의 operations가 승인 밖이다(operationAuthorityByTask에 없다): ${authorityId}`,
        );
      }
      return approved;
    });
...
function assertResumableRun(kernel: OrchestrationKernel, document: TaskDagDocument): void {
  ...
  for (const task of existing) {
    ...
    const approvedOps = node.operations
      ...
      .filter((op): op is ApprovedOperation => op !== null);
