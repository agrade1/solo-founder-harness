...
import { runSession, type SessionStatus, type SessionOutcome } from "./sessionRunner.js";
import { autoApprove } from "./approvalQueue.js";
import { pickModel, nextStage, shouldDegrade, type DegradeStage, type DegradeThreshold, type Difficulty } from "./modelPolicy.js";
import type { ExecutionProvider, SessionEvent, SessionUsage } from "./types.js";
...
export interface MissionTask {
  ...
  inputs?: string[];
  dod?: string[];
  difficulty?: Difficulty; // 강등 단계 C에서 모델 선택에 사용
  deps?: string[]; // 선행 태스크 id (전부 merged여야 착수)
...
export interface MissionBrief {
  goal: string;
  tasks: MissionTask[];
  degradeOnLimit?: "auto" | "ask"; // 기본 auto
  preapproval?: { dependencies?: string[]; contractChanges?: boolean; tokenBudget?: number };
}
...
export interface MissionReport {
  goal: string;
  startedStage: DegradeStage;
  endedStage: DegradeStage;
  degradeHistory: DegradeEvent[];
  tasks: TaskResult[];
  merged: string[]; // 병합된 태스크 id
  deferred: string[]; // 보류/실패 태스크 id (사람이 아침에 결정)
  rateLimitWaits: { count: number; totalMs: number };
...
export interface RunMissionOpts {
  repoRoot: string;
  brief: MissionBrief;
  coderProvider: ExecutionProvider;
  reviewProvider: ExecutionProvider;
  baseBranch?: string; // 기본 develop
  startStage?: DegradeStage; // 기본 B
  threshold?: DegradeThreshold;
  reviewRounds?: number;
  now?: () => number; // 테스트 클럭 (기본 Date.now)
...
export async function runMission(opts: RunMissionOpts): Promise<MissionReport> {
  ...
  let stage: DegradeStage = opts.startStage ?? "B";
  const waits = { count: 0, totalMs: 0 };
  const degradeHistory: DegradeEvent[] = [];
  ...
  for (const task of opts.brief.tasks) {
    ...
    const outcome: SessionOutcome = await runSession({
  ...
  const totalUsage = tasks.reduce(
    (acc, t) => ({ inputTokens: acc.inputTokens + (t.usage?.inputTokens ?? 0), outputTokens: acc.outputTokens + (t.usage?.outputTokens ?? 0) }),
...
export function renderMissionReport(r: MissionReport): string {
  const L: string[] = [];
  ...
  if (r.degradeHistory.length) {
    L.push(`## 모델 강등 이력`);
    for (const d of r.degradeHistory) L.push(`- ${d.from} → ${d.to} (태스크 ${d.afterTask} 이후)`);
    L.push(``);
  }
  L.push(`## 태스크`);
