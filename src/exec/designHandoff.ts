...
import {
  ...
  type DesignContractError,
  type InventoryComponent,
} from "../core/designContract.js";
import { linkInventory, type LinkedComponent } from "../tools/registryInventory.js";

export class DesignHandoffError extends Error {
  readonly code: string;
  readonly errors: DesignContractError[];
  constructor(code: string, message: string, errors: DesignContractError[] = []) {
    super(message);
...
export function parseScreens(uxFlowMd: string): Array<{ id: string; name: string }> {
...
/** 사람 승인 레코드 — 무엇을 승인했는지(tokens digest)까지 포함해야 유효하다. */
export interface DesignApproval {
  /** kernel `decision` 메시지 id(또는 v1 design_gate 기록 id). */
  decisionId: string;
  /** 승인 시점 tokens.json digest. 현재 tokens와 다르면 승인 재사용으로 거부한다. */
  tokensSha256: string;
}

/** 구현 세션이 화면별로 받는 지시 — 컴포넌트는 이름 참조만이며 원문 코드를 싣지 않는다. */
...
export interface BuildHandoffInput {
  designMd: string;
  uxFlowMd: string;
  /** 이미 추출된 tokens 객체(추출은 `validate.ts:extractTokensJson`가 단일 출처). */
  tokens: unknown;
  /** filtered shadcn proxy가 돌려준 공식 `@shadcn/*` 목록. 비공식이 섞이면 던진다. */
  registryRefs?: string[];
  screens: HandoffScreen[];
  approval: DesignApproval;
}

/** 구현 세션에 전달되는 **닫힌 형태**. 디자인 원문·registry 원문은 담지 않는다(경로·digest만). */
...
export function tokensDigest(tokens: unknown): string {
 ...
 */
export function buildDesignHandoff(input: BuildHandoffInput): DesignHandoffContract {
  ...
  const inventory: InventoryComponent[] = parseInventory(input.designMd).components;
  ...
  const scopeErrors: DesignContractError[] = [];
