...
import type { OrchestrationKernel, RegisterArtifactInput, SubmitInput } from "./orchestrationKernel.js";
...
function frozenClone<T>(value: T, what: string): T {
  try {
...
export class StableController {
  ...
  constructor(private readonly opts: StableControllerOpts) {
    ...
    if (typeof opts.handoff !== "function") fail("controller_config_invalid", "opts.handoff는 함수여야 한다");
  ...
  private async runTask(taskId: string): Promise<TaskOutcome> {
    ...
    try {
      ...
      const h = sealHandoff(
        this.sealed.handoff({ task: frozenClone(task, "task"), inputs, manifest: this.approvedManifest() }),
      );
      ...
      if (handle) await provider.stop(handle, `controller_${outcome.marker}`).catch(() => undefined);
    }
  }
  ...
  private verifiedInputs(task: OrchestrationTask): readonly ArtifactPointer[] {
    ...
    const snapshot = frozenClone<readonly ArtifactPointer[]>(inputs, "의존 포인터");
  ...
  private verifyPointers(refs: readonly ArtifactPointer[]): void {
    for (const ref of refs) verifyArtifactFile(this.sealed.kernel.workspaceRoot, ref.path, ref.sha256);
  }

  /** 승인된 커밋·checkout 신원·만료를 확인한다. **반환된 `targetRoot`가 유일한 cwd 근거다.** */
  private async verifyBoundary(cwd: string): Promise<VerifiedExecutionBoundary> {
   ...
   */
  private syncGate(boundary: VerifiedExecutionBoundary, pointers: readonly ArtifactPointer[]): void {
   ...
   */
  private consumeTurn(handle: SessionHandle): Promise<Extract<SessionEvent, { kind: "result" }>> {
    return consumeExactlyOneTerminal(
      this.sealed.provider.events(handle),
      {
...
function captureKernel(kernel: OrchestrationKernel): CapturedKernel {
  if (kernel === null || typeof kernel !== "object") fail("controller_config_invalid", "opts.kernel이 kernel이 아니다");
  for (const m of KERNEL_METHODS) {
    if (typeof (kernel as unknown as Record<string, unknown>)[m] !== "function") {
...
function captureProvider(provider: ExecutionProvider): CapturedProvider {
  for (const m of PROVIDER_METHODS) {
    if (typeof (provider as unknown as Record<string, unknown>)[m] !== "function") {
...
function buildPins(get: () => StableControllerOpts, kernel: CapturedKernel): ReadonlyArray<Pin> {
  const pin = (what: string, read: () => unknown): Pin => ({ what, read, pinned: read() });
  const pins: Pin[] = [
  ...
  for (const m of PROVIDER_METHODS) pins.push(pin(`provider.${m}`, () => (get().provider as unknown as Record<string, unknown>)[m]));
  for (const m of KERNEL_METHODS) pins.push(pin(`kernel.${m}`, () => (get().kernel as unknown as Record<string, unknown>)[m]));
