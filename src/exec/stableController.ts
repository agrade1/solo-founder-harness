...
/** 정책 입력 — handoff가 "이 turn이 무엇을 필요로 하는가"를 **명시**한다(추측 금지). */
export interface ExecutionRequest {
  /** 정확히 승인된 명령 문자열만(`manifest.allowedCommands` 동치 비교). */
  commands?: string[];
  /** 정확히 pin된 dependency만. */
  dependencies?: Array<{ name: string; version: string }>;
  /** 정확히 승인된 도메인만(하위 도메인 자동 허용 없음). */
  networkDomains?: string[];
  /** 이 turn이 쓸 수 있어야 하는 workspace-relative 경로(⊆ task.ownership ⊆ writableRoots). */
  writePaths?: string[];
  /** 로컬 merge를 요구하는가(`manifest.localMergeAllowed`가 true여도 원격 쓰기는 여전히 hard deny). */
  localMerge?: boolean;
  /** MCP 패키지 지정자(`pkg@1.2.3`). `@latest`는 hard deny다. */
  mcpPackages?: string[];
  /** handoff가 스스로 선언한 의도. hard deny 의도가 하나라도 있으면 거부다. */
  intents?: string[];
}

/** 컴파일된 단일 결정. 통과한 값만 담긴다(정규화된 사본 — 이후 변조는 다음 컴파일에서 걸린다). */
...
export interface HandoffContext {
  task: OrchestrationTask;
  /** 의존 task가 낸 **검증된** artifact 포인터(controller가 넘기기 직전에 다시 확인한 것). */
  inputs: ArtifactPointer[];
  manifest: MilestoneApprovalManifest;
}

export interface ControllerHandoff {
  /** provider 세션 명세. `cwd`는 실행 경계가 checkout 신원·HEAD로 검증한다. */
  spec: SessionSpec;
  /** 착수 프롬프트. **durable state에 저장되지 않는다.** */
  prompt: string;
  /** 이 turn이 요구하는 실행 권한(정책 입력). */
  request?: ExecutionRequest;
  /** 완료 시 artifact로 등록할 산출물(등록·durable 직전에 각각 재검증된다). */
...
export interface TaskOutcome {
  ...
  /** 등록·검증된 artifactId. */
  artifacts: string[];
  /** bounded usage 카운터(durable state에는 들어가지 않는다). */
...
interface SealedBinding {
  ...
  /** 검증·정규화된 승인 사본. 실행 경계에는 **이 값만** 넘긴다. */
  manifest: MilestoneApprovalManifest;
  startedAtMs: number;
}
...
export interface StableControllerOpts {
  ...
  /** 시각 권위. `start` 시점에 봉인되고 만료·경과 검사마다 다시 호출된다(교체하면 드리프트다). */
  nowMs?: () => number;
}

export class StableController {
  private readonly sealed: SealedBinding;
  private tokensUsed = 0;

  constructor(private readonly opts: StableControllerOpts) {
  ...
  async advanceOnce(): Promise<AdvanceOutcome> {
    ...
    for (const task of started) tasks.push(await this.runTask(task.taskId));
  ...
  private preflight(): string | null {
    ...
    if (!Number.isFinite(now)) return "controller_clock_unreadable";
    const expiresAtMs = Date.parse(this.sealed.manifest.expiresAt);
    if (!Number.isFinite(expiresAtMs) || now >= expiresAtMs) return "manifest_expired";
    if (now - this.sealed.startedAtMs >= this.sealed.manifest.maxElapsedMs) return "budget_elapsed_exhausted";
   ...
   */
  private assertNoBindingDrift(): void {
    ...
    for (const k of SEALED_KEYS) {
      if (now[k] !== this.sealed[k]) fail("controller_binding_drift", `봉인된 실행 신원이 바뀌었다: ${k}`);
    }
  }

  private async runTask(taskId: string): Promise<TaskOutcome> {
    ...
    try {
      ...
      if (!h || typeof h.prompt !== "string" || !h.spec) fail("handoff_invalid", "handoff가 spec·prompt를 주지 않았다");
      ...
      if (handle) await this.opts.provider.stop(handle, `controller_${outcome.marker}`).catch(() => undefined);
    }
  }
  ...
  private verifiedInputs(task: OrchestrationTask): ArtifactPointer[] {
    const inputs: ArtifactPointer[] = [];
    for (const depId of task.dependsOn) {
      const dep = this.opts.kernel.getTask(depId);
      if (!dep) fail("unknown_task", `미상 의존 task: ${depId}`);
  ...
  private verifyPointers(refs: readonly ArtifactPointer[]): void {
    for (const ref of refs) verifyArtifactFile(this.opts.kernel.paths.workspaceRoot, ref.path, ref.sha256);
  }

  /** 승인된 커밋·checkout 신원·만료를 확인하고 **직전 동기 재검증**까지 지난다. */
  private async assertBoundary(cwd: string): Promise<void> {
   ...
   */
  private async consumeTurn(handle: SessionHandle): Promise<Extract<SessionEvent, { kind: "result" }>> {
    let result: Extract<SessionEvent, { kind: "result" }> | null = null;
    let iterated = 0;
    try {
      for await (const e of this.opts.provider.events(handle)) {
        iterated++;
        if (iterated > MAX_TURN_EVENTS) fail("provider_stream_unbounded", "provider 스트림이 상한을 넘었다");
    ...
    if (!result) fail("provider_no_result", "이 turn에 종료 결과가 없다");
    if (result.isError) fail("provider_result_error", `provider turn이 실패로 끝났다(${result.terminalReason ?? "unknown"})`);
    return result;
  }

  private applyTurn(outcome: TaskOutcome, result: Extract<SessionEvent, { kind: "result" }>): void {
    outcome.turns += 1;
   ...
   */
  private boundedSummary(outcome: TaskOutcome): string {
  ...
  private resultEnvelope(task: OrchestrationTask, pointers: ArtifactPointer[]): AgentMessageEnvelope {
    return {
...
function resultBody(taskId: string, outcome: TaskOutcome, pointers: ArtifactPointer[]): string {
