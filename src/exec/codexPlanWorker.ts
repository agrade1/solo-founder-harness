 ...
 */
export function codexWorkerArgs(cwd: string): readonly string[] {
...
export interface CodexWorkerLaunch {
  /** 승인·digest 재검증을 이미 통과한 codex 실행 파일(kernel이 turn마다 다시 본다). */
  executable: string;
  /** 승인·계약 검증을 이미 통과한 격리 홈. 자식 env는 이 값 하나다. */
  codexHome: string;
  /** 리뷰 대상 checkout(= workspace 루트). */
  cwd: string;
  prompt: string;
...
function workerError(code: (typeof CODEX_WORKER_CODES)[number], what: string): OrchestrationError {
 ...
 */
export function startCodexPlanTurn(launch: CodexWorkerLaunch): WorkerStream {
  ...
  const run = async (): Promise<void> => {
    ...
    child.stdout?.on("data", (chunk: string) => {
      ...
      for (const e of parser.push(chunk)) {
        if (e.kind === "assistant" && e.text.length > 0) lastMessage = e.text;
      }
    });
    ...
    for (const e of parser.finish({ code: 0, signal: null })) {
      if (e.kind === "result") usage = { inputTokens: e.usage.inputTokens, outputTokens: e.usage.outputTokens };
      else if (e.kind === "assistant" && e.text.length > 0) lastMessage = e.text;
    }

    if (killedBy === "cancel") throw workerError("worker_cancelled", "취소로 세션을 종료했다");
    if (killedBy === "deadline") throw workerError("worker_deadline_exceeded", "세션 상한을 넘겨 종료했다");
    if (lastMessage.length === 0) throw workerError("worker_no_output", "모델 메시지가 없다");

    const json = extractPlanJson(lastMessage);
    if (json === null) throw workerError("worker_plan_missing", "응답에서 계획 JSON을 찾지 못했다");
    let parsed: unknown;
    try {
  ...
  return {
    async *[Symbol.asyncIterator]() {
      yield { kind: "started", seq: 1 } as WorkerEvent;
      await run();
