...
import type { AgentMessageEnvelope, AgentMessageType, OrchestrationTask } from "./orchestrationTypes.js";
import type { AgentRequest } from "./autopilotTypes.js";
import type { OrchestrationKernel } from "./orchestrationKernel.js";

/** 요청 1건의 처리 결과 — 성공이면 `code`가 없다. */
export interface AgentRequestOutcome {
  kind: AgentRequest["kind"];
  /** spawn이면 child taskId, 전달이면 대상 지정 문자열(감사용 라벨). */
  target: string;
  /** kernel 거부 코드. 성공이면 `null`. */
  code: string | null;
}

export interface ApplyAgentRequestsInput {
  kernel: OrchestrationKernel;
  /** 요청을 낸 task. `taskId`만 쓰고 나머지는 durable state에서 다시 읽는다. */
  taskId: string;
  turnId: string;
  requests: readonly AgentRequest[];
  /** messageId 발급기(호출자의 id 생성기를 그대로 쓴다 — 이 모듈은 난수를 만들지 않는다). */
 ...
 */
export function applyAgentRequests(input: ApplyAgentRequestsInput): AgentRequestOutcome[] {
  const outcomes: AgentRequestOutcome[] = [];
  for (const req of input.requests) {
    const outcome = applyOne(input, req);
    outcomes.push(outcome);
    if (outcome.code !== null) break;
  }
  return outcomes;
}

/** 이 kind만 골라 적용한다(호출 시점이 kernel 상태 계약에 묶여 있으므로 호출자가 나눠 부른다). */
export function requestsOfKind<K extends AgentRequest["kind"]>(
  requests: readonly AgentRequest[],
  kind: K,
): Array<Extract<AgentRequest, { kind: K }>> {
  return requests.filter((r): r is Extract<AgentRequest, { kind: K }> => r.kind === kind);
}

/** 감사 라벨 — 요청 갈래별 대상 표기. */
function requestTarget(req: AgentRequest): string {
  return req.kind === "spawn_child" ? req.childTaskId : req.kind === "deliver_status" ? req.deliverTo : ORCHESTRATOR_ID;
}

function applyOne(input: ApplyAgentRequestsInput, req: AgentRequest): AgentRequestOutcome {
...
function renderBody(type: AgentMessageType, filled: Record<string, string>): string {
  return REQUIRED_BODY_HEADINGS[type].map((h) => `## ${h}\n\n${filled[h] ?? "- (없음)"}`).join("\n\n");
}

function spawnBody(req: Extract<AgentRequest, { kind: "spawn_child" }>): string {
...
function assignmentBody(req: Extract<AgentRequest, { kind: "spawn_child" }>): string {
...
function decisionRequestBody(req: Extract<AgentRequest, { kind: "request_decision" }>, senderTaskId: string): string {
...
function statusBody(req: Extract<AgentRequest, { kind: "deliver_status" }>, senderTaskId: string): string {
