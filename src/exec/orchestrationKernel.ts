...
import type { TypedGitWorktreeOperation, TypedRunProcessOperation, TypedWriteFileOperation } from "./typedPlan.js";
...
export interface TaskSeed {
  ...
  /**
   * **이 task의 지시가 실은 승인 operation authorityId 집합**(V3 M11 — `B-38`/`C-111`).
   * 생략·`null`이면 "지시가 operation 축을 선언하지 않았다"이고 그때는 manifest 게이트 하나만 적용된다.
   * 배열이면 turn의 계획은 그 집합 **안**에서만 operation을 낼 수 있다(`issueOperationDispatchPermit`).
   * **권위를 만들지 않는다** — 여기 적힌 id도 `operationAuthorityByTask`에 없으면 집행되지 않는다.
   */
...
export interface DispatchAuthority {
  ...
  /**
   * **지금 자원을 점유 중인 다른 task들의 ownership 합집합**(V3 M9 T3 — 대장 `B-29`).
   *
   * `ownership`만 보면 같은 경로를 소유한 두 task가 동시에 running일 때 **둘 다 통과해 조용히
   * 덮어쓴다**(로드맵 M9 위험 1). `taskDag.ts`의 `ownership_conflict`는 Tech Lead 문서를 막지만
   * `requestSpawn`으로 만든 child는 그 문서를 지나지 않으므로 런타임 게이트가 함께 있어야 한다.
   *
   * 자기 자신은 빠져 있고, `waiting_children`·terminal task도 빠져 있다(점유하지 않으므로 동시
   * 쓰기가 성립하지 않는다). **dispatch 시점의 durable state에서 새로 읽는다** — 스냅샷이 아니다.
   */
  readonly concurrentOwnership: readonly string[];
 ...
 */
function taskWorktreePath(workspaceRoot: string, runId: string, taskId: string): string {
 ...
 */
function gitWorktreeArgs(action: GitWorktreeAction, path: string, approvedCommit: string): readonly string[] {
  switch (action) {
    case "add":
...
/** `git_worktree` 권능 뒤의 실행 명세(모듈 사설). `LaunchRecord`와 같은 규율이다. */
...
const GENUINE_WORKTREE_CAPABILITIES = new WeakMap<object, WorktreeRecord>();

/** `run_process`와 같은 모양의 봉인 권능(담긴 값은 감사용이고 권위는 등록부에서만 나온다). */
 ...
 */
export function resolveWorktreeCapability(op: TypedGitWorktreeOperation, handle: unknown): WorktreeCapability {
  const { authority: auth, permitRecord } = dispatchFrom(handle, op);
  const approved = resolveApprovedOperation(op, auth);
  if (approved.kind !== "git_worktree") throw operationDenied("승인 레코드의 kind와 다르다");
  const git = auth.manifest.executionAuthority.git;
  const capability: WorktreeCapability = Object.freeze({
    operationId: op.operationId,
    authorityId: op.authorityId,
    runId: auth.runId,
    taskId: auth.taskId,
...
export function isGenuineWorktreeCapability(v: unknown): boolean {
 ...
 */
export async function executeWorktreeOperation(
  grant: unknown,
  op: TypedGitWorktreeOperation,
  capability: unknown,
  options: { signal?: AbortSignal } = {},
): Promise<Readonly<OperationOutcome>> {
  // ① 권능 게이트가 가장 먼저다(재생 시도가 grant 상태에 가려지지 않는다).
  const rec0 =
    typeof capability === "object" && capability !== null ? GENUINE_WORKTREE_CAPABILITIES.get(capability) : undefined;
  if (rec0 === undefined) throw processDenied("process_capability_invalid", "worktree 권능이 kernel 발급 값이 아니다");
  if (rec0.spent) throw processDenied("process_capability_spent", "이미 소비된 worktree 권능이다(재생 금지)");
  ...
  const cap = capability as WorktreeCapability;
  if (
    rec0.issuer !== rec.issuer ||
  ...
  if (!lstatOrNull(joinPath(authority.workspaceRoot, ".git"))?.isDirectory()) {
    throw processDenied(
      ...
      "승인된 저장소 루트가 주 checkout이 아니다(linked worktree에서는 worktree를 만들지 않는다 — 변경이 승인 루트 밖에 남는다)",
    );
  }
  const policy = authority.manifest.autopilotPolicy;
  const args = gitWorktreeArgs(
    approved.action,
    taskWorktreePath(authority.workspaceRoot, authority.runId, authority.taskId),
    authority.manifest.approvedCommit,
  );
  ...
  const supervised = await superviseProcess({
    ...
    //
    // **읽기 질의의 30초 상수를 쓰지 않는다**(대장 `B-31`). `TRUSTED_GIT_TIMEOUT_MS`는 쓰기가 0인
    // 질의용 값이라 잘려도 저장소에 아무것도 남기지 않지만, `worktree add`는 **tree 전체 checkout**이라
    // 작업량이 repo 크기에 비례하고 중간에 잘리면 잔재가 남는다. 그리고 그 잔재 중 일부는
    // **닫힌 action 집합으로 되돌릴 수 없는 모양**이다(실측: `worktree.test.ts`의 `[B-31]` 두 테스트).
    // 그래서 사람이 이미 승인한 상한을 쓴다 — `maxAttemptElapsedMs`는 `approvalManifest.ts`가
    // `run_process.timeoutMs`에 강제하는 바로 그 상한이므로 **operation 1건에 이 승인이 허용한 최대치**다.
    //
    // 기각한 대안 둘:
    //  ⓐ `gitWorktreeAuthority`에 `timeoutMs` 필드 추가 — 상한은 이미 승인 문서 안에 있고, 새 필드는
    //    "경로·브랜치·커밋·timeout을 담을 필드가 하나도 없다"는 이 갈래의 성질만 깎는다. 더 촘촘한 눈금이
    //    실제로 필요해지면 그때 **더하는** 변경이다.
    //  ⓑ repo 크기 비례 상한 — 크기를 재려면 닫힌 질의 allow-list에 질의를 하나 더 열어야 한다.
    //    얻는 것은 추정치이고 여는 것은 보안 축이다.
    //
    // 남는 위험: 이 값은 attempt 예산 전체이므로 git이 매달리면 그만큼 슬롯을 쥔다(loop는 in-flight
    // operation을 wall deadline으로 끊지 않는다 — `options.signal`만이 끊는다). 같은 위험을
    // `run_process`가 이미 같은 상한으로 지고 있으므로 **새로 생기는 종류는 아니다**.
    timeoutMs: policy.maxAttemptElapsedMs,
  ...
  if (supervised.exitCode !== 0) {
    throw processDenied(
      ...
      `git worktree가 0이 아닌 종료 코드를 냈다(성공으로 접지 않는다): ${supervised.exitCode ?? "null"}`,
    );
  }
...
/**
 * **읽기 질의 1건의** wall deadline. 승인 정책이 고를 값이 아니다.
 *
 * 근거를 고쳐 적는다(대장 `B-31`): 이전 판은 "질의는 상수 작업량이다"라고 했는데 그것은 사실이 아니다 —
 * `diff --quiet HEAD`는 worktree 크기에 선형이다. 이 상수가 질의에 대해 성립하는 진짜 이유는 작업량이
 * 아니라 **kill의 결과**다: 세 질의 모두 쓰기가 0이라 deadline kill이 저장소에 아무것도 남기지 않고,
 * 그래서 30초에 잘려도 재시도가 언제나 가능하다. **변경 계열에는 그 성질이 없으므로 이 상수를 쓰지
 * 않는다**(`executeWorktreeOperation`은 승인된 `maxAttemptElapsedMs`를 쓴다).
 */
 ...
 */
function assertPlanWithinAssignment(task: OrchestrationTask, plan: TypedExecutionPlan): void {
  ...
  for (const op of plan.operations) {
    if (!assigned.includes(op.authorityId)) {
      throw new OrchestrationError(
        ...
        `계획의 operation이 이 task의 지시에 없다: ${op.operationId}(authority ${op.authorityId})`,
      );
    }
  }
}
...
export function resolveApprovedOperation(
  op: TypedWriteFileOperation | TypedRunProcessOperation | TypedGitWorktreeOperation,
...
function concurrentOwnershipOf(state: OrchestrationRunState, selfTaskId: string): string[] {
  ...
  for (const t of state.tasks) {
    if (t.taskId === selfTaskId) continue;
...
export function resolveWriteAuthority(
  ...
  if (auth.concurrentOwnership.some((own) => pathWithin(op.path, own))) {
    throw new OrchestrationError(
      "operation_ownership_contended",
      "지금 자원을 점유 중인 다른 task가 이 경로를 소유한다(동시 쓰기를 허용하지 않는다)",
    );
  }
