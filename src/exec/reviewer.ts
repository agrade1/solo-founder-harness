...
export interface ReviewVerdict {
  ...
  /** 리뷰어가 명시한 판정. `pass`만 통과이고, Critical 목록과 모순되면 애초에 만들어지지 않는다. */
...
export async function reviewDiff(inp: ReviewInput): Promise<ReviewVerdict> {
  ...
  if (!result) throw new ReviewGateError("reviewer_no_result", "리뷰어 스트림에 종료 결과가 없다");
  if (result.isError) {
  ...
  if (!m) throw new ReviewGateError("reviewer_verdict_invalid", "리뷰어 출력에 `## Verdict: pass|revise|block`이 없다");
  const verdict = m[1].toLowerCase() as ReviewVerdictKind;
  const critical = extractCriticalRisks(raw);
  if ((verdict === "pass") !== (critical.length === 0)) {
    throw new ReviewGateError(
      "reviewer_verdict_invalid",
      `verdict(${verdict})와 Critical 목록(${critical.length}건)이 모순이다 — 어느 쪽도 통과 근거로 쓰지 않는다`,
    );
  }
