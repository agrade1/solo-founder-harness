...
export async function reviewDiff(inp: ReviewInput): Promise<ReviewVerdict> {
  ...
  try {
    ...
    let stream: AsyncIterable<SessionEvent>;
    try {
...
function parseReviewResult(raw: string, subject: ReviewSubject): ParsedReview {
  ...
  for (const line of sections.get("Findings (P0/P1/P2)")!) {
    ...
    if (f) {
      ...
      findings.push({ severity: f[1] as ReviewFinding["severity"], text });
      continue;
    }
    if (NONE_RE.test(t)) {
      none = true;
 ...
 */
function assertSubjectSection(lines: string[], subject: ReviewSubject): void {
