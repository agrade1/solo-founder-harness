...
export interface PromptCompilerDeps {
  projectRoot: string; // inputs 상대경로 해석 기준
  readFile?: (absPath: string) => string | null; // 기본 fs
}
...
function isContract(path: string, spec: SessionSpec): boolean {
  if (spec.contractPaths?.length) return spec.contractPaths.includes(path);
  return /API_CONTRACT/i.test(basename(path));
}

/** SessionSpec → 착수 프롬프트. */
export function compilePrompt(spec: SessionSpec, deps: PromptCompilerDeps): string {
  const read = deps.readFile ?? defaultRead;
  const resolve = (p: string) => (isAbsolute(p) ? p : join(deps.projectRoot, p));
  ...
  if (spec.ownership?.length) specLines.push(`- 담당 경로(ownership, 이 밖은 수정 금지): ${spec.ownership.join(", ")}`);
  if (spec.forbidden?.length) specLines.push(`- 금지: ${spec.forbidden.join("; ")}`);
  parts.push(`# 담당 범위\n${specLines.join("\n")}`);

  // ③ API_CONTRACT 전문 인라인
  for (const c of contracts) {
    const content = read(resolve(c));
    if (content !== null) parts.push(`# API 계약 (반드시 준수 — 전문)\n<<< ${c} >>>\n${content.trim()}\n<<< 끝 >>>`);
    else parts.push(`# API 계약\n(경로 ${c} 를 찾지 못함 — 있으면 먼저 Read하라)`);
  }

  // ④ 배경 문서 = 경로 + Read 지시
  if (background.length) {
    parts.push(
      `# 배경 문서 (전부 읽지 말고 필요한 부분만 Read로 열어 참고)\n` +
