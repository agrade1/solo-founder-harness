...
export async function consumeExactlyOneTerminal(
  ...
  codes: TerminalCodes,
  maxEvents: number,
  ErrorType: new (code: string, message: string) => Error & { code: string },
): Promise<Extract<SessionEvent, { kind: "result" }>> {
  let result: Extract<SessionEvent, { kind: "result" }> | null = null;
  let seen = 0;
  try {
    for await (const e of stream) {
      if (++seen > maxEvents) throw new ErrorType(codes.unbounded, "스트림이 이벤트 상한을 넘었다");
      // 종료 뒤에는 아무 것도 오지 않는다 — 두 번째 종료 결과도, 늦은 assistant·status도 거부다.
      if (result) throw new ErrorType(codes.duplicate, "종료 결과 뒤에 이벤트가 더 왔다(종료는 정확히 1건이다)");
      if (e.kind === "result") result = e;
    }
  } catch (err) {
    if (err instanceof Error && typeof (err as { code?: unknown }).code === "string") throw err;
    throw new ErrorType(codes.streamFailed, "스트림 소비가 실패했다");
  }
  if (!result) throw new ErrorType(codes.noResult, "이 스트림에 종료 결과가 없다");
  if (result.isError) {
...
export function hasReadOnlyExecutionContract(provider: unknown): boolean {
  return (
    typeof provider === "object" &&
    provider !== null &&
    (provider as Record<symbol, unknown>)[READ_ONLY_EXECUTION_CONTRACT] === true
  );
}
