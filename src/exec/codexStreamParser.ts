 ...
 */
function meta(codexType: string, extra: Record<string, string | number | boolean | null> = {}): RawEvent {
...
export interface CodexParserContext {
  ...
  model: string;
  /** provider가 넘긴 실행 cwd(경계가 확인한 targetRoot). */
  cwd: string;
...
type Outcome = { reason: string; permission: boolean; text: string };
...
export class CodexJsonlParser {
  ...
  private failure: Outcome | null = null;
  /** 성공 종료 이벤트를 봤는가(중복 판정용). */
  private success: Outcome | null = null;
  private lastMessage = "";
   ...
   */
  protocolFail(reason: string, detail = ""): void {
  ...
  private fail(reason: string, text: string, permission = false): void {
    if (!this.failure) this.failure = { reason, text, permission };
  }

  private marker(type: string, extra: Record<string, string | number | boolean | null> = {}): SessionEvent {
  ...
  private event(raw: RawEvent, out: SessionEvent[]): void {
    ...
    switch (raw.type) {
      ...
      case "item.updated": {
        ...
        if (itemType === "mcp_tool_call") return this.mcpViolation(out);
      ...
      case "turn.completed":
        if (!this.success && !this.failure) this.usage = usageOf(raw.usage);
  ...
  private itemCompleted(raw: RawEvent, postTerminal: boolean, out: SessionEvent[]): void {
    const item = (raw.item ?? {}) as Record<string, unknown>;
    const itemType = bounded(item.item_type ?? item.type, 64) || "unknown";
  ...
  private mcpViolation(out: SessionEvent[]): void {
   ...
   */
  private recordTerminal(o: Outcome, isFailure: boolean, out: SessionEvent[]): void {
    ...
    if (isFailure) this.fail(o.reason, o.text, o.permission);
    else this.success = o;
  }
}
