/**
 * claude -p --output-format stream-json 의 NDJSON을 SessionEvent로 정규화한다.
...
function toUsage(u: unknown): SessionUsage {
...
function readContent(message: unknown): { text: string; toolUses: ToolUse[]; stopReason: string | null } {
...
export function normalize(raw: RawEvent): SessionEvent {
...
export function parseLine(line: string): SessionEvent | null {
  ...
  if (!obj || typeof obj !== "object" || typeof (obj as RawEvent).type !== "string") return null;
  return normalize(obj as RawEvent);
}

/** 여러 줄(완결된 텍스트) → 이벤트 배열. */
export function parseAll(text: string): SessionEvent[] {
