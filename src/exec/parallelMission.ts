...
export interface RunParallelMissionOpts {
  ...
  baseBranch?: string;
  startStage?: DegradeStage;
  threshold?: DegradeThreshold;
  reviewRounds?: number;
  concurrency?: number; // 동시 실행 세션 상한 (기본 3)
  now?: () => number;
...
export async function runParallelMission(opts: RunParallelMissionOpts): Promise<MissionReport> {
  ...
  while (remaining.size > 0) {
    ...
    if (waveWaitMs > 0 && remaining.size > 0) await sleep(waveWaitMs);
  }
  ...
  const totalUsage = tasks.reduce(
    (a, t) => ({ inputTokens: a.inputTokens + (t.usage?.inputTokens ?? 0), outputTokens: a.outputTokens + (t.usage?.outputTokens ?? 0) }),
