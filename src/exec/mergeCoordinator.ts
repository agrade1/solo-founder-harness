...
export type MergeStatus = "merged" | "conflict" | "gate_failed" | "error";
...
export interface MergeSerialOpts {
  repoRoot: string;
  base: string; // 예: develop
  items: MergeItem[];
  regate?: boolean; // 병합 후 L1 게이트 재실행 (기본 true)
  cleanup?: boolean; // 병합 성공한 worktree 제거 (기본 true, 브랜치는 보존)
}

/** items를 순서대로 base에 병합. 각 항목 결과를 반환(부분 성공 허용). */
export async function mergeSerial(opts: MergeSerialOpts): Promise<MergeOutcome[]> {
  const regate = opts.regate !== false;
  const results: MergeOutcome[] = [];

  for (const it of opts.items) {
    const base: MergeOutcome = { taskId: it.taskId, branch: it.branch, status: "error" };
    try {
