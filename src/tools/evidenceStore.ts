 ...
 *
 * **저장 응답은 파일, 중앙은 포인터**(로드맵 §3.2). 외부에서 들여온 **응답 바이트**는
 * content-addressed 파일로만 남고, kernel state·프롬프트에 실리는 것은 `EvidenceItem`(포인터 +
 * bounded 발췌)뿐이다.
 *
 * [C-126/A-7] 여기서 다루는 것은 **웹 페이지 원문이 아니다**: 열려 있는 경로는 search뿐이고 그것이
 * 담는 것은 Tavily search 응답의 `content` 필드(스니펫)다. `sha256`도 **저장한 응답 바이트**의 것이다
 * — "원문 검증"으로 읽히면 그것이 곧 과대주장이다(extract는 봉인 상태 · `researchGateway` §5.4).
 *
 * 새 SoR을 만들지 않는다 — JSONL은 사람이 읽는 인덱스이고 권위는 파일 내용(sha256)에 있다.
 * `liveEvidence.ts`와 다른 것이다: 그쪽은 live acceptance의 **metrics 전용**이고 경로·응답 바이트를
 * 담지 않는다.
 */
...
/** 저장 응답 1건의 byte 상한. 넘으면 저장하지 않고 거부한다(조용한 절삭이 아니다). */
...
/** 중앙·프롬프트가 운반하는 유일한 형태. **응답 본문 필드가 없다.** */
export interface EvidenceItem {
  source: string; // 응답이 가리킨 URL
  sha256: string; // **저장한 응답 바이트**의 digest = 파일 신원 (웹 원문의 해시가 아니다)
  retrievedAt: string; // ISO
  bytes: number; // 저장 응답 byte 수
...
/**
 * 응답 바이트를 content-addressed 파일로 저장하고 포인터를 돌려준다.
 * 같은 입력 → 같은 바이트 · 같은 경로(결정성). 이미 있는 응답은 다시 쓰지 않는다.
 */
export function storeEvidence(dir: string, input: StoreEvidenceInput): EvidenceItem {
  ...
  if (bytes > MAX_RAW_BYTES) {
    throw new EvidenceError("raw_too_large", `저장 응답이 상한 ${MAX_RAW_BYTES} bytes를 넘는다: ${bytes}`);
  }
  ...
  try {
    ...
    try {
      ...
      throw new EvidenceError("evidence_raw_unreadable", `기존 저장 응답 파일을 읽을 수 없다: ${rawPath} (${(re as Error).message})`);
    }
    const actual = createHash("sha256").update(onDisk).digest("hex");
    if (actual !== sha256) {
      throw new EvidenceError(
        "evidence_hash_mismatch",
        `content-addressed 경로에 다른 바이트가 있다: ${rawPath} (기대 ${sha256} · 실제 ${actual}) — 덮어쓰지 않고 거부한다`,
      );
    }
  }
