...
const ALLOWED_TOOLS: readonly string[] = deepFreeze(["get_project_registries", "list_items_in_registries", "search_items_in_registries", "view_items_in_registries", "get_item_examples_from_registries"]);
...
const EXPECTED_BARE_7: readonly string[] = deepFreeze([...ALLOWED_TOOLS, ...FORBIDDEN_TOOLS].sort());
...
export function getForbiddenTools(): string[] {
  return [...FORBIDDEN_TOOLS];
}
export function getExpectedBare7(): string[] {
  return [...EXPECTED_BARE_7];
}
export function isAllowedTool(bare: string): boolean {
  return ALLOWED_TOOLS.includes(bare);
}
export function isForbiddenTool(bare: string): boolean {
  return FORBIDDEN_TOOLS.includes(bare);
}
export function nsName(bare: string): string {
  return `mcp__${SHADCN_SERVER}__${bare}`;
}
export function isAllowedProtocolVersion(v: unknown): v is string {
...
function assertOnlyKeys(obj: Record<string, unknown>, allowed: string[]): void {
  for (const k of Object.keys(obj)) if (!allowed.includes(k)) throw new ShadcnPolicyError("extra_key", "허용되지 않은 인자 key");
}
function assertExactStrArray(v: unknown, expected: string[], label: string): void {
  if (!Array.isArray(v) || v.length !== expected.length || v.some((x, i) => x !== expected[i])) throw new ShadcnPolicyError("bad_arg", `${label}가 정확히 [${expected.join(",")}] 아님`);
}
function assertIntRange(v: unknown, min: number, max: number, label: string): void {
  if (typeof v !== "number" || !Number.isInteger(v) || v < min || v > max) throw new ShadcnPolicyError("bad_arg", `${label}가 정수 ${min}~${max} 범위 아님`);
}
function assertQuery(v: unknown): void {
  if (typeof v !== "string" || v.length < 1 || v.length > 200) throw new ShadcnPolicyError("bad_arg", "query가 1~200자 문자열 아님");
  if (CONTROL_RE.test(v)) throw new ShadcnPolicyError("bad_arg", "query에 제어문자 포함");
}
 ...
 */
export function validateToolArgs(bare: string, args: unknown): Record<string, unknown> {
  if (isForbiddenTool(bare)) throw new ShadcnPolicyError("forbidden_tool", "금지 도구");
  if (!isAllowedTool(bare)) throw new ShadcnPolicyError("unknown_tool", "미확정 도구");
  if (!isPlainObject(args)) throw new ShadcnPolicyError("bad_arg", "arguments가 객체 아님");

  switch (bare) {
    case "get_project_registries":
      if (Object.keys(args).length !== 0) throw new ShadcnPolicyError("extra_key", "get_project_registries는 빈 객체만 허용");
      return {};
    case "list_items_in_registries":
      ...
      if ("limit" in args) assertIntRange(args.limit, 1, 20, "limit");
      if ("offset" in args) assertIntRange(args.offset, 0, 1000, "offset");
      return { ...args };
    ...
    case "view_items_in_registries": {
      ...
      if (!Array.isArray(items) || items.length < 1 || items.length > 10) throw new ShadcnPolicyError("bad_arg", "items가 1~10개 배열 아님");
      for (const it of items) {
        if (typeof it !== "string" || it.length < 1 || it.length > 200) throw new ShadcnPolicyError("bad_arg", "item이 1~200자 문자열 아님");
        if (!it.startsWith("@shadcn/")) throw new ShadcnPolicyError("bad_arg", "item이 @shadcn/ prefix 아님");
        if (it.includes("..")) throw new ShadcnPolicyError("bad_arg", "item에 traversal(..) 포함");
        if (it.includes("://")) throw new ShadcnPolicyError("bad_arg", "item에 URL 포함");
        if (CONTROL_RE.test(it)) throw new ShadcnPolicyError("bad_arg", "item에 제어문자 포함");
      }
      return { items: [...items] };
    }
 ...
 */
export function restrictedToolList(): { name: string; description: string; inputSchema: Record<string, unknown> }[] {
