...
export function assertValidSecretRefs(refs: string[], profileId: string): void {
  ...
  refs.forEach((r, i) => {
    if (!isValidSecretRef(r)) badIndexes.push(i);
  });
  if (badIndexes.length > 0) {
    throw new Error(
      `profile '${profileId}': secretRefs는 환경변수 이름(^[A-Z][A-Z0-9_]*$)만 허용한다(값 금지). ` +
...
export function redactSecrets(text: string, values: string[] = []): string {
  ...
  for (const v of values) {
    if (v) out = out.split(v).join(REDACTED); // 비어있지 않은 값은 길이와 무관하게 치환 (1~3자 포함)
  }
