...
export interface ShadcnToolSchema {
  name: string;
  title?: string; // optional (공식 Tool.title)
  description?: string; // optional string (공식 Tool.description)
  inputSchema: Record<string, unknown>; // root type:"object" 강제
  outputSchema?: Record<string, unknown>; // 존재 시 root type:"object" 강제
  annotations?: Record<string, unknown>; // untrusted hint — 권한 판정 근거로 쓰지 않는다
}

export interface ShadcnSchemaSnapshot {
  ...
  tools: ShadcnToolSchema[]; // name 정렬. raw protocol payload 아님(추출 schema만, value scrub).
  configHash: string;
  timestamp: string;
}

export interface ShadcnSchemaOperationSummary {
  initialize: number;
  initialized: number;
  toolsListPages: number;
  toolCalls: 0; // 항상 0 — tools/call 코드 경로 없음
}

export interface ShadcnSchemaResult {
  schemaDiscovery: true;
  snapshotPath: string;
  snapshot: ShadcnSchemaSnapshot;
  operationSummary: ShadcnSchemaOperationSummary;
}

export interface RunShadcnSchemaProbeOpts {
  serviceCwd: string;
  runtimeDir: string;
  now: () => string;
  timeoutMs?: number;
  /** scrub 전용 secret 이름. child env로 전달하지 않는다. */
  redactNames?: string[];
}
 ...
 */
function assertSchemaSafe(v: unknown, depth: number, scrub: (s: string) => string): void {
  if (depth > MAX_SCHEMA_DEPTH) throw new ShadcnSchemaProbeError("bad_schema", `schema 깊이 ${MAX_SCHEMA_DEPTH} 초과`);
  if (typeof v === "string") {
    if (Buffer.byteLength(v, "utf8") > MAX_STRING_BYTES) throw new ShadcnSchemaProbeError("bad_schema", `schema 문자열 ${MAX_STRING_BYTES} byte 초과`);
    return;
  }
  if (Array.isArray(v)) {
    if (v.length > MAX_OBJECT_KEYS) throw new ShadcnSchemaProbeError("bad_schema", `schema 배열 길이 ${MAX_OBJECT_KEYS} 초과`);
    for (const x of v) assertSchemaSafe(x, depth + 1, scrub);
    return;
  }
  if (isPlainObject(v)) {
    const keys = Object.keys(v);
    if (keys.length > MAX_OBJECT_KEYS) throw new ShadcnSchemaProbeError("bad_schema", `schema 객체 키 수 ${MAX_OBJECT_KEYS} 초과`);
    for (const k of keys) {
      if (Buffer.byteLength(k, "utf8") > MAX_STRING_BYTES) throw new ShadcnSchemaProbeError("bad_schema", "schema 키 크기 초과");
      // key는 scrub으로 마스킹할 수 없다(schema 왜곡) → scrub 대상이면 fail-closed(원 key 미노출).
      if (scrub(k) !== k) throw new ShadcnSchemaProbeError("secret_in_schema_key", "schema object key가 secret/credential 형태로 감지됨 (key는 표시하지 않음)");
...
function deepScrub<T>(v: T, scrub: (s: string) => string): T {
  if (typeof v === "string") return scrub(v) as unknown as T;
  if (Array.isArray(v)) return v.map((x) => deepScrub(x, scrub)) as unknown as T;
  if (isPlainObject(v)) {
    const out: Record<string, unknown> = {};
    for (const [k, val] of Object.entries(v)) out[k] = deepScrub(val, scrub);
...
function assertObjectSchema(v: unknown, label: string, scrub: (s: string) => string): Record<string, unknown> {
  if (!isPlainObject(v)) throw new ShadcnSchemaProbeError("tool_missing_field", `${label}(객체) 누락`);
  if (v.type !== "object") throw new ShadcnSchemaProbeError("bad_schema", `${label} root type이 "object"가 아님`);
  assertSchemaSafe(v, 0, scrub);
  return v;
}

/** tool 객체 하나를 공식 계약대로 검증·추출한다(bare name; raw envelope 아님). */
function extractTool(raw: unknown, scrub: (s: string) => string): ShadcnToolSchema {
  if (!isPlainObject(raw)) throw new ShadcnSchemaProbeError("tool_missing_field", "tool이 객체가 아님");
  const name = raw.name;
  if (typeof name !== "string" || name.length === 0) throw new ShadcnSchemaProbeError("tool_missing_field", "tool.name 누락");
  if (Buffer.byteLength(name, "utf8") > MAX_STRING_BYTES) throw new ShadcnSchemaProbeError("bad_schema", "tool.name 크기 초과");

  const tool: ShadcnToolSchema = { name, inputSchema: assertObjectSchema(raw.inputSchema, `tool '${name}': inputSchema`, scrub) };

  if (raw.title !== undefined) {
    if (typeof raw.title !== "string") throw new ShadcnSchemaProbeError("bad_schema", `tool '${name}': title이 문자열 아님`);
    tool.title = raw.title;
  }
  if (raw.description !== undefined) {
    if (typeof raw.description !== "string") throw new ShadcnSchemaProbeError("bad_schema", `tool '${name}': description이 문자열 아님`);
  ...
  if (raw.annotations !== undefined) {
    if (!isPlainObject(raw.annotations)) throw new ShadcnSchemaProbeError("bad_schema", `tool '${name}': annotations가 plain object 아님`);
  ...
  if (Buffer.byteLength(JSON.stringify(tool), "utf8") > MAX_TOOL_BYTES) throw new ShadcnSchemaProbeError("bad_schema", `tool '${name}' 크기 ${MAX_TOOL_BYTES} byte 초과`);
  return tool;
}

export async function runShadcnSchemaProbe(opts: RunShadcnSchemaProbeOpts): Promise<ShadcnSchemaResult> {
  ...
  const norm = (code: string, message: string) => new ShadcnSchemaProbeError(code, scrub(message));
  ...
  try {
    ...
    if (!entry || !("command" in entry)) throw norm("config_server", "shadcn stdio 서버 엔트리가 없음");
    ...
    if (e instanceof ShadcnSchemaProbeError) throw new ShadcnSchemaProbeError(e.code, scrub(e.message));
    if (e instanceof McpConfigError) throw norm(`config_${e.code}`, e.message);
    throw norm("config", (e as Error).message);
  }

  const collected = await new Promise<{ protocolVersion: string; serverInfo: { name: string; version: string }; tools: ShadcnToolSchema[]; toolsListPages: number }>(
    (resolveP, reject) => {
      let settled = false;
      ...
      const tools: ShadcnToolSchema[] = [];
      const names = new Set<string>();
      let protocolVersion = "";
      let serverInfo: { name: string; version: string } = { name: "", version: "" };
      let done: { protocolVersion: string; serverInfo: { name: string; version: string }; tools: ShadcnToolSchema[]; toolsListPages: number } | null = null;
      ...
      const handleMessage = (msg: Record<string, unknown>) => {
        if (msg.jsonrpc !== "2.0") throw new ShadcnSchemaProbeError("jsonrpc_version", "jsonrpc가 '2.0'이 아님");
        if (!("id" in msg) || msg.id === undefined) return; // 서버 notification → 무시
        if (msg.id !== pendingId) throw new ShadcnSchemaProbeError("jsonrpc_id_mismatch", "응답 id가 요청 id와 불일치");
        if (msg.error !== undefined) throw new ShadcnSchemaProbeError(phase === "init" ? "init_error" : "tools_error", `${phase} JSON-RPC error 응답`);
        const result = msg.result;
        if (!isPlainObject(result)) throw new ShadcnSchemaProbeError(phase === "init" ? "init_error" : "tools_error", `${phase} result 누락`);
        ...
        if (phase === "init") {
          ...
          if (!isPlainObject(result.capabilities)) throw new ShadcnSchemaProbeError("capabilities", "capabilities가 plain object 아님");
          if (!("tools" in result.capabilities)) throw new ShadcnSchemaProbeError("capabilities", "capabilities.tools 부재");
        ...
        if (!Array.isArray(list)) throw new ShadcnSchemaProbeError("tools_error", "tools/list result.tools가 배열이 아님");
        for (const raw of list) {
          const tool = extractTool(raw, scrub);
          const fullName = `mcp__${SHADCN_SERVER}__${tool.name}`; // 직접 서버는 bare 이름 → host namespacing
          if (names.has(fullName)) throw new ShadcnSchemaProbeError("duplicate_tool", "중복 도구");
          names.add(fullName);
          tool.name = fullName;
          tools.push(tool);
          if (names.size > MAX_TOOLS) throw new ShadcnSchemaProbeError("too_many_tools", `도구 수 ${MAX_TOOLS} 초과`);
        }
        const nextCursor = result.nextCursor;
        if (nextCursor !== undefined && nextCursor !== null) {
          if (typeof nextCursor !== "string" || nextCursor.length === 0) throw new ShadcnSchemaProbeError("tools_error", "nextCursor가 유효 문자열 아님");
          if (cursorsSeen.has(nextCursor)) throw new ShadcnSchemaProbeError("repeat_cursor", "반복 cursor(페이지네이션 루프)");
          cursorsSeen.add(nextCursor);
          if (toolsListPagesSent >= MAX_PAGES) throw new ShadcnSchemaProbeError("too_many_pages", `tools/list 페이지 ${MAX_PAGES} 초과`);
      ...
      const onLine = (line: string) => {
        ...
        if (!isPlainObject(msg)) throw new ShadcnSchemaProbeError("malformed_line", "JSON-RPC 메시지가 객체가 아님");
        handleMessage(msg);
      };
      ...
      child.stdout.on("data", (d: Buffer) => {
        if (settled || done) return; // 수집 완료 후 라인은 무시
        ...
        try {
          ...
          fail((e as ShadcnSchemaProbeError).code ?? "verify", (e as Error).message);
        }
      });
      ...
      child.on("close", (code) => {
        ...
        if (code !== 0) fail("nonzero_exit", `MCP 서버 비정상 종료 (code ${code}): ${stderr.trim() || "(stderr 없음)"}`);
        else fail(phase === "init" ? "no_init" : "no_tools", `tools/list 완료 전 종료 (code ${code})`);
      });
  ...
  const snapshot: ShadcnSchemaSnapshot = {
    mode: "schema-discovery",
    usableForHandoff: false,
  ...
  try {
    ...
    throw norm("persist", `schema snapshot 기록 실패 (기존 파일/symlink?): ${(e as Error).message}`);
  }

  const operationSummary: ShadcnSchemaOperationSummary = { initialize: 1, initialized: 1, toolsListPages: collected.toolsListPages, toolCalls: 0 };
