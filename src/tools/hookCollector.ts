...
import { normalizeHook, MAX_STDIN_BYTES, type CollectorConfig, type HookKind, type ToolTraceRecord } from "./toolTrace.js";
...
function isBlocking(hookKind: HookKind): boolean {
...
export interface CollectResult {
  exitCode: number;
  record: ToolTraceRecord | null;
  wrote: boolean;
  stderr: string[];
}

/** argv[2]=hookKind, argv[3]="deny"?. deny는 PreToolUse에서만 허용. 유효하지 않으면 null. */
export function parseArgs(argv: string[]): { hookKind: HookKind; deny: boolean } | null {
  const kind = argv[2];
  if (!kind || !HOOK_KINDS.includes(kind as HookKind)) return null;
  const deny = argv[3] === "deny";
  if (argv[3] !== undefined && argv[3] !== "deny") return null; // 알 수 없는 3번째 인자
  if (deny && kind !== "PreToolUse") return null; // deny는 PreToolUse만
  return { hookKind: kind as HookKind, deny };
}

export type ConfigResult = { ok: true; config: CollectorConfig } | { ok: false; reason: string };
...
export function parseConfig(env: NodeJS.ProcessEnv): ConfigResult {
  ...
  if (!tracePath) return { ok: false, reason: "missing_trace_path" };
  ...
  if (env.HARNESS_TOOL_MAP !== undefined) {
    ...
    for (const [k, v] of Object.entries(parsed as Record<string, string>)) clean[k] = v;
    toolMap = clean;
  }
...
export function collect(input: CollectInput): CollectResult {
  ...
  try {
    if (Buffer.byteLength(payloadRaw, "utf8") > MAX_STDIN_BYTES) throw new Error("oversized");
  ...
  try {
    ...
    const code = (e as { code?: string }).code ?? "normalize_error";
  ...
  if (deny) return { exitCode: 2, record, wrote: true, stderr: ["PreToolUse deny matcher → tool_denied 기록 후 차단(exit 2)"] };
  return { exitCode: 0, record, wrote: true, stderr: [] };
}

/** args + env(config) 실패까지 포함해 종료코드를 결정한다. */
export function runCollector(input: { argv: string[]; env: NodeJS.ProcessEnv; payloadRaw: string; now: string }): CollectResult {
...
export function main(): void {
  ...
  for (const line of res.stderr) process.stderr.write(line + "\n");
